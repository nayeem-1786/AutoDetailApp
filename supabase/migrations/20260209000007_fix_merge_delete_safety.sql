-- Fix merge_customers():
-- 1. Add safety guard to DELETE: AND id != keep_id
-- 2. Clear auth_user_id on merged records before copying to keep (UNIQUE constraint)
-- 3. Clear square_customer_id on merged records before copying to keep (UNIQUE constraint)
...
BEGIN
  ...
  -- Clear UNIQUE columns on merged records BEFORE we copy them to keep
  ...
  LOOP
    ...
    -- Square IDs: fill if kept is NULL (already cleared on merged records above)
    ...
    -- Auth user ID: fill if kept is NULL (already cleared on merged records above)
  ...
  -- Delete merged customers (safety: never delete keep_id)
