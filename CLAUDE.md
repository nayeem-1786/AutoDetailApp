...
- **Middleware IP restriction**: `getClientIp()` treats `::1`/`127.0.0.1` as null (local dev). IP check only enforces when a real public IP is present (`clientIp && !ips.includes(clientIp)`). Local/dev connections always pass through. Cache TTL is 10 seconds.
