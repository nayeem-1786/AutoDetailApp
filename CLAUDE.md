...
20. **Money (Money-Unify epic in progress, Phases Unify-1 + Unify-2 + Unify-3 complete)**: New money-handling code MUST use integer cents — column suffix `_cents`, variable suffix `Cents` (camelCase) or `_cents` (snake_case). Canonical helpers live in `src/lib/utils/money.ts`: `toCents()`, `fromCents()`, `STRIPE_MIN_AMOUNT_CENTS = 50`, `STRIPE_MIN_DOLLARS` (derived). Format helpers in `src/lib/utils/format.ts`: `formatMoney(cents)` for all display, `formatMoneyForInput(cents)` for controlled dollar-edit inputs. The legacy `formatCurrency(dollars)` survives the epic and is removed at Unify-Final. Loyalty: `LOYALTY.REDEEM_RATE_CENTS = 5` for cents-context math (alongside legacy `LOYALTY.REDEEM_RATE = 0.05`). The `money/no-unsuffixed-money-prop` ESLint rule (`eslint-rules/money-no-unsuffixed-money-prop.js`, currently `'warn'`, scheduled for `'error'` at Unify-Final once all family phases land) flags violations at write time. Existing dollars-canonical code in families not yet migrated stays as-is until that family's phase. The Stripe minimum value (50 cents = $0.50) is duplicated at the DB layer via `appointments.payment_link_amount_cents_check` — changing it requires both a code change AND a migration. Loyalty `REDEEM_RATE` is duplicated as both the float (`0.05`) and the integer (`5`) until Unify-Final. **Phase Unify-3 (Family D — Catalog) form-state pattern**: admin service-edit form interfaces (`ScopeTier.price`, `SpecialtyTier.price`, `FlatPricing.flat_price`, `CustomPricing.custom_starting_price`, `PerUnitPricing.per_unit_price`, `VehicleSizePricing.sedan` etc. in `src/components/service-pricing-form.tsx`) hold DOLLARS under bare names; parent admin pages convert `fromCents()` at DB load and `toCents()` at DB save. Booking wire (`bookingSubmitSchema.price_cents` + `bookingAddonSchema.price_cents`) carries integer cents; the route collapses cents → dollars at entry so downstream Family C/A writes stay in dollars until those families migrate (cleanup TODOs tagged `// TODO Unify-5` / `Unify-6` / `Unify-7` / `Unify-8` at every shim site). POS reducers (`ticket-reducer.ts` + `quote-reducer.ts`) convert cents → dollars at the `resolveServicePriceWithSale()` boundary so `TicketItem.unitPrice` / `standardPrice` stay in dollars (Family A — Unify-5). See `docs/dev/MONEY.md` for full canonical-model documentation, opt-out details, and the per-family migration status table.
