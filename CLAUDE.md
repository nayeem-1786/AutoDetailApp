...
- Sticky ticker: Ticker renders ABOVE header (first thing on page). Ticker is `sticky top-0 z-50`, header is `sticky z-40` with `top: var(--ticker-height, 0px)`. `useTickerHeight` hook in `TopBarTickerFiltered` sets `--ticker-height` CSS variable on `:root` via `ResizeObserver`. When no tickers visible, `--ticker-height: 0px` and header sticks at top-0.
- Hero section WCAG readability fixes + per-slide color overrides:
  - Light mode fix: `data-hero-scope` + inline CSS var defaults force white text on dark overlay regardless of theme toggle
  - Memorial Day accent: `#1e40af` → `#60a5fa` (visible on dark backgrounds, 8.6:1 contrast)
  - Light mode CTA: `#65a30d` → `#4d7c0f` (4.6:1, passes WCAG AA normal text)
  - Per-slide color overrides: 6 nullable columns (`text_color`, `subtitle_color`, `accent_color`, `overlay_color`, `cta_bg_color`, `cta_text_color`). Admin collapsible Color Overrides section with hex input + color picker + reset. Frontend applies via inline CSS vars.
...
#### Session 2 — Cart Page, Stripe Checkout, Orders (Done)
- Database: `orders` + `order_items` tables (migration `20260217000001_orders.sql`), RLS policies (customer view own, service role full), indexes, `update_updated_at` trigger
...
- **Business settings API**: `GET/PATCH /api/admin/settings/business` — generic endpoint for reading/writing `business_settings` table keys. GET takes `?key=xxx` query param, PATCH takes `{ key, value }` body. Uses upsert with `onConflict: 'key'`. Used by tickers admin (`ticker_enabled`) and ads admin (`ads_enabled`).
- **`setFeatureFlag()` pattern**: Uses update → fallback insert. If the flag row doesn't exist (e.g., migration not applied), it auto-creates the row with name derived from key. NEVER use raw `.update()` on `feature_flags` without checking the result — it silently does nothing if the row doesn't exist.
- **Particle canvas z-index**: `z-30` — above page content, below sticky header (z-50), nav dropdowns (z-50), cart drawer (z-90+), and mobile menu (z-100).
...
- **Admin page** (`/admin/website/footer/page.tsx`): Full footer management UI with 3 collapsible section cards (Main Footer, Service Areas, Bottom Bar).
  ...
  - **Main Footer**: Column CRUD (max 4), drag-and-drop reorder, content type badges (Links/HTML/Business Info)
  - **Links editor**: Add/edit/delete/reorder links, toggle active, open-in-new-tab checkbox
  - **HTML editor**: Textarea + save + dark-background preview
  - **Business Info**: Read-only info panel with link to business settings
  - **Service Areas**: Prefix text editor saves to `footer_sections.config.prefix_text`, link to city pages
  - **Bottom Bar**: Custom copyright text (optional, saves to config), bottom link CRUD
- **Website index card**: Added "Footer" card with Rows3 icon to `/admin/website/page.tsx`
- Uses all Session 1 API routes — no new routes created
- TypeScript clean, build passes.

### Configurable Footer System — Session 2 (Frontend)
- **New content type**: Migration `20260219000003` adds `business_info` to `footer_columns.content_type`.
- **Rewrites**: `site-footer.tsx` (server, accepts `footerData` prop), `footer-client.tsx` (client, dynamic rendering from FooterData with 3 conditional sections)
- All 3 layouts pass `footerData` directly. TypeScript clean, build passes.

### Configurable Footer System — Session 1 (DB + API)
- **Migration**: `footer_sections` (3 seeded), `footer_columns` (2 seeded), `footer_bottom_links` (1 seeded), `footer_column_id` FK on `website_navigation`
- **Data layer**: `getFooterData()` cached, **API routes** (5 files under `/api/admin/footer/`), cache revalidation wired
...
- **SiteHeader**: Split into server wrapper (`site-header.tsx`) + new client component (`header-client.tsx`). Scroll-aware backdrop blur, animated dropdown menus, mobile hamburger with motion height animation, lime "Book Now" CTA, utility bar with phone + "Mobile Service" info
- **HeroCarousel**: Framer-motion slide transitions with `custom` direction, bottom-aligned content, lime CTAs, `overlay_opacity` 0-100 scale, first slide `<h1>` for SEO
- **SiteFooter**: Split into server wrapper + client component (`footer-client.tsx`). Trust badges strip, 12-column grid, contact info with lime accents, Google/Yelp review badges, service area city links
