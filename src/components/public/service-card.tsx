...
export function ServiceCard({ service, categorySlug }: ServiceCardProps) {
  ...
  return (
    <Link href={`/services/${categorySlug}/${service.slug}`} className="group block">
      <div className="h-full rounded-2xl bg-brand-surface p-6 border border-white/10 transition-shadow hover:shadow-md">
        <div className="flex items-start justify-between">
          <h3 className="font-display text-lg font-semibold text-white group-hover:text-lime transition-colors">
            {service.name}
          </h3>
          <div className="ml-3 flex-shrink-0 text-gray-400 group-hover:text-lime transition-colors">
