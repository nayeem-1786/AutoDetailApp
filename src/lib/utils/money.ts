/**
 * Money helpers — canonical module for all money math in the codebase.
 *
 * Renamed from refund-math.ts → money.ts in Phase Money-Unify-1 because
 * the file's scope outgrew refunds: it now exports the canonical
 * cents/dollars conversion pair (toCents, fromCents) plus the
 * single-source-of-truth Stripe minimum constants used by booking,
 * pay-link, refund, and checkout surfaces.
 *
 * The old refund-math.ts path survives as a deprecated re-export shim
 * through the Money-Unify epic; it is deleted in Unify-Final once every
 * importer has migrated to '@/lib/utils/money'. See docs/dev/MONEY.md.
 */

/**
 * Minimum charge Stripe accepts, in cents. $0.50 USD = 50 cents.
 * Single source of truth — all server and client enforcement sites
 * import this constant.
 *
 * NOTE: This value is also enforced at the DB layer via the
 * appointments.payment_link_amount_cents_check constraint
 * (CHECK col >= 50). Changing it requires both a code change AND
 * a migration to update the constraint. See MONEY.md "Cross-system
 * synchronization points".
 */
export const STRIPE_MIN_AMOUNT_CENTS = 50;

/**
 * Minimum charge Stripe accepts, in dollars. Derived from
 * STRIPE_MIN_AMOUNT_CENTS. Use for dollars-context comparisons;
 * use STRIPE_MIN_AMOUNT_CENTS for cents-context.
 *
 * NOTE: This value is also enforced at the DB layer via the
 * appointments.payment_link_amount_cents_check constraint
 * (CHECK col >= 50). Changing it requires both a code change AND
 * a migration to update the constraint. See MONEY.md.
 */
export const STRIPE_MIN_DOLLARS = STRIPE_MIN_AMOUNT_CENTS / 100;
