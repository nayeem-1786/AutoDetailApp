...
export async function GET(request: NextRequest) {
  try {
    ...
    // Permission checks: reports.export + reports.quickbooks_status
    const exportDenied = await requirePermission(employee.id, 'reports.export');
    if (exportDenied) return exportDenied;
    const qboDenied = await requirePermission(employee.id, 'reports.quickbooks_status');
    if (qboDenied) return qboDenied;
