...
interface ProcessedCampaignResult {
  campaign_id: string;
  name: string;
...
interface ProcessScheduledResult {
  processed: number;
  campaigns: ProcessedCampaignResult[];
}
...
/**
 * Find and dispatch all campaigns whose status='scheduled' and scheduled_at <= now().
 * Shared by GET (cron at every 5 min via scheduler.ts) and POST (admin-session "process now").
 * Not exported — Next.js route files only permit HTTP-method exports (GET/POST/etc.).
 */
async function runProcessScheduled(): Promise<ProcessScheduledResult> {
  ...
  const results: ProcessedCampaignResult[] = [];
...
/**
 * Cron entry point. Invoked every 5 min by src/lib/cron/scheduler.ts.
 * Auth: x-api-key header against CRON_API_KEY (matches lifecycle-engine pattern).
 */
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await runProcessScheduled();
    return NextResponse.json({ data });
  } catch (err) {
    console.error('Process scheduled campaigns error (cron):', err);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}

/**
 * Admin-session entry point. Reserved for a future admin "process now" button.
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error('Process scheduled campaigns error (admin):', err);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
