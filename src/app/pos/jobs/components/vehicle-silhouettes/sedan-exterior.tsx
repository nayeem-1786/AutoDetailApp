...
/**
 * Sedan exterior silhouette — professional Illustrator artwork, landscape top-down view
 * Car faces RIGHT: nose=right, tail=left, driver=top, passenger=bottom
...
export function SedanExterior({ photoCounts, onZoneTap }: SilhouetteProps) {
  ...
  return (
    <svg viewBox="30 370 2100 1430" className="mx-auto w-full max-w-[500px]">
      ...
      {/* Layout: Rear=LEFT, Front=RIGHT, Driver=TOP, Passenger=BOTTOM */}

      {/* Rear (left end) */}
      <rect x={70} y={770} width={255} height={620} rx={20}
        fill={zc('exterior_rear')} stroke={zb('exterior_rear')}
        strokeWidth={7} opacity={0.7} cursor="pointer"
        onClick={() => onZoneTap('exterior_rear')}
      />
      <text x={198} y={1095} textAnchor="middle" fontSize={42} fill="#374151" fontWeight="500">Rear</text>

      {/* Trunk */}
      <rect x={330} y={810} width={385} height={540} rx={15}
        fill={zc('exterior_trunk')} stroke={zb('exterior_trunk')}
        strokeWidth={7} opacity={0.7} cursor="pointer"
        onClick={() => onZoneTap('exterior_trunk')}
      />
      <text x={523} y={1095} textAnchor="middle" fontSize={44} fill="#374151" fontWeight="500">Trunk</text>

      {/* Roof (center) */}
      <rect x={720} y={840} width={790} height={480} rx={15}
        fill={zc('exterior_roof')} stroke={zb('exterior_roof')}
        strokeWidth={7} opacity={0.7} cursor="pointer"
        onClick={() => onZoneTap('exterior_roof')}
      />
      <text x={1115} y={1095} textAnchor="middle" fontSize={48} fill="#374151" fontWeight="500">Roof</text>

      {/* Hood */}
      <rect x={1520} y={810} width={355} height={540} rx={15}
        fill={zc('exterior_hood')} stroke={zb('exterior_hood')}
        strokeWidth={7} opacity={0.7} cursor="pointer"
        onClick={() => onZoneTap('exterior_hood')}
      />
      <text x={1698} y={1095} textAnchor="middle" fontSize={44} fill="#374151" fontWeight="500">Hood</text>

      {/* Front (right end) */}
      <rect x={1880} y={790} width={220} height={580} rx={20}
        fill={zc('exterior_front')} stroke={zb('exterior_front')}
        strokeWidth={7} opacity={0.7} cursor="pointer"
        onClick={() => onZoneTap('exterior_front')}
      />
      <text x={1990} y={1095} textAnchor="middle" fontSize={42} fill="#374151" fontWeight="500">Front</text>

      {/* Driver Side (TOP) */}
      <rect x={380} y={420} width={1495} height={365} rx={25}
        fill={zc('exterior_driver_side')} stroke={zb('exterior_driver_side')}
        strokeWidth={7} opacity={0.7} cursor="pointer"
        onClick={() => onZoneTap('exterior_driver_side')}
      />
      <text x={1130} y={618} textAnchor="middle" fontSize={42} fill="#374151" fontWeight="500">Driver Side</text>

      {/* Passenger Side (BOTTOM) */}
      <rect x={380} y={1375} width={1495} height={365} rx={25}
        fill={zc('exterior_passenger_side')} stroke={zb('exterior_passenger_side')}
        strokeWidth={7} opacity={0.7} cursor="pointer"
        onClick={() => onZoneTap('exterior_passenger_side')}
      />
      <text x={1130} y={1573} textAnchor="middle" fontSize={42} fill="#374151" fontWeight="500">Passenger Side</text>

      {/* Wheels */}
      <g cursor="pointer" onClick={() => onZoneTap('exterior_wheels')}>
        <ellipse cx={1625} cy={535} rx={100} ry={100}
          fill={zc('exterior_wheels')} stroke={zb('exterior_wheels')}
          strokeWidth={7} opacity={0.7}
        />
        <ellipse cx={1621} cy={1622} rx={100} ry={100}
          fill={zc('exterior_wheels')} stroke={zb('exterior_wheels')}
          strokeWidth={7} opacity={0.7}
        />
        <ellipse cx={655} cy={540} rx={100} ry={100}
          fill={zc('exterior_wheels')} stroke={zb('exterior_wheels')}
          strokeWidth={7} opacity={0.7}
        />
        <ellipse cx={652} cy={1617} rx={100} ry={100}
          fill={zc('exterior_wheels')} stroke={zb('exterior_wheels')}
          strokeWidth={7} opacity={0.7}
        />
      </g>
      <text x={1080} y={1775} textAnchor="middle" fontSize={36} fill="#6b7280" fontWeight="500">Wheels (all 4)</text>
      ...
      {EXTERIOR_ZONES.map((z) => {
        ...
        const positions: Record<string, { x: number; y: number }> = {
          exterior_rear: { x: 280, y: 830 },
          exterior_trunk: { x: 660, y: 870 },
          exterior_roof: { x: 1440, y: 895 },
          exterior_hood: { x: 1820, y: 870 },
          exterior_front: { x: 2050, y: 850 },
          exterior_driver_side: { x: 500, y: 475 },
          exterior_passenger_side: { x: 500, y: 1690 },
          exterior_wheels: { x: 1625, y: 650 },
        };
        const pos = positions[z.key];
        if (!pos) return null;
        return (
          <g key={z.key}>
            <circle cx={pos.x} cy={pos.y} r={40} fill="#2563eb" />
            <text x={pos.x} y={pos.y + 14} textAnchor="middle" fontSize={40} fill="white" fontWeight="bold">
              {count}
            </text>
          </g>
        );
