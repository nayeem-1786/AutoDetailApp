...
export function AddonSuggestions() {
  ...
  return (
    <>
      <div className="mx-4 mb-2 rounded-lg border border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-900/30 px-3 py-2">
        {/* Header row */}
        <div className="mb-1.5 flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            <Sparkles className="h-3.5 w-3.5 text-blue-500 dark:text-blue-400" />
            <span className="text-xs font-medium text-blue-700 dark:text-blue-400">Suggested Add-Ons</span>
          </div>
          <button
            onClick={() => setDismissed(true)}
            className="flex h-6 w-6 items-center justify-center rounded text-blue-400 dark:text-blue-300 hover:bg-blue-100 dark:hover:bg-blue-900/40 hover:text-blue-600 dark:hover:text-blue-400"
        ...
        <div className="flex flex-wrap gap-1.5">
          {suggestions.map((addon) => (
            <button
              ...
              className="inline-flex items-center gap-1 rounded-full border border-blue-200 dark:border-blue-800 bg-white dark:bg-gray-900 px-3 py-1.5 text-xs font-medium text-blue-700 dark:text-blue-400 transition-colors hover:border-blue-300 dark:hover:border-blue-700 hover:bg-blue-100 dark:hover:bg-blue-900/40 active:scale-[0.98]"
            >
              <span className="max-w-[140px] truncate">{addon.addonServiceName}</span>
              {addon.comboPrice != null && (
                <span className="whitespace-nowrap text-blue-500 dark:text-blue-400">
