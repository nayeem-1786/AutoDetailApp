...
**Out of scope:** POS receipt print/preview — `generateReceiptHtml()` stays for thermal printer and POS dialog. Only the EMAIL version of receipts gets templated.
...
LAYER 1: Brand Kit (global settings — colors, logo, fonts, social links)
    ↓ inherited by
LAYER 2: Layout Templates (3 static HTML skeletons — header/body/footer frames)
    ↓ wraps
...
### What They Are
3 pre-built HTML email skeletons with placeholder slots, stored as **static files in the codebase** (not DB — these rarely change). The user does NOT edit layout structure. They define the structural frame: where the logo goes, how the header looks, where content sits, what the footer contains.
...
| **Minimal** | No header band — logo top-left small, clean white with subtle border, compact footer | Receipts, booking confirmations, internal |
...
### How They Work
Each layout is a static HTML template file in `src/lib/email/layouts/`:
- `standard.html`
- `minimal.html`
- `promotional.html`

Slots marked with double-brace placeholders:
```html
{{LOGO_HTML}}
{{HEADER_CONTENT}}
{{BODY_CONTENT}}
{{FOOTER_CONTENT}}
{{UNSUBSCRIBE_LINK}}
{{PRIMARY_COLOR}} → resolved from Brand Kit
{{ACCENT_COLOR}} → resolved from Brand Kit
...
| layout | TEXT NOT NULL DEFAULT 'standard' | `standard`, `minimal`, `promotional` |
...
- **`campaigns`**: ADD `email_body_blocks JSONB`, `email_layout TEXT DEFAULT 'promotional'`, `email_preview_text TEXT`
...
renderEmail(template, layoutName, brandKit, variables, { isMarketing })
  ...
  → 4. Load layout HTML file (standard.html / minimal.html / promotional.html)
  → 5. Inject Brand Kit values into layout placeholders
  → 6. Inject compiled inner HTML into {{BODY_CONTENT}} slot
  → 7. Inject footer (business info, social links, unsubscribe for marketing)
  → 8. Generate plain text fallback (strip HTML, keep text)
  → { html, text }
...
4. Subject, preview_text, layout editable per campaign
...
Sub-phase 1: Database + Brand Kit + Rendering Engine
  ├── Migration: email_templates, email_template_assignments, drip_sequences,
  │   drip_steps, drip_enrollments, drip_send_log + ALTER lifecycle_rules, campaigns
  ├── Brand Kit: business_settings seed values + admin page
  ├── Layout HTML files: src/lib/email/layouts/{standard,minimal,promotional}.html
  ├── src/lib/email/types.ts — Block data shapes
  ├── src/lib/email/block-renderers.ts — 11 block renderers (table-based, inline styles)
  ├── src/lib/email/layout-renderer.ts — Full pipeline (Brand Kit + layout + blocks → HTML)
  ├── src/lib/email/photo-resolver.ts — Dynamic gallery photo queries
  ├── src/lib/email/template-resolver.ts — Segment routing logic
  ├── src/lib/email/send-templated-email.ts — High-level send function
  └── src/lib/email/variables.ts — Extended variable definitions per category

Sub-phase 2: API Routes — Templates
  ├── CRUD: templates, assignments
  ├── Preview render endpoint
  ├── Test send endpoint
  ├── Reset to default endpoint
  └── Gallery photo browser endpoint (for photo_gallery block manual pick)

Sub-phase 3: Admin UI — Template List + Brand Settings
  ├── Email Templates page with category tabs
  ├── Brand Settings tab (color pickers, logo, fonts, social links)
  └── Sidebar nav entry

Sub-phase 4: Admin UI — Block Editor
  ├── Block palette (left sidebar)
  ├── Block canvas with drag-and-drop (center)
  ├── Block properties panel (right) — including photo gallery picker + coupon editor
  ├── Variable inserter dropdown
  ├── Email preview (iframe, desktop/mobile toggle)
  ├── Test send dialog
  └── Template picker modal (shared by campaigns, drips)

Sub-phase 5: Campaign + Automation Integration
  ├── Campaign page: tabs (One-Time | Drip)
  ├── Campaign wizard: template picker + inline block editor in Message step
  ├── Automation editor: template_id dropdown + preview
  └── Lifecycle engine: add email rendering + sending for standalone rules

Sub-phase 6: Drip Campaign System
  ├── API routes: CRUD for drip sequences + steps + enrollments
  ├── Drip builder page (trigger, steps timeline, stop conditions, nurture)
  ├── Drip list view (under Campaigns "Drip" tab)
  ├── Drip analytics (funnel, drop-off, conversion)
  ├── Lifecycle engine: drip enrollment + execution + stop condition checking
  └── Customer detail page: active sequences section

Sub-phase 7: Sender Migration (backward compatible)
  ├── Order emails (4 types) — template fallback
  ├── Job completion — template fallback
  ├── Quote send — template fallback + switch to sendEmail()
  ├── Appointment notification — template fallback
  └── Stock alerts — template fallback

Sub-phase 8: Seed Data + Compliance
  ├── Seed: 16 system templates with body_blocks matching current hardcoded HTML
  ├── Seed: default email_template_assignments
  ├── Seed: example drip sequences (win-back, welcome series)
...
Sub-phase 9: Docs + Cleanup
  └── Update DB_SCHEMA.md, FILE_TREE.md, CHANGELOG.md
```
...
- `src/lib/email/layouts/standard.html`
- `src/lib/email/layouts/minimal.html`
- `src/lib/email/layouts/promotional.html`
...
**Admin pages:**
- `src/app/admin/marketing/email-templates/page.tsx` (list + brand settings tabs)
- `src/app/admin/marketing/email-templates/[id]/page.tsx` (block editor)
...
- `src/app/admin/marketing/campaigns/page.tsx` — Tabs (One-Time | Drip)
- `src/app/admin/marketing/campaigns/_components/campaign-wizard.tsx` — Template picker + block editor
- `src/app/api/marketing/campaigns/route.ts` — Accept email_body_blocks, email_layout
- `src/app/api/marketing/campaigns/[id]/route.ts` — Accept email_body_blocks, email_layout
- `src/app/api/marketing/campaigns/[id]/send/route.ts` — Render from blocks when present
- `src/app/admin/marketing/automations/[id]/page.tsx` — Template dropdown
- `src/app/admin/marketing/automations/new/page.tsx` — Template dropdown
- `src/app/api/marketing/automations/[id]/route.ts` — Accept email_template_id
- `src/app/api/cron/lifecycle-engine/route.ts` — Email rendering + drip engine phases
...
### Brand Kit + Templates
1. **Brand Kit** → Change primary color, verify all template previews update
2. **Create template** → Verify saves with blocks, layout, preview_text, variables
3. **Preview** → Verify TABLE-based HTML, inline styles, no CSS variables, bulletproof buttons
4. **Test send** → Send to personal email, verify in Gmail + Apple Mail
5. **Photo gallery (manual)** → Browse, pick pairs, verify side-by-side rendering with labels
6. **Photo gallery (dynamic)** → Configure service_match, trigger send, verify correct photos pulled
7. **Segment routing** → Two templates, same key, different segments → verify correct one selected
8. **Reset to default** → Edit system template, reset, verify reverts

### Campaign + Automation Integration
9. **Campaign with template** → Select template, customize blocks, send, verify branded email
10. **Automation with template** → Set template_id, trigger event, verify email sends with latest template content
11. **Backward compat** → Existing campaigns with raw email_body still work
12. **Backward compat** → All 8 senders work without customized templates (hardcoded fallback)

### Drip Campaigns
13. **Create drip** → 3-step win-back with delays, templates, coupon on step 3
14. **Auto-enrollment** → "No visit in 30 days" trigger, verify eligible customers enrolled
15. **Step execution** → Verify step 1 sends, then step 2 after delay
16. **Stop condition** → Enrolled customer books → verify drip stops with reason "booked"
17. **Per-step exit** → Step with exit_action="tag" → verify tag added on exit
18. **Nurture handoff** → Stopped customer auto-enrolled in nurture sequence
19. **Manual enrollment** → Add customer from admin UI
20. **Customer detail** → Verify active sequences section shows enrollments with actions
21. **Analytics** → Verify funnel + conversion metrics
22. **Pause/resume** → Pause sequence, verify no sends. Resume, verify picks back up

### Compliance
23. **CAN-SPAM** → Marketing emails include physical address + unsubscribe link in footer
24. **Unsubscribe** → Click unsubscribe link → verify email consent revoked via existing Mailgun webhook flow
