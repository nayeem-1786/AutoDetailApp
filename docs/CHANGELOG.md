...
- **Customer type update**: Both customer lookup paths (phone match + email match) now select `customer_type` and set it to `'enthusiast'` when null. Existing `'enthusiast'` or `'professional'` values are never overridden. Fixes customers created by voice agent or import with null type staying unclassified.

---

## fix: Record booking deposits as transactions + webhook logging — 2026-04-12

- **Booking route** (`src/app/api/book/route.ts`): After appointment creation with a deposit, now inserts a `transactions` row so the deposit appears in Admin > Transactions. Receipt number auto-generated by DB trigger (`SD-XXXXXX`). Notes include service total and balance due.
- **Stripe webhook** (`src/app/api/webhooks/stripe/route.ts`): `payment_intent.succeeded` now handles booking deposits (via `is_deposit` metadata) with logging instead of silently breaking.
- **Appointment detail dialog** (`src/app/admin/appointments/components/appointment-detail-dialog.tsx`): Shows "Deposit Collected: $X (Balance: $Y)" in green when `deposit_amount > 0`.
- **TypeScript types** (`src/lib/supabase/types.ts`): Added `payment_type` and `deposit_amount` to `Appointment` interface (columns already existed in DB).
...
- Added `meta.headerClassName` support to DataTable for per-column flex wrapper styling.
- Action, Points, Balance headers use `justify-center` so text + sort arrow are centered as a unit.
- Cell content centered to match headers.
...
(Superseded by 0x1D byte control fix above)
