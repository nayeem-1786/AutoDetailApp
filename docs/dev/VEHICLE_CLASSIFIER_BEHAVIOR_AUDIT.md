...
|----|----------|--------|-----------|-------|-----------|
| **C1** | **Critical** | Public booking Step 1 classifier silently defaults to `automobile` + `category_confident=false` (failure mode a) OR literally hangs (failure mode b) under anonymous RLS denial | `step-vehicle.tsx:136–137` (browser client) → `vehicle-categories.ts:746–750` (`vehicle_makes` query) → migration `20260223000001_create_vehicle_makes.sql` (anon-not-granted RLS) | Anon JWT lacks SELECT policy on `vehicle_makes` | **Option A (recommended):** New migration adding `CREATE POLICY "vehicle_makes_anon_read" ON vehicle_makes FOR SELECT TO anon USING (is_active = true);`. Single line, restores production. Security risk: low — makes are public knowledge already exposed via `/api/vehicle-makes` to anon. **Option B (architectural):** Move the `vehicle_makes` lookup in `resolveVehicleClassification` to a public API route mirroring `/api/vehicle-makes` (admin client bypasses RLS). Larger fix, more correct long-term but slower to ship. |
| **S1** | **Significant** | `resolveVehicleClassification` destructures `await ... ` as `const { data: makeRows }` and ignores the `error` field. This makes `{ data: [], error: null }` (no rows match) indistinguishable from `{ data: null, error: <RLS denial> }`. Both fall through to dev-warn + silent default. Caller cannot detect "classifier was denied" vs "classifier ran but found no matching make." | `vehicle-categories.ts:746–778` | Missing error introspection | Refactor: read `error` too; if `error` is non-null AND not a permission denial (so genuine 0-row matches still default to automobile cleanly), surface via `categoryConfident=false` + a new optional field `classifier_error: string \| null`. Caller (esp. step-vehicle.tsx) could log this for telemetry. ~10 lines. |
| **M1** | **Moderate** | Non-automobile specialty_tier "auto-detection" is hardcoded `DEFAULT_SPECIALTY_TIERS[category]` (smallest tier) regardless of model. The function comment says this is intentional ("staff corrects in POS"), but the operator's mental model from the audit prompt ("is the sorting system working?") may expect model-aware detection. | `vehicle-categories.ts:706–839` (Layer 3) + `:638–643` defaults | By design | Documentation only — not a code change. Add a paragraph to `CLAUDE.md` Rule 19 clarifying the asymmetry: automobile size_class is auto-detected (Layers 2/4/5), non-automobile specialty_tier is operator-picked with smallest-tier seed. |
| **Mi1** | **Minor** | Dead no-op useEffect at `step-vehicle.tsx:225–232` — both `if` blocks are empty (comments only). Has been dead since the useEffect was written (no commit ever populated the bodies). | `step-vehicle.tsx:225–232` | Code smell | Delete the useEffect. Future cleanup. |
| **Mi2** | **Minor** | The two `vehicle-makes` data paths (combobox via `/api/vehicle-makes`, classifier via direct browser client) are inconsistent. Combobox uses server admin client (works for anon); classifier uses browser client (fails for anon). Same logical operation, two different access patterns. | `src/app/api/vehicle-makes/route.ts` vs `vehicle-categories.ts:746` | Drift over time | Fixing C1 via Option B (architectural) eliminates this drift. Option A (RLS hotfix) leaves it but unblocks production. |
