...
| Session 4 (revised 2026-05-24): D36 + D37 + Issue 33 mitigation + Issue 34 capture | ⚪ ready | Scope revised from D35+D36 bundle to a tighter 4-item bundle that does NOT pivot the tool. Test 4 (2026-05-24, against full deploy `acef3613`) demonstrated upsert_customer fires reliably (5 dispatch calls in single conversation) — D35's pivot recommendation was based on tests against a build that did not contain upsert_customer; D37 supersedes D35. Bundle four changes: (a) **send_quote_sms 60-second idempotency guard (D36, unchanged)** — match on customer_id + vehicle_id + service-set within last 60 sec → return existing quote_id with `was_duplicate:true` + `instructions_for_agent` silent-acknowledge; (b) **invocation discipline prompt rule (D37)** — call upsert_customer ONLY when new field data exists; first call on first_name learn, subsequent calls only on last_name/email/address/customer_type signal change; no redundant idempotent writes; (c) **combo-pricing mitigation prompt rule (Issue 33)** — agent should NOT state bundle/combo pricing without first calling get_services to verify combo will apply; prefer standalone prices over promising discounts that may not materialize; endpoint-level combo-pricing fix DEFERRED to a separate session; (d) **last_name capture prompt rule (Issue 34)** — when customer agrees to receive quote AND existing customer has no last_name, ask casually for full name; parse into first_name + last_name if provided; call upsert_customer with last_name before send_quote_sms; do not block quote on declining the ask. Tests for all four changes. Estimated 2-2.5 hours CC. |
