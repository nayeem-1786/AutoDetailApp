...
**Parallel prompt-fidelity track (Workstream J + production-driven cascades) — status as of 2026-05-25:** While Layers 5–6 of the 6-layer rollout remain not-started, the live v2 agent has been under continuous production-driven prompt + pricing-fidelity refinement since the Layer-4 deploy. Issues 26–38 are captured in `SMS_AI_V2_PROMPT_OBSERVATIONS.md` (Section 2); decisions D33–D42 are locked (Section 7). Most are resolved: Issue 26 (server-side phone injection, #57), Issues 30–32 + 34 (idempotency guard + upsert_customer cadence, #59/#62), Issue 33 (combo-resolver, #65/#66), Issue 35 (mandatory customer reply D38 + runner backstop, #67/#69), Issue 36 (size_class endpoint fix — closed by D41 #77 after D39 #74 + D40 #75 + the #76 diagnostic), Issue 37 (resolver prefix-match D42, #78). **Open:** Issue 38 (tier-intent gap within multi-tier services) — audited 2026-05-25 (#79, `3a9b06fe`), B1 fix recommended, implementation deferred to Sessions A/B/C. See ledger rows #73–79 and the Workstream J section below.
...
As of 2026-05-25 the SMS-AI v2 Issue 36→38 fidelity arc has just closed out: Issues 26–37 are resolved and deployed; the only open customer-facing fidelity gap is **Issue 38** (tier-intent within multi-tier services), audited 2026-05-25 with **B1 recommended and ready to implement** (`docs/dev/ISSUE_38_TIER_INTENT_AUDIT.md`). Remaining work, by track:

**Track 0 — Close Issue 38 (immediate):** Implement B1 (Sessions A/B/C) — optional `tiers` + `quantities` CSV params on `send_quote_sms`, a parallel Critical Rule in the prompt, and `ResolvePriceOptions.tierName` honored in the scope/specialty/vehicle_size branches. ~1 focused session, backward-compatible, +12–18 tests. This is the last known way the agent can quote one price and bill another, so it should run before resuming feature work. **(This catch-up branch + Sessions A/B/C all touch ROADMAP + CHANGELOG — see the merge-ordering note in the session prompt to avoid doc conflicts.)**
...
Recommended sequencing: Track 0 (Issue 38) → Track 2 reliability/security foundation → resume Track 1 feature roadmap.
