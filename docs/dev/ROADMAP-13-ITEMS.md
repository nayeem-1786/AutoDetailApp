...
- Session 5 (context refresh) and Session 4 (refined-flow prompt rewrite)
  are independent — refresh is data plumbing, prompt is rule rewrites
- Session 4 (prompt) goes after Session 5 so D20 quote_status is available
  for the refined-flow rules to reference
- Session 6 (verification) is last
