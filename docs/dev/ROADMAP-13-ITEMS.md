...
|---|---|---|---|---|---|
| 2026-06-02 | 144 | Fix: Step 1 classifier rule THIRD refinement — restore mundane automobile auto-select with make/model-scoped override survival | Production code change, merged to main | _(this commit)_ (branch `fix/step1-classifier-rule-third-refinement`) | Refines #143's classifier-output rule based on operator feedback that #143's "manual-only for mundane" position rolled back previously-working pre-#143 behavior (Civic→Sedan, Suburban→SUV, F-150→Truck/SUV pre-selected). Operator's original stated rule was about **non-automobile** auto-select; #143 over-strengthened to all 5 categories. **Rule-evolution history:** pre-#143 auto-selected all 5 (latent, surfaced when #142 fixed RLS); #143 first pass "NEVER" (over-strengthened); #143 final pass "exotic/classic only" (Q-A.4 Option (iii)); **#144 (this):** exotic/classic AND mundane automobile auto-pre-select; customer can correct; correction survives until make OR model changes; non-automobile specialty_tier still manual-pick (unchanged). **Implementation (`step-vehicle.tsx`):** `effectiveSizeClass` non-specialty branch becomes `manualSizeClass ?? (category === 'automobile' ? classification?.size_class ?? null : null)`. NEW useEffect on `[make, model]` clears `manualSizeClass` + `manualSpecialtyTier` when either changes, with `isInitialMakeModelMountRef` guard so edit-from-Step-4 round-trips don't nuke prior pick. Independent of #143's exotic/classic-clear useEffect. **No separate `manualOverride: boolean`** — `manualSizeClass !== null` IS the override signal. **Tests (+2 net → 2866):** flipped 2 (Civic → Sedan IS highlighted; Suburban → SUV IS highlighted); removed 1 (manual-survives-across-make-change contradicts #144 rule); added 3 (override survives year-typing same make+model; clears on model change to Accord; clears on make change to Toyota). Unchanged tests: exotic/classic × 3 (Ferrari/Mustang 1965/Sedan→Ferrari wipe); non-automobile × 2; year propagation × 2. **Scope:** 1 prod MOD + 1 test MOD. ~30 prod lines net. **Gates:** tsc 0, lint 0err/97warn, 2866/2866, build clean. **CLAUDE.md Rule 22** updated. **STEP1_SIZE_CLASS_AND_MUSTANG_CLASSIC_AUDIT.md** annotated with #144 re-refinement note. Not a 13-item entry (third refinement of post-audit fix arc). |
