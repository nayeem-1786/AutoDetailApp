...
|---|---|---|---|---|---|
| 2026-05-26 | 84 | Issue 41 audit — visual surface tier_name rendering inventory across all consumers (read-only) | Audit done | _(this commit)_ (branch `audit/issue-41-tier-visual-surfaces`) | Companion to Issue 39's chip-composition audit (audit session #83, `audit/issue-39-services-chip-composition`, commit `fd92dbb2`). Read-only diagnostic mapping every surface that renders `quote_items.tier_name` (or parallel `appointment_services.tier_name` / `transaction_items.tier_name`) across the codebase. **Deliverable** `docs/dev/ISSUE_41_TIER_VISUAL_SURFACES_AUDIT.md` (~700 lines, 10 targets + TL;DR + risk matrix + empirical reproduction). **Headline finding:** **15 visual surfaces render `tier_name` as a raw snake_case slug to humans today.** Issue 39's audit Target 9 named 5; this audit identifies 10 NEW: (a) `src/app/pos/lib/receipt-template.ts:599-600` (thermal via `generateReceiptLines`) + `:1063-1064` (HTML via `generateReceiptHtml`) — 1 file × 2 sites serves 3 of 4 receipt consumers (`print-copier` + `email` + `html` + `customer/receipts/html` + `print-server`); (b) `src/app/api/appointments/[id]/notify/route.ts:96/137/160` (template HTML + plain-text fallback + HTML fallback) + `src/app/api/pos/appointments/[id]/notify/route.ts:102/146` (POS variant × 2 contexts); (c) `src/app/(public)/pay/[token]/page.tsx:290-291` (customer-facing pay-page checkout-stage surface — captured as Issue 42-class but absorbed into Issue 41's helper rollout, no separate ticket); (d) `src/app/admin/quotes/[id]/page.tsx:405-406` (admin quote DETAIL page distinct from the slide-over); (e) `src/app/pos/components/transactions/transaction-detail.tsx:276-277` (POS transaction detail). **Recommended architecture: Option U (two-layer split)** — `src/lib/quotes/tier-display.ts` (new) exports `renderTierToken(item) → string | null` (low-level: qty>1 returns `${qty} ${pluralize(qty_label)}`, qty=1 returns `tier_label` with title-cased `tier_name` fallback, sentinel `'default'` returns null); Issue 39's `services-summary.ts` (the chip helper) consumes it for parenthetical content; the 15 visual surfaces consume it directly and wrap the token in surface-appropriate presentation (em-dash, parens, sub-line, monospace ` - `, PDF column). Mirrors Session 71's `line-item-pricing.ts` precedent (shared formatter, per-surface rendering autonomous). Option V (single helper with surface enum) rejected — inverts dependency direction by coupling helper to presentation layers; Option W (parallel no-dependency helpers) rejected — duplicates Issue 39's per-tier rendering rule across two helpers (drift trap). **Combined Issue 39 + Issue 41 scope: SPLIT into 2 sequenced sessions** per memory #8 (~300 LOC or >3 files = split; combined estimate crosses both at ~1000 LOC / ~18 files). **Session 1 (D45):** create `tier-display.ts` + `services-summary.ts` + adopt at 6 chip-composing routes (Issue 39 scope) + widen 6 SELECTs to join `service_pricing(tier_label, qty_label)` + Q-0084 chip verification. ~90-120 min, ~34 tests, ~500-650 LOC. **Session 2 (D46):** adopt `renderTierToken` at 15 visual surfaces (Issue 41 scope) + Q-0084 per-surface visual verification. ~90-120 min, ~25-30 tests, ~250-350 LOC. Fires AFTER Session 1 merges. **Sequenced (not parallel)** because Session 2 depends on Session 1's helper API and the parallel-session worktree-fragility memory documents that coordination cost of true parallel runs exceeds the wall-clock savings (lesson learned during today's D43 A/B/C arc). **Memory #15 (4 receipt surfaces) honored:** 3 of 4 receipt surfaces share `receipt-template.ts` (2 edit sites cover all 3 tier-rendering consumers); 4th surface (SMS receipt via `buildSummaryLine`) explicitly inspected — renders NO `tier_name` (only vehicle + total) and needs no change. **Pre-flight verification for Session 1:** D43 fully merged ✓ (commit `8f01f3a7`); Issue 39 audit present ✓; no competing parallel session on `src/lib/quotes/` (verify via `git worktree list` + `git stash list` immediately before fire). **Issue 40 disposition (per Issue 39 + Issue 41 alignment):** operator makes 2 admin edits to `service_pricing.tier_label` (`"Floor Mats Only"` → `"Floor Mats"`, `"Carpet & Mats Package"` → `"Carpet & Mats"`) AFTER Session 1 ships and BEFORE Session 2 fires so verification screenshots show clean labels at each stage. **Operator decisions pending (2, both small, neither blocks implementation):** (1) Issue 40 timing — before or after Session 1? Default AFTER. (2) `vehicle_size`/`specialty` single-tier qty=1 — surface tier in helper? Default NO (matches Issue 39 operator decision 6). **Hard rules respected:** no `src/` changes, no migrations, no tests; only new files = the audit deliverable + this ROADMAP row + CHANGELOG entry + Issue 41 capture in `SMS_AI_V2_PROMPT_OBSERVATIONS.md`. Worktree-isolated commit (per `feedback-parallel-doc-sessions-use-worktree` memory). **Verification (audit only):** `git diff --name-only` shows only the 4 doc files; no `src/` modifications. |
