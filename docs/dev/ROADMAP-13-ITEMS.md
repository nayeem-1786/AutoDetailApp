...
**Document version:** v4.0 (2026-05-27) — **v4.0 adds (2026-05-27):** SMS-AI v2 prompt-fidelity arc fully closed across Issues 39-46 (8 issues across ~17 sessions #85-96 between 2026-05-26 and 2026-05-27). Closures: Issue 39 (D45 chip composition, `0ad3e89d`), Issue 40 (operator admin tier_label edits), Issue 41 (D46 15 visual surface adoption, `1a6aea73`), Issue 42 (D48 `appointment_services.quantity` schema + conversion, `d4e2fd97`), Issue 43 (D47 Critical Rule 8 price-lookup discipline), Issue 44 (D47 Critical Rule 9 scope-tier enumeration), Issue 45 (D49+D50 Option A proactive auto-send + universal friction-question prohibition), Issue 46 (UI label channel-aware refinement, two iterations). New Critical Rules count: 19 → 22 (post-D47 + post-D49). New captures: Issue 47 (soft-friction "Want to go with that?" pattern surfaced during D50 verification, deferred), misleading docstring at `system-prompt.ts:12` (claims admin override capability not actually implemented in v2 path). New observability: `[SmsAiV2.send_quote_sms.auto_send_trigger]` log line in `tool-dispatcher.ts` for false-fire monitoring. Operator's universal-scoping principle captured as standing rule in `SMS_AI_V2_PROMPT_OBSERVATIONS.md` Section 1. **v3.9 (2026-05-25):** session ledger backfilled with 7 rows (#73–79) covering the SMS-AI v2 Issue 36→38 cascade that landed 2026-05-24 evening → 2026-05-25 — quote source tracking (`ea42962b`, surfaced from git log); D39 (`20a94b0a`) + D40 (`76d9b58e`), the two insufficient Issue-36 `size_class` attempts; the Issue 36 Phase B diagnostic (`f682dc2e`) that broke the 3-attempt failed-fix run by locating the endpoint root cause; D41 (`a59e41b4`), THE Issue 36 fix; D42 (`270bde7e`), the prefix-match resolver fallback that closes Issue 37; and the Issue 38 tier-intent audit (`3a9b06fe`, recommends B1, fix deferred). 13-item table unaffected (all out-of-scope SMS-AI v2 / Workstream J work). **v3.8 (2026-05-19, evening)** — Items 1, 6, 12, 15a, 15b, 15c completed; 15d deferred; 15e scoped; **Item 15f COMPLETE (all sub-layers + Phase 1 Layers 8a + 8b + 8c + 8d + 8d-bis + 8e + 8f done — Phase 1 closed on `10f7cffb`)**; **Item 15g COMPLETE (all 5 layers done)**. **v3.8 adds**: 2026-05-19 JSONB fix cluster — `ai.txt` P0 fix (`1b96405f`, merge `feef903d`) and coupon-enforcement P1 fix (`a55335de`, merge `17ebbd48`) both shipped today + cross-consumer drift resolved via new shared helper `src/lib/utils/coupon-enforcement.ts`. Companion audit `docs/dev/AUDIT_VOICE_POLL_AND_COUPON_ENFORCEMENT_2026-05-19.md` on branch `audit/voice-poll-and-coupon-enforcement` (`83bfae64`). New follow-ups surfaced today: migration LIKE-pattern recurring failure (manual force-fix needed twice — homepage-settings + coupon-enforcement), SemVer / version tracking not implemented, voice-calls-poll cron P2 fix scoped, QBO P1 batch deferred pending QuickBooks reconnection plan. **v3.7 adds (carried forward)**: Roadmap Snapshot summary (13-item status table + roll-up) and Out-of-Scope Workstreams section covering SMS AI v2, Next.js security upgrade, original triage, infra hardening, codebase JSONB audit, and process improvements shipped 2026-05-18 → 2026-05-19.
**Last session updated:** 2026-06-03 — **Session #150: FIX — "Edit in POS" button wires deep-link with parameterized `returnToPath` + excludes `no_show` from render gate + semantic regression tests** (branch `fix/edit-in-pos-button-wire-deep-link-from-schedule`, isolated `git worktree`; **merged to main this session**). Closes the Day-1 UX gap from the Edit-in-POS audit (`d1eb1e24`, `docs/dev/EDIT_IN_POS_BUTTON_AUDIT.md`). Phase 2B (`ad4f6269`, Item 15e) explicitly passed `onEditInPos={() => { /* no-op */ }}` from the POS Schedule host to suppress the admin deep-link "because we're already in POS" — the button was visible-but-inert on POS Schedule from day one. **All 3 audit decisions LOCKED + applied:** (Q1) replaces `onEditInPos` no-op with `returnToPath?: string` (default `/admin/appointments`; POS Schedule passes `/pos/jobs`) → same drain hook + Sale-tab edit flow runs from both contexts, just returning to different surfaces; (Q2) `canEditServices` render gate switched to the new shared predicate `isServiceEditableStatus` in `status-transitions.ts` → `no_show` now excluded from button render in lockstep with `service-edit.ts`'s server cascade refusal; (Q3) trivial `typeof onEditInPos === 'function'` POS-test replaced with `returnToPath === '/pos/jobs'` + `onEditInPos === undefined` semantic assertions, paired with new dialog-level tests asserting `returnToPath="/pos/jobs"` routes Save to Schedule, custom `returnToPath` does NOT contain `/admin/appointments` (regression-lock the no-op anti-pattern closed), button RENDERS for `pending`/`confirmed`/`in_progress`, NOT-RENDERS for `completed`/`cancelled`/`no_show`. **4 prod files + 2 test files (+26 net prod lines, +5 net test cases):** `status-transitions.ts` (+23 — new predicate + constant), `service-edit.ts` (-5 — collapses 6-line inline check to 1-line predicate call), `appointment-detail-dialog.tsx` (+2 — prop rename + onClick rewrite), `job-queue.tsx` (-2 — drop no-op, add returnToPath); test files extend the existing dialog test suite + strengthen the POS Schedule scope test. No reducer/migration/schema/permission change. Deep-link mechanism verified before fix (`use-edit-mode-drain.ts:14-67`) — `returnTo` is a URL query param validated by `isSafeInternalPath`; `/pos/jobs` passes. Gates: tsc 0, lint 0err/97warn (baseline unchanged), 2967/2967 (+5), build clean. Memory #8 ≤4 prod files / ≤50 prod lines: both respected. Memory #29 Targeted respected — no comprehensive dialog parity sweep (deferred to a separate session). Audit doc updated: RESOLVED with merge commit hash. Not a 13-item entry (POS Schedule UX fix; Item 15e ✅ closed since 2026-05-27 — this is post-15e operator UAT follow-up). **Earlier 2026-06-03** — **Session #145: CHORE — ROADMAP-13-ITEMS audit + #118 ledger backfill + Roll-up/Status-Table drift reconciliation** (branch `chore/roadmap-13-items-audit-and-118-backfill`, isolated `git worktree`; **merged to main this session**). Doc-only. Backfills the missing #118 ledger row (`audit/sale-vs-quotes-shared-component-parity-sweep`, commits `31199db7` / merge `c79dcf91`) that landed on main 2026-05-28 without a ROADMAP entry; corrects the "renumbered #118 → #119" misread inside #119's row (the parity-sweep DID land as #118, and the panel-parity fix landed as #119 — both shipped that day). Reconciles drift between the Status Table (15e ✅ CLOSED 2026-05-27) and the Roll-up (was "In progress (1): 15e"); also reconciles the "Total items" line (was "9 done" but enumerated only 8 — 15e was missing from the list). Adds a new "Current Status — Evidence-Based (as of 2026-06-03, post-Session #145)" section with per-item evidence table + recent-activity summary + Target C operator-decision surface (5 more missing audit-ledger rows for #112/#125/#126/#127/#128/#135; Memory #12 booking-multi-service framing question; older session-number gaps left as-is). Per-item table verdicts: 9 DONE (1, 6, 12, 15a–c, 15e, 15f, 15g) + 1 CLOSED (5) + 1 ABSORBED (15d) + 10 NOT STARTED (2, 3, 4, 7, 8, 9, 10, 11, 13, 14) + 0 IN PROGRESS + 0 BLOCKED — matches the corrected Roll-up. Every "DONE" claim cites a session number + commit hash per Memory #11. **Big-picture finding:** every session #110 → #144 (35 sessions across 2026-05-27 → 2026-06-03) ends with "Not a 13-item entry" — the 13-item scope itself has been idle since 2026-05-17 (15f Layer 8f); all recent work has been on out-of-scope follow-up arcs (catalog CRUD, POS Sale-vs-Quotes parity, vehicle taxonomy/classifier/form behavior, public-booking Path B architecture, SMS-AI Issue 39–46 cascade, Q-D pricing_model immutability). No production code, no migrations, no tests. Gates run as sanity check (no-op since no code touched). **Earlier 2026-05-29 — Session #124: FIX — Admin Services Edit add-on dropdown + parent-self exclusion + named UNIQUE-constraint messages (sibling to #123)** (branch `fix/admin-services-edit-addon-dropdown-and-parent-self-exclusion`, isolated `git worktree`; **merged to main this session**). Closes all three open questions from #123's audit (`docs/dev/PREREQ_SERVICE_DROPDOWN_AUDIT.md`) — Target 4 sibling, pre-existing parent-self latent, UNIQUE-constraint specificity — bundled per operator lock. **(1) Add-on edit dropdown un-disabled** (mirrors #123 byte-for-byte): removed `disabled={!!editingAddon}` at `page.tsx:1714`; new `src/app/admin/catalog/services/[id]/addon-helpers.ts` — `getEditAddonOptions(allServices, addons, editingAddonServiceId, parentServiceId)` returns current value PLUS unused non-primary services that are not the parent (mirrors prereq helper plus `classification !== 'primary'` clause); wired `describeSupabaseError` into the `saveAddon` catch. **(2) Parent-self exclusion (defense in depth) on BOTH eligibility filters + BOTH edit helpers:** `addonEligibleServices` + `prereqEligibleServices` now have `s.id !== serviceId &&` (covers ADD mode); `getEditPrereqOptions` + `getEditAddonOptions` now take `parentServiceId` and exclude it (covers EDIT mode). The current-selection clause always takes precedence (by construction the editing row's prereq/addon cannot be the parent — CHECK forbids — so this never accidentally re-introduces it). **(3) Named UNIQUE-constraint messages** added to `CONSTRAINT_MESSAGES` in `src/lib/utils/supabase-error.ts`: `service_prerequisites_..._key` → "That prerequisite is already configured for this service."; `service_addon_suggestions_..._addon_service__key` (note the double-underscore — Postgres 63-char truncation, verified against `DB_SCHEMA.md`) → "That add-on is already configured for this service." Regexes use `.*key` tail to survive future rename / re-truncation. **Tests +10 → 2635 (within expected +6-10):** new `addon-helpers.test.ts` (7 cases — current/unused-non-primary/primary-excluded/other-used-excluded/parent-excluded + 2 defensive); `prereq-helpers.test.ts` extended +1 parent-self case (existing 5 updated for new 4th arg); `supabase-error.test.ts` +2 cases for the new constraint messages. Scope: 3 prod files modified + 1 new prod file + 1 new test + 2 extended tests; ~90 prod lines net. No reducer/migration/schema/permission change. **Closes the prereq/add-on edit UX cluster completely.** Gates: tsc 0, lint 0err/94warn (none new in touched), 2635/2635, build clean. Not a 13-item entry (admin catalog UX polish, sibling closeout to #123). **Earlier 2026-05-29** — **Session #123: AUDIT + FIX — Admin Services Edit prereq-service dropdown un-disabled + error-surfaced** (branch `audit/admin-services-edit-prereq-service-dropdown`, isolated `git worktree`; **merged to main this session**). Operator-reported: on Admin → Services → Edit → Prerequisites, the "which service is the prerequisite" dropdown is greyed out in edit mode. **Audit conclusion — classification (a) half-built convention.** `git blame` on `src/app/admin/catalog/services/[id]/page.tsx:1807` traces `disabled={!!editingPrereq}` (paired with the allServices fallback at `:1810`) to the **initial bulk Phase 1 admin commit** `846ece126` (2026-02-01) — no comment, no design discussion. Schema: `service_prerequisites` PK on `id`, UNIQUE `(service_id, prerequisite_service_id)`, CHECK `service_id <> prerequisite_service_id`; **zero FKs reference `service_prerequisites.id`**; runtime queries only by parent `service_id`. The save handler (`savePrereq()`, `:903-941`) already includes `prerequisite_service_id` in the UPDATE payload — the DB would accept the change today if the UI were enabled. Not (b) schema-forced, not (c) data-integrity, not (d) permissions. **Fix applied in-session (3 files, ~25 prod lines):** removed `disabled={!!editingPrereq}`; new `src/app/admin/catalog/services/[id]/prereq-helpers.ts` — pure helper `getEditPrereqOptions(allServices, prerequisites, editingPrereqServiceId)` returns current value PLUS every unused service (preserves the selection while still excluding already-used prereqs that would collide with UNIQUE at save); wired `describeSupabaseError` (C1/S3 / #111) into `savePrereq()` catch so UNIQUE-23505 and CHECK-23514 violations surface as actionable text instead of a fixed generic toast. **Test (+5 → 2625):** unit-tested the pure helper's invariants (current value always present; unused included; other already-used excluded; degenerate; defensive duplicate). Mirrors #119 precedent — no admin-page render harness for a ~2150-line component. **Targets 3 & 4 surfaced (intentionally NOT fixed):** (a) **sibling gap** — add-on edit dropdown at `page.tsx:1708` carries the identical `disabled={!!editingAddon}` pattern from the same bulk commit (recommend sibling follow-up session); (b) **pre-existing latent** — eligibility filters do not exclude the parent service itself, so an operator can pick "this service" as its own prereq/add-on → CHECK 23514 (now surfaces via `describeSupabaseError` on prereq path; add-on path still on the generic toast). Add+delete confirmed working today (`page.tsx:1649/1681/1684`); the disable was friction, not a functional blocker. Deliverable `docs/dev/PREREQ_SERVICE_DROPDOWN_AUDIT.md` with 3 open operator Qs. Gates: tsc 0, lint 0err/94warn (none new), 2625/2625, build clean. Not a 13-item entry (admin catalog UX polish). **Earlier 2026-05-29** — **Session #122: FIX — Track A gate-order correction (prerequisite dialog primary, manager-PIN only on override)** (branch `fix/track-a-prereq-primary-gate-order`, isolated `git worktree`; **merged to main this session**). Corrects a UX gate-order bug in the #121 `useValidatedServiceAdd` helper: tapping an `addon_only` service that ALSO has prerequisites fired the **manager-PIN (add-on gate) FIRST**, then the prerequisite dialog — backwards. **LOCKED contract now enforced:** (1) prerequisite check is PRIMARY — unmet → `PrerequisiteWarningDialog` (add a prereq, or manager-override behind its OWN Override button); satisfied → commit; the add-on gate never fires when prereqs exist. (2) add-on-only gate is CONDITIONAL — fires ONLY when the service has **no prerequisites configured** (pure add-on, no parent dependency) AND is `addon_only` AND solo (no `primary`/`both` anchor). Rationale: when prereqs exist they ARE the gate; a satisfied/overridden prereq implicitly authorizes the add-on. **2 prod files (~40 lines):** `use-prerequisite-check.ts` now returns `hasPrerequisites` (the endpoint already reported it — was folded into `canAdd`) so the helper knows whether the add-on gate applies; `use-validated-service-add.tsx` runs the prereq check first and gates the add-on warning on `!hasPrerequisites && isAddOnSolo`, with `handleAddOnSoloOverride` committing directly (no re-check). No reducer/migration/schema/permission change; `PrerequisiteWarningDialog`/`ManagerPinDialog`/endpoint/Track B untouched; the 3 consumer surfaces unchanged (fix internal to the helper). **Tests (+5 → 2620):** the 4 add-on-solo "no network" assertions locked the wrong behavior and were rewritten (prereq check runs first now); new per-surface "addon_only WITH unmet prereqs → prereq dialog (not add-on PIN)" + engine "addon_only WITH satisfied prereqs → commits, no PIN". Gates: tsc 0, lint 0err/97warn, build clean. Not a 13-item entry (POS prereq/gating arc). **Earlier 2026-05-28** — **Session #121: FIX — Track A `useValidatedServiceAdd` shared helper (prereq + add-on gating across Sale/Quotes/register-tab)** (branch `fix/track-a-useValidatedServiceAdd-shared-helper`, isolated `git worktree` parallel to Track B which merged #120 first → this session is #121, rebased onto it cleanly; **merged to main this session**). Closes the LAST Sale-vs-Quotes parity gap **G5** (`SALE_VS_QUOTES_PARITY_SWEEP.md`) plus the never-built **add-on-only gating** and the **register-tab no-add-time-validation** gap (`POS_PREREQ_ENFORCEMENT_AND_GATING_AUDIT.md` Issues 2+3, `POS_SALE_VS_QUOTES_PARITY_AUDIT.md` Target 5 — all now RESOLVED). One surface-agnostic hook `src/app/pos/hooks/use-validated-service-add.tsx` runs **add-on-only gate → prerequisite check → commit** and owns both warning dialogs + the prerequisite auto-add. Add-on-only solo = **warn + manager-PIN override** (reuses `pos.override_prerequisites`, mirrors the prereq override). Routed through all three surfaces: Sale `catalog-browser` (byte-behavior-identical reference — `addServiceChecked`→hook's `addService`), Quotes `quote-builder` (search/picker + browse via new `customerIdOverride`/`vehicleIdOverride`/`serviceIdsOverride` props on `<CatalogBrowser>` — the G5 fix), register-tab favorites + picker (gained validation they never had); `service-detail-dialog` now runs the check in callback mode too. **No reducer/migration/schema change.** +17 tests (engine + per-surface prereq-fire + add-on-solo + override + Sale parity guard + G5 context-binding proof). Gates: tsc 0, lint 0err/97warn (none new), 2615/2615 combined with Track B, build clean. **Earlier 2026-05-28** — **Session #120: FIX — Track B Quotes-panel parity wiring (G2/G3/G4) + structural guard test** (branch `fix/track-b-quotes-panel-parity-wiring`, isolated `git worktree`; **merged to main this session**). Closes three Sale-vs-Quotes gaps from the #118 sweep (`SALE_VS_QUOTES_PARITY_SWEEP.md`) by mirroring Sale exactly — Quotes was a partial copy of Sale and never wired these. **G2 (vehicle edit unreachable → reachable):** `editingVehicle` state + `onEditVehicle` on `<CustomerVehicleSummary>` (the pencil only renders when the prop is passed, `customer-vehicle-summary.tsx:116`) + `editVehicle` on `<VehicleCreateDialog>` with an edit-branch `onCreated` dispatching `SET_VEHICLE` (`blockedByPayment:false` — quotes have no checkout). **G3 (reprice-failure silent → surfaced):** panel `toast.warning` watcher (mirror `ticket-panel.tsx:123-143`) + amber "No <size> pricing" badge in `quote-item-row.tsx` (mirror `ticket-item-row.tsx:138-151`); `quote-reducer` already set `repriceFailed`. **G4 (`CustomerTypePrompt` never shown → mounted):** opened from `handleSelectCustomer`/`handleCustomerCreated` when `customer_type` is null, sequenced into the vehicle selector on dismiss (faithful quote-flow adaptation — Sale has no vehicle step), reuses the #119 `handleCustomerTypeChanged`. **No reducer/migration change** (sweep Target 5: `SET_CUSTOMER`/`SET_VEHICLE` identical both sides). **STRUCTURAL GUARD (the durable win):** `src/app/pos/__tests__/sale-vs-quotes-shared-prop-parity.test.tsx` — a source-contract test asserting every prop Sale wires on each of the 7 shared components is also wired in Quotes (minus the documented Sale-only allowlist `disabled`), plus `CustomerTypePrompt` mounted in both (G4) and `repriceFailed` surfaced in both (G3); **verified it has teeth** (removing `onEditVehicle` failed it naming the prop). + `quote-item-row-reprice-badge.test.tsx` (G3 badge). No QuoteTicketPanel render harness (#119 established it's disproportionate). **G6 (swipe+undo) DEFERRED** — Minor; needs a `RESTORE_ITEM` quote action + list restructure (out of Track B scope). Scope: 2 prod files (`quote-ticket-panel.tsx`, `quote-item-row.tsx`) + 2 test files; **no Sale changes, no Track A files** (`catalog-browser`/`quote-builder`/`register-tab`). Gates: tsc 0, lint 0err/94warn (none new), **2598/2598 (+14)**, build clean. Not a 13-item entry (Sale-vs-Quotes parity arc). **Earlier 2026-05-28** — **Session #119: FIX — wire `onCustomerTypeChanged` in the Quotes pill mount (silent customer demotion)** (branch `fix/quotes-customer-type-pill-callback-wiring`, isolated `git worktree` parallel to the read-only parity-sweep audit which took #118 on main first; **merged to main this session**, renumbered #118 → #119). Resolves the Critical finding from #117 (`POS_CUSTOMER_TYPE_PILL_PARITY_AUDIT.md`, now RESOLVED). The shared `CustomerTypeBadge` PATCHes the permanent customer record on tap then calls `onTypeChanged` so the host syncs local state; Sale wired it (`ticket-panel.tsx:413`), Quotes never did → every Quotes tap silently re-tagged the customer globally while the pill showed stale state. **Fix (~9 lines, 1 prod file, mirror Sale):** `handleCustomerTypeChanged` in `quote-ticket-panel.tsx` dispatches `SET_CUSTOMER` with the new `customer_type` + wired the prop + `CustomerType` import; no reducer change (`quote-reducer.ts:361` already handles it). Sale + badge + API untouched; product decision LOCKED (global+permanent, no confirm). **Tests (+4 → 2584):** new `customer-type-badge.test.tsx` locks tap-persists+signals, cycle order, the stale-state regression (the bug), and display-follows-prop; panel wiring guaranteed by tsc + manual verification (no QuoteTicketPanel harness — disproportionate to build). Gates: tsc 0, lint clean, 2584/2584, build clean. **Earlier 2026-05-28** — **Session #117: read-only audit — POS customer-type pill Sale-vs-Quotes parity + persistence** (branch `audit/pos-customer-type-pill-sale-vs-quotes-parity`, isolated `git worktree`; **merged to main this session**). Fourth Sale-vs-Quotes parity gap today. The pill (Unknown/Enthusiast/Professional) cycles in Sale but shows "Customer type cleared" in Quotes. **Root cause = missing prop (classification (c) half-built wiring), NOT wrong-context:** shared `CustomerTypeBadge` (mounted via `CustomerVehicleSummary` in both) PATCHes the customer record then calls `onTypeChanged` so the host syncs LOCAL state. Sale wires `onCustomerTypeChanged` (`ticket-panel.tsx:413`→`:357-360` dispatch `SET_CUSTOMER`); **Quotes omits it** (`quote-ticket-panel.tsx:830-840`) → `onTypeChanged` undefined → local `customer.customer_type` never updates → badge renders stale value → every tap repeats one transition (operator's customer was `professional`→null→"cleared"). **Persistence: YES both surfaces** — badge PATCHes `/api/pos/customers/[id]/type` → direct `customers.customer_type` update + audit row (`route.ts:38-69`); live DB 732 enthusiast/285 professional/370 null. Pill is a GLOBAL permanent record change, not quote-scoped → data-integrity hazard in Quotes (stale UI silently demotes). **Fix:** mirror Sale (~3 lines in `quote-ticket-panel.tsx`; reducer already supports `SET_CUSTOMER` at `quote-reducer.ts:361`). **Bundle** into the same Quotes-parity fix session as an independent commit (NOT part of the prereq/gating shared helper). Deliverable `docs/dev/POS_CUSTOMER_TYPE_PILL_PARITY_AUDIT.md`; 3 open operator Qs. No source/migration/test changes. Not a 13-item entry. **Earlier 2026-05-28** — **Session #116: read-only audit — POS Sale-vs-Quotes prereq + add-on gating parity** (branch `audit/pos-sale-vs-quotes-prereq-gating-parity`, isolated `git worktree`; **merged to main this session** per current workflow). Pins the Sale-vs-Quotes mechanism difference. **Corrects the prior audit's overgeneralization:** Quotes are NOT missing the prereq check — `quote-builder.tsx` calls `checkPrerequisites` in all add paths since 2026-03-14 (`4afe5ed94`). Real defect is **context, not absence**: the quote BROWSE view delegates to `<CatalogBrowser>` whose internal check is hardwired to the SALE-ticket context (`catalog-browser.tsx:76-80`, `useTicket()`), not the quote's; quote search/picker use correct quote context (`quote-builder.tsx:221-225`). Wrong context can over-fire (empty sale ticket) and silently under-fire ("doesn't fire") when a Sale ticket already holds the prereq. Surface matrix: Sale/catalog-browser ✓; Sale/register-tab favorites ✗ no-check (re-verified); Quotes search/picker ✓; Quotes browse ⚠ wrong-context. Hook is surface-agnostic (`use-prerequisite-check.ts:28-32`); only CatalogBrowser lacks a prereq-context override. Add-on gating still absent everywhere. **Recommendation:** one shared add-with-validation helper (addon-gate → prereq-check → dispatch) routed through Sale + Quotes (both views) + register-tab — fixes quote-browse context + register-tab no-check + gating in one (Rule 11/22). Deliverable `docs/dev/POS_SALE_VS_QUOTES_PARITY_AUDIT.md`; 3 open operator questions. No source/migration/test changes; fix is a separate session. Not a 13-item entry. **Earlier 2026-05-28** — **Session #115: read-only audit — POS prereq-enforcement regression + add-on gating + create-service tier persistence** (branch `audit/pos-prereq-enforcement-regression-and-addon-gating`, isolated `git worktree`). Decided whether #114 is fix-forward or revert. **Verdict: #114 caused none of the 3 post-deploy issues — all pre-existing gaps surfaced by creating/testing the new `addon_only` "Paint Correction Prep".** (1) create-flow only writes 3 `vehicle_size` tiers — no exotic/classic (`new/page.tsx:231-237`; live DB confirms 3 tiers); (2) add-on-only gating NEVER built (no `classification` check in any POS add path — grep: test fixtures only); (3) prereq enforcement is intact in catalog-browser/ServiceDetailDialog (funnels through `addServiceChecked`, unchanged by #114) but `register-tab.tsx` has ZERO prereq handling in any path (favorites + picker dispatch `ADD_SERVICE` directly) — true before and after #114 — and the client fails open (`use-prerequisite-check.ts:77-78,109-110`). #114's diff is a behavior-preserving tier-selection refactor (`selectPricingTierForVehicle()`); detection endpoint + hook untouched, no `picker-engine` import. **Recommendation: FIX-FORWARD** (revert reintroduces the wanted Suburban $75→$110 fix and fixes nothing). Deliverable `docs/dev/POS_PREREQ_ENFORCEMENT_AND_GATING_AUDIT.md` with 6 targets + 4 open operator questions (chief: confirm exact repro screen). No source/migration/test changes; no fixes applied. Not a 13-item entry. **DO NOT merge — operator + Claude review, then scope fixes.** **Earlier 2026-05-28** — **Session #113: harden + version-control the VPS deploy script** (branch `chore/harden-deploy-smartdetails-script`, infra/repo-hygiene, no app code/migrations/deploy; built in an isolated `git worktree` parallel to other active sessions). Brought the previously-unversioned `/usr/local/bin/deploy-smartdetails` into the repo at `scripts/deploy/deploy-smartdetails.sh` (+ README) and hardened it after the 2026-05-27 transient-`npm ci` incident: H1 diagnosable npm ci (tee + `${PIPESTATUS[0]}`, no `--silent`), H2 honest probability-ordered fail message, H3 retry-once (absorbs transient blips; genuine errors fail both attempts → exit 2), H4 `rm -rf .next` before build, H5 timestamped phase log. All 4 guards + 9 steps + exit codes 0-5 preserved. `/usr/local/bin/deploy-smartdetails` to become a symlink to the repo copy (future hardening ships via `git pull`). Verified `bash -n` + stub-harness trace of the retry/PIPESTATUS/`set -e` logic (shellcheck unavailable). Not a 13-item entry (infra). #112 left earmarked for the deferred catalog S1 slug-on-rename remediation. **DO NOT merge — operator reviews, merges, runs one-time VPS symlink migration.** **Earlier 2026-05-27** — **Catalog CRUD wiring audit** (read-only, branch `audit/catalog-services-products-tiers-addons-crud-wiring`, doc-only, in an isolated `git worktree` per [[feedback-parallel-doc-sessions-use-worktree]] since this ran parallel to the Item 15e β-2 code session). Root-caused the operator-reported "Failed to create service": the Add-New-Service form inserts via the browser Supabase client but omits NOT-NULL `services.slug` (no slug in `serviceCreateSchema`, no slug input/auto-gen) → every insert hits a 23502 NOT-NULL violation masked by a generic toast; products create works because it generates+inserts slug. RLS (`services_write = is_employee()`) and permissions (`services.edit` granted to admin/super_admin) both ruled out via live DB. Swept all catalog CRUD (services/products/tiers/add-ons/categories): **1 Critical** (create), **3 Significant** (slug-drift-on-rename SEO, sale-price CHECK trap, systemic generic-toast masking), **5 Minor**, + Informational. Live integrity sweep clean (30 active services, 432 products, no orphans/missing-pricing/test-data; 1 stale suggestion → inactive add-on). Findings + severity table + remediation sequence in `docs/dev/CATALOG_CRUD_WIRING_AUDIT.md`. No source/migration/test changes; no fixes applied (operator locks remediation plan). Not a 13-item entry (catalog services are out-of-scope of the 13 active items). **Earlier 2026-05-27** — Roadmap bump to v4.0 capturing the SMS-AI v2 prompt-fidelity arc closure (Issues 39-46) shipped across sessions #85-96 between 2026-05-26 and 2026-05-27. Doc-only session. **Earlier 2026-05-25** — **Catch-up session: ROADMAP ledger backfill for sessions #73–79 (SMS-AI v2 Issue 36→38 arc).** Doc-only. Added 7 ledger rows for work that landed 2026-05-24 evening → 2026-05-25 but outran session-end capture during the Issue 36 cascade: #73 quote source tracking (surfaced from git log — was NOT in the original catch-up inventory, which started at D39), #74 D39 + #75 D40 (two insufficient Issue-36 attempts — prompt/schema, then dispatcher injection), #76 Issue 36 Phase B diagnostic (read-only — located the endpoint main-tier root cause at `services/route.ts:268+325`), #77 D41 (THE Issue 36 fix — 2-line `sizeClass` pass), #78 D42 (3-tier prefix-match resolver fallback, closes Issue 37), #79 Issue 38 tier-intent audit (read-only — B1 recommended over B2, fix deferred to Sessions A/B/C). Sources: CHANGELOG.md (authoritative) + `SMS_AI_V2_PROMPT_OBSERVATIONS.md` Sections 2/7 + git log across all branches (audit sessions #76/#79 live on un-merged `audit/*` branches, given ledger rows with their branch commit hash per existing precedent, e.g. #70 `25e9e981`). No source code, no migrations, no test changes. **Earlier 2026-05-17** — **Item 15f Phase 1 Layer 8f landed — Phase 1 COMPLETE — Item 15f COMPLETE**: Tests-only session, zero production code changes. Three deliverables: **(1)** new end-to-end integration test file `src/lib/appointments/__tests__/edit-flow.integration.test.ts` (14 cases) joins load → drain → save and pins cross-layer invariants — source=appointment + source=job (Option G4) happy paths, source=job null appointment_id refusal, modifier-only edits, combined service+modifier edits, all-services-removed save block, bogus UUID 404, parameterized status-guard lockstep (`completed`/`cancelled`/`no_show`), drain↔cascade pricing parity for mobile_fee synthesis. **(2)** Extended `pos-workspace-products-gating.test.tsx` with the missing Layer 8d gates — barcode-scanner short-circuit (captures `onScan` callback via hoisted mock; verifies edit-mode never hits the barcode-lookup API) + global-search filteredProducts (ProductGrid never mounts in edit mode). +3 cases. **(3)** New `docs/dev/PHASE_1_TEST_COVERAGE.md` — per-surface × test-type coverage matrix (50+ rows across all Phase 1 layers), 9 documented intentional gaps (G1-G9) with rationale, file-to-test mapping for future maintenance, hand-off notes. **Layer 4 ESLint regression verified**: rule at `'error'` (eslint.config.mjs:76), zero `eslint-disable services/no-bespoke-pricing` comments in `src/`, both new/modified test files lint clean. +17 tests net new. **1503/1503 vitest pass** (was 1486 at Layer 8e). typecheck 0 new errors on touched files (2 pre-existing unrelated test errors persist); ESLint 0 errors / 98 warnings unchanged baseline; production build compiled clean. **Phase 1 is now CLOSED.** Item 15f's overall scope (engine + hook + booking/voice/SMS migrations + ESLint rule + edit-via-POS pivot for operator surfaces) is complete.
...
**Total items:** 10 not-started + 9 done + 1 closed + 1 absorbed (Items 1, 6, 12, 15a, 15b, 15c, 15e, 15f, 15g done; Item 5 closed: NFC already enabled per Stripe support; Item 15d absorbed into Item 15e on 2026-05-27). *(Done count reconciled in Session #145 — 15e was missing from this enumeration although it was already marked CLOSED in the Status Table below.)*

---

## Roadmap Snapshot (2026-05-27)
...
| 15d | "Today's Tickets" Combined View | 🔀 absorbed | Absorbed into Item 15e (POS Jobs Unified Operations View) on 2026-05-27. The unified day-of operations view + appointment-detail modal + multi-type ticket support are now ONE workstream under 15e. |
| 15e | POS Jobs Unified Operations View (absorbs Item 15d) | ✅ CLOSED (2026-05-27) | Reframed 2026-05-27 from "POS Appointments Modal Parity" to "POS Jobs Unification". Absorbed Item 15d. Shipped across Phase 1A/1B (Schedule scope endpoint + populate gate + scope toggle), 2A/2B (dual-context AppointmentDetailDialog + POS Schedule mount + status pill), the status-sync audit, and 2C-α/β-1/β-2 (un-materialize: lifecycle-sync seam + endpoints → shared modal + dryRun → admin Save intercept + POS "Revert to Pending" button). Deferred follow-ups: Item 15h (full bidirectional appointment↔job sync) + Item 13 (Detailer Mobile Link). Capability reference preserved at `ITEM_15E_ADMIN_APPOINTMENT_CAPABILITY_REFERENCE.md`. |
...
- **Done (9):** 1, 6, 12, 15a, 15b, 15c, 15e, 15f, 15g — plus prior closure of Item 5 (NFC already enabled per Stripe support). *(15e flipped from "In progress" to ✅ CLOSED on 2026-05-27 — see the per-item table above. Roll-up reconciled with the table in Session #145.)*
...
- **Not started (10):** 2, 3, 4, 7, 8, 9, 10, 11, 13, 14.
- **Blocked (0):** nothing currently blocked.

---

## Current Status — Evidence-Based (as of 2026-06-03, post-Session #145)

> Audit snapshot from Session #145 cross-referencing the Status Table above against the
> session-by-session ledger and CHANGELOG entries through #144. The 13-item table itself has
> been correct since the v3.8 (2026-05-19) bump — what's drifted is the meta-context around
> it: the "Last session updated" pointer (was #124, now #144); the Roll-up's "In progress (1):
> 15e" line (15e closed 2026-05-27 per the Status Table — now reconciled above); and several
> missing ledger rows for sessions that landed on main without a ROADMAP-13 entry. Every
> "DONE" claim below cites the session number + commit hash per Memory #11; "UNKNOWN" is
> reserved for cases where evidence is ambiguous.
>
> **The big picture:** every session from #110 through #144 (35 sessions) ends with "Not a
> 13-item entry (...follow-up arc)" in its ledger row. The 13-item scope itself has been
> idle since 2026-05-17 (15f Layer 8f, the last implementation sub-layer). All recent work
> has been on out-of-scope follow-up arcs — catalog CRUD, POS Sale-vs-Quotes parity,
> vehicle taxonomy/classifier/form behavior, public-booking architecture (Path B), and Q-D
> pricing_model immutability.

### Per-item evidence table

| # | Item | Documented status | Evidence | Verdict |
|---|---|---|---|---|
| 1 | POS Customer Search → Create Smart Prefill | ✅ done | Ledger #1 (`6b0413dd`), 2026-05-15 | ✅ DONE |
| 2 | Tip on Full-Payment Stripe Payment Link | ⚪ not started | No ledger entry covers Item 2 | ✅ NOT STARTED |
| 3 | Receipt Tip Display Audit + Fixes | ⚪ not started | No ledger entry covers Item 3 | ✅ NOT STARTED |
...
| 8 | Assign Customer to Walk-In Ticket Post-Completion | ⚪ not started | No ledger entry covers Item 8 | ✅ NOT STARTED |
| 9 | BT Scanner Intermittent Failures | ⚪ not started | No ledger entry covers Item 9 | ✅ NOT STARTED |
| 10 | Swipe-to-Delete on Inventory Counts (iPad) | ⚪ not started | No ledger entry covers Item 10 | ✅ NOT STARTED |
| 11 | Keypad / Scan-Each Toggle for Inventory Counts | ⚪ not started | No ledger entry covers Item 11 | ✅ NOT STARTED |
| 12 | Appointments in POS Footer + Edit Appointment from POS | ✅ done | Ledger #12 area, 2026-05-15/16 | ✅ DONE |
| 13 | Detailer Mobile Link (Full Mobile Workflow) | ⚪ not started | No ledger entry; largest greenfield | ✅ NOT STARTED (revisit ROI) |
| 14 | Intake Control Panel + Per-Vehicle Zones + Photo Approval | ⚪ not started | No ledger entry; second-largest greenfield | ✅ NOT STARTED |
| 15a | Edit Services in Admin Appointment Dialog (cascade) | ✅ done | Multiple ledger rows through Layer 8e (2026-05-17 `8726053d` first ship; bespoke modal deleted Layer 8e) | ✅ DONE |
| 15b | Cancel Appointment from POS Appointments Tab | ✅ done | Ledger area 2026-05-16 | ✅ DONE |
| 15c | "Change Time" Affordance on Jobs Card | ✅ done | Ledger area 2026-05-16 | ✅ DONE |
| 15d | "Today's Tickets" Combined View | 🔀 absorbed | Status Table + Decisions superseded note — absorbed into 15e 2026-05-27 | ✅ ABSORBED |
| 15e | POS Jobs Unified Operations View (absorbs 15d) | ✅ CLOSED (2026-05-27) per Status Table | Shipped across Phase 1A/1B + 2A/2B + status-sync + 2C-α/β-1/β-2; deferred follow-ups split into Item 15h + Item 13 | ✅ DONE (per Status Table) — Roll-up drift reconciled this session |
| 15f | Service Picker Engine: canonical resolver + hook + migration | ✅ done | Phase 1 complete 2026-05-17 (`10f7cffb`); all sub-layers 8a–8f shipped | ✅ DONE |
| 15g | Lifecycle Persistence: Discount/Coupon/Loyalty across Quote→Job→Txn | ✅ done | All 5 sub-layers complete 2026-05-17; unblocked Phase 1 of 15f | ✅ DONE |

**Roll-up (evidence-confirmed):** 9 done (1, 6, 12, 15a–c, 15e, 15f, 15g) + 1 closed (5) + 1 absorbed (15d) + 10 not started (2, 3, 4, 7, 8, 9, 10, 11, 13, 14) + 0 in progress + 0 blocked. Matches the corrected Roll-up above.

### Recent activity not in the 13-item table (last 35 sessions)

Sessions #110 → #144 (2026-05-27 → 2026-06-03) all ended with "Not a 13-item entry." The work clusters are:

- **#110** — Item 15e Phase 2C-β-2 corrective (Supabase 1:1 cardinality shape in `has_active_job`)
- **#111–#114** — Catalog CRUD wiring audit + fix arc (C1 slug; S3 error helper; #112 audit, #114 fix for prereq size-aware pricing via `selectPricingTierForVehicle`)
- **#115–#124** — POS Sale-vs-Quotes parity arc (4 audits + sweep + Track-A `useValidatedServiceAdd` helper + Track-B panel parity + gate-order correction + admin services-edit dropdown sibling)
...
- **#149** — POS Schedule filter N+2: search input + status + detailer dropdowns + AND-across-categories client-side filter (`entryMatchesFilters` pure predicate). Target A verdict: NO endpoint extension (audit D.6/D.7 locks status/detailer/search as client-side filters over the date-window fetch). 2 prod files + 2 test files; 238 prod lines net — within Memory #8 budget. +38 tests. Implements F.2 + F.6 from the design; X2 status restriction enforced by dropdown options + regression test. Optional N+3 polish next, or skip to N+4 Admin > Appointments retirement.
...
The audit found these gaps beyond #118; each is surfaced here rather than silently swept (Memory #29 Targeted discipline + the session prompt's explicit "for non-trivial gaps, SURFACE for operator decision"):

1. **5 additional missing ledger rows** (all read-only audits that landed on main without a ROADMAP-13 ledger entry):
   - **#112** — `audit/pos-prerequisite-autoadd-size-aware-pricing` (commit `b6584959`, merge `15c0a78f`, 2026-05-28). Audit deliverable: `POS_PREREQUISITE_PRICING_AUDIT.md`. The audit that scoped #114's fix.
   - **#125** — `audit/exotic-classic-create-form-and-public-site-leak` (commit `16cd1adb`, merge `464d544c`, 2026-05-29). Bundled: admin Add-New-Service form's exotic/classic silent-drop + public-site leak sweep.
   - **#126** — `audit/vehicle-taxonomy-comprehensive` (merge `1dd4cac7`, 2026-05-29). Broadest vehicle audit: schema, POS, admin, public, pricing/prereq.
   - **#127** — `audit/public-booking-form-reset-and-flow-verification` (merge `d5ea9e65`, 2026-05-30). Diagnoses non-automobile + make-selection silent-reset bug + end-to-end flow walk for all 5 categories.
   - **#128** — `audit/vehicle-form-pattern-unification-feasibility` (merge `c872c05d`, 2026-05-30). Resolves F5 from #127 — unification verdict was NO-UNIFY per `VEHICLE_FORM_UNIFICATION_AUDIT.md`.
   - **#135** — `audit/vehicle-forms-comprehensive-behavior` (merge `d3c65ae3`, 2026-05-31). Triggered by operator's holistic-not-patch framing on vehicle-form behavior; scoped #136's bundle of fixes.

   All 5 are read-only audits with full CHANGELOG entries. Operator decision: backfill these as a sibling session to #145 (~5 ledger rows; ~30 minutes), or leave the CHANGELOG as the operative record for audit sessions of this era.

2. **15e Roll-up vs Status Table drift** — RECONCILED this session. The Status Table said `✅ CLOSED (2026-05-27)` but the Roll-up at line 58–62 still said `In progress (1): 15e`. The "Total items" line at line 23 underrepresented the done count (listed 8 items, said "9 done"). Both reconciled in #145; 15e now appears in both Done lists.

3. **"Last session updated" header staleness** — UPDATED this session. Was `2026-05-29 — Session #124`; now reflects #144 (the Q-D session that shipped 2026-06-03). The `Document version: v4.0 (2026-05-27)` block is preserved as-is — operator may want to bump to v4.1 to reflect the post-#124 catalog + Sale-vs-Quotes + vehicle/classifier arcs explicitly, but that's a content-scoping decision deferred here.

4. **Memory #12 — "Booking multi-service SHELVED INDEFINITELY"** — SURFACED for operator decision. The architectural audit (`PUBLIC_BOOKING_ARCHITECTURAL_AUDIT.md`, commit `709befa5`) Q-Arch-2 LOCKED single-primary stays — captured in CLAUDE.md Rule 22's W5 paragraph and in the #140 ledger row's "multi-primary work (Path B locked — Q-Arch-2 single-primary stays)" out-of-scope note. The ROADMAP-13 itself never carried a "booking multi-service" item, so there is nothing to formally re-status here — but if Memory #12's framing should be made discoverable from this document (e.g., a new entry in the "Closed items" or "Decisions superseded" tables citing Q-Arch-2 as the durable reason), that's an operator product-decision, not a clerical fix. Not applied unilaterally.

5. **Older session-number gaps (#12, #15, #16, #18, #38, #56, #83, #84)** — left as-is. Some are likely renumbering / parallel-session collisions / abandoned branches from the early sprint; chasing them down would expand scope well beyond Memory #29's Targeted discipline. Surfaced here for awareness only; not actionable from CHANGELOG alone.

### Suggested next move (informational only)

If anyone wants to actually make progress on the 13-item backlog, the natural Wave 2 starting point (Items 3, 4, 2 — the tip cluster) is sequential and self-contained; the Item-3 audit was scoped 2026-05-15 (see Wave 2 section below). Wave 3 (Items 7, 8) is smaller. Wave 4 (Items 9, 10, 11) needs hardware-side investigation (BT scanner). Wave 5 (Items 13, 14) is the multi-week greenfield commitment; revisit ROI before scheduling.
...
| Layer 5: Cutover (delete specialty pivot block, delete `staff_notification_inbound_specialty` template) | ✅ done (Phase A 2026-06-17 + Phase B monitoring + Phase C 2026-06-18) | **Phase C shipped 2026-06-18** — v1 single-shot responder eradicated. Deleted `src/lib/services/messaging-ai.ts` (511 lines incl. `getAIResponse`/`buildSystemPrompt`/`CustomerContext`) + `src/lib/services/messaging-ai-prompt.ts` (114 lines incl. `getDefaultSystemPrompt`). Deleted ~448 lines from `src/app/api/webhooks/twilio/inbound/route.ts` (legacy 5-query context fetch, getAIResponse call, specialty-pivot block, `extractQuoteRequest` auto-quote, `extractAddonActions` markup-block addon approval, send-auto-reply chunking). Webhook file 990 → 448 lines. Removed `extractAddonActions` from `src/lib/services/job-addons.ts` + the AUTHORIZE_ADDON/DECLINE_ADDON regex constants (v2 uses tool calls `approve_addon`/`decline_addon` instead). Soft-deleted `staff_notification_inbound_specialty` (Concern E LOCKED: `is_active=false`; row preserved for historical/audit purposes) + removed slug from `src/lib/sms/sms-contracts.source.ts` + regen of generated contracts. Refactored `src/lib/sms-ai/system-prompt.ts` — extracted prompt body into pure `getStandardTemplate(): string`; `buildV2SystemPrompt` is now async + DB-first with hardcoded fallback (closes audit §10 admin-textarea drift). New migration `20260618190500_seed_messaging_ai_instructions_v2.sql` seeds verbatim v2 prompt body into `business_settings.messaging_ai_instructions` (operator pre-flight verified 6,549-byte row = verbatim v1 default, zero customization → safe ON CONFLICT DO UPDATE overwrite). Admin page `/admin/settings/messaging` rewired to import `getStandardTemplate` (no UX change — Admin UI Option A LOCKED, no warning banner). Observability: per-inbound `[SmsAiV2 prompt] source=db bytes=<n>` / `source=fallback reason=<...>` log line (partial Layer 6 — structured metrics still TBD). Tests updated: `sms-ai-v2-routing.test.ts` rewritten (all-phones-v2 default; kill-switch / non-allowlisted / flag-throw now assert "no AI fires" instead of "legacy fires"); `start-words-gate.test.ts` `getAIResponseMock` removed; `system-prompt.test.ts` migrated to async (supabase mocked, 174 calls awaited, all `it()` async); +10 new tests for DB-source/null/empty/whitespace/error fallback branches + getStandardTemplate snapshot. **Post-Phase-C kill-switch semantics: "v2 off, NO AI" (worse than legacy v1, manual inbox until fix). True rollback = git revert + redeploy + SQL-restore messaging_ai_instructions.** See `docs/dev/SMS_AI_V2_ROLLBACK.md` Phase C runbook section. Closes the audit §10 finding from 2026-05-19 (admin textarea no longer silently stale for live v2 traffic). | _(legacy preserved below for ledger context)_ **Phase A shipped 2026-06-17** — flipped `sms_ai_v2_globally_enabled` to `true` via migration `20260617223520_flip_sms_ai_v2_globally_enabled.sql` (single idempotent UPDATE; applied via `supabase db push --linked`). All inbound SMS now routes through SMS AI v2 (was: 3-phone allowlist `+13107564789`/`+14243396994`/`+13103101445`). v1 code path remains in source as fallback but is unreachable on live traffic; `sms_ai_v2_kill_switch` flip is the only path back to v1 — see `docs/dev/SMS_AI_V2_ROLLBACK.md`. Motivation: 2026-06-17 ~22:00 PST diagnostic confirmed v1 was silently dropping inbound SMS for non-allowlisted phones (`+13109461688` sent 7 messages with `is_ai_enabled=true` + `status='open'`, zero AI replies); allowlisted `+13107564789` got v2 reply in 4 seconds. **Phase B** (operator-driven monitoring) — operator validates v2 behavior on non-allowlisted traffic for 30-60 minutes post-deploy; PM2 log `[SmsAiV2 routing]` line + `messages` table query for `sender_type='ai'` rows on non-allowlisted phones. Green light → schedule Phase C; anomaly → kill_switch rollback per runbook. **Phase C** (pending) — delete `getAIResponse()`, `messaging-ai.ts`, specialty pivot block, `[AUTHORIZE_ADDON]` parse block in webhook, legacy 5-query customer-context block. v2 becomes the only SMS AI path in code. **Phase D** (pending) — delete `staff_notification_inbound_specialty` SMS template (orphaned after Phase C). Closes the Ferrari-loop bug visible in production conversation Nayeem Khan / May 18. |
| Layer 6: Tests + observability | 🟡 partial (admin DB-prompt wiring shipped 2026-06-18 as part of Phase C; structured metrics still TBD) | Final layer. Phase C (Layer 5) delivered: (a) admin textarea ↔ runtime prompt wiring (closes audit §10), (b) per-inbound `[SmsAiV2 prompt] source=db bytes=<n>` / `source=fallback reason=<…>` log line for prompt-source telemetry, (c) +10 new prompt-source tests (DB-set / null / empty / whitespace / error fallback branches + getStandardTemplate snapshot + grounding-substitution parity). Remaining: structured metrics (counter of prompt-source distribution per hour, agent-run latency histogram, tool-call failure rate dashboard), per-inbound trace IDs end-to-end, alerting on fallback-rate spikes (signals operator-prompt regression). Not blocking — v2 is fully functional in production. |

**Parallel prompt-fidelity track (Workstream J + production-driven cascades) — status as of 2026-05-27:** While Layers 5–6 of the 6-layer rollout remain not-started, the live v2 agent has been under continuous production-driven prompt + pricing-fidelity refinement since the Layer-4 deploy. Issues 26–46 are captured in `SMS_AI_V2_PROMPT_OBSERVATIONS.md` (Section 2); decisions D33–D50 are locked (Section 7). **All resolved:** Issue 26 (server-side phone injection, #57), Issues 30–32 + 34 (idempotency guard + upsert_customer cadence, #59/#62), Issue 33 (combo-resolver, #65/#66), Issue 35 (mandatory customer reply D38 + runner backstop, #67/#69), Issue 36 (size_class endpoint fix — closed by D41 #77 after D39 #74 + D40 #75 + the #76 diagnostic), Issue 37 (resolver prefix-match D42, #78), Issue 38 (tier-intent gap, 3-session D43 implementation #80/#81/#82; audited at #79 `3a9b06fe`), Issue 39 (D45 chip composition `0ad3e89d`), Issue 40 (operator admin tier_label edits), Issue 41 (D46 15-visual-surface adoption `1a6aea73`), Issue 42 (D48 `appointment_services.quantity` schema + conversion `d4e2fd97`, #92/#93), Issue 43 (D47 Critical Rule 8 price-lookup discipline), Issue 44 (D47 Critical Rule 9 scope-tier enumeration), Issue 45 (D49 Option A proactive auto-send #95 + D50 universal friction-question prohibition #96), Issue 46 (UI label channel-aware refinement, two iterations). **The SMS-AI v2 fidelity arc (Issues 26–46) is now fully closed via D43-D50; remaining SMS-AI v2 work is the Layer 5–6 legacy cutover/observability (NOT fidelity). After Layer 5-6 closes, v2 admin UI wiring becomes feasible (operator-editable prompt via `business_settings.messaging_ai_instructions`).** See ledger rows #73–96 and the Workstream J section below.
...
### Captured / Documentation Follow-ups (2026-05-27)

Items surfaced during the SMS-AI v2 prompt-fidelity arc (Issues 39-46) closure but deferred — not yet promoted to scheduled sessions.

**Issue 47 — Soft-friction permission pattern ("Want to go with that?")** [⚠️ MONITOR]

Surfaced 2026-05-27 during D50 verification (Scenario 4 — customer-question don't-fire). After agent answered the customer's "Can you confirm the total?" request, agent closed with "Want to go with that?" — a soft permission-ask about configuration confirmation (NOT quote-send specifically, so it doesn't violate Rule 17's universal prohibition).

**Status:** ACCEPTABLE for now — the customer explicitly asked for verification, so a confirmatory follow-up is conversationally natural. NOT a hard friction-step regression.

**Watch for:** if this pattern recurs across many customer interactions (not just after explicit verification requests), it becomes a generalization risk — the LLM may extend "Want me to send a quote?" → "Want to go with that?" as a similar permission-eliciting niche. If pattern recurs, escalate to D51 (extend Rule 17's universal prohibition to cover ALL permission-eliciting questions about quote/configuration confirmation).

**Tracked:** in observation log via `[SmsAiV2.send_quote_sms.auto_send_trigger]` PM2 monitoring — operator reviews log frequency vs. friction-question recurrence baseline.

**`system-prompt.ts:12` misleading docstring** [⚠️ DOC FIX] (surfaced 2026-05-27)

The JSDoc at `src/lib/sms-ai/system-prompt.ts:12` claims: *"Operators can override the active prompt via business_settings.messaging_ai_instructions"*. This is aspirational documentation — the v2 runner (`agent-runner.ts` calling `buildV2SystemPrompt()`) does NOT actually read `messaging_ai_instructions` from DB. The function is purely static-string from code.

Only the v1 path (`src/lib/services/messaging-ai.ts`) reads `messaging_ai_instructions` and applies it to v1 LLM calls. Since v2 is the active path for live conversations, the override is effectively non-functional.

**Fix path:** small doc edit either (a) remove the misleading line or (b) replace with accurate text ("The admin UI's saved prompt at `business_settings.messaging_ai_instructions` is currently applied only by the v1 SMS path; v2 uses this file's hardcoded prompt. Wiring v2 to read DB is tracked under Workstream A Layers 5-6 / v2 admin UI follow-on.")

**Tracked:** in Workstream A Layers 5-6 follow-on (gated on v1 eradication so wiring v2 to DB doesn't risk admin-prompt drift during cutover).
...
- 2026-05-27: Reframed to "POS Jobs Unified Operations View" (session #99). Phase 1
  (1A endpoint + populate gate, 1B scope toggle + flag) COMPLETE. Phase 2 plan
  locked from the reuse verification audit (#103): reuse the admin
  `AppointmentDetailDialog` dual-context (not fork). **Phase 2 operator decisions
  locked:** (1) rich admin-dialog reuse; (2) POS status changes FIRE webhooks
  (not suppress); (3) full `STATUS_TRANSITIONS` exposure (no POS narrowing);
  (4) no new permission keys; (5) no role-defaults migration.
- 2026-05-27: Phase 2A (session #104) COMPLETE — foundation half (Memory #8 split).
  Shared lift (`src/lib/appointments/status-transitions.ts` + `types.ts` with admin
  re-export), dialog parameterized (3 admin-default props) + dark-mode pass
  (no-op in admin), combined `PATCH /api/pos/appointments/[id]` (HMAC + per-field
  perms + STATUS_TRANSITIONS enforced + webhooks fire) with 17 tests. Admin
  byte-identical; the new PATCH is inert until 2B mounts the dialog. **Phase 2B
  (next):** mount + wire the dialog into the POS Schedule scope + Schedule status
  pill.
- 2026-05-27: Phase 2B (session #105) COMPLETE — wiring half. **Phase 2 done.**
  Single production file (`job-queue.tsx`): tap a Schedule card → fetch
  appointment + bookable staff → mount the reused admin dialog (POS context
  props) → save via 2A's POS PATCH → close + refetch. Cancel hands off to the POS
  cancel dialog. Added appointment-status pill (POS-local helper, dark-aware) to
  Schedule cards. **STATUS_DOT_COLORS NOT lifted** (admin dots vs POS pills are
...
- 2026-05-27: **Fresh ROI signal from Item 15e Phase 2C** — the brief for that
  session asked to file a NEW "Item 16 — Mobile Detailer Remote Job Management,"
  but that scope is THIS item (magic link, no detailer auth, per-job token
  expiring at job close, mobile photo/intake/complete/payment). Captured here
  instead of duplicating. The new evidence: during the Phase 2 status-sync
  divergence investigation, **4 of 6 divergent appointment/job rows were mobile
  appointments** where the mobile detailer could not advance the job from POS
  (POS is IP-whitelisted to the shop; the detailer is off-premises), so jobs sat
  in `scheduled` while admin moved the appointment. This is a concrete, recurring
  operational gap (not hypothetical) and is the strongest ROI data point to date
  for scheduling this epic. The Phase 2C-α `lifecycle-sync.ts` seam +
  `executeUnMaterialize` may also be relevant infra when this item lands (the
  mobile flow advances `jobs.status`, which Item 15h's bidirectional sync would
  reconcile to `appointments.status`).
...
|---|---|---|---|---|---|
| 2026-06-19 | 154 | Workstream A — SMS AI v2 Layer 6 Tier 1 observability | Production code change (1 NEW prod file + 6 prod MOD + 1 webhook MOD + 4 test MOD + 3 doc MOD), merged to main; NO migrations, NO schema, NO admin UI, NO behavior changes | _(this commit)_ (branch `feat/sms-ai-v2-layer-6-tier-1-observability`) | Targeted Type 1 audit + fix (Memory #29). **Scope LOCKED via 4-question AskUserQuestion flow:** Q1 Tier 1 only (Tier 2 audit_log + Tier 3 admin UI deferred to future sessions with their own pre-flight); Q2 no audit_log writes this session; Q3 stable key=value format (not JSON-line); Q4 3 prefixes ([SmsAiV2] / [SmsAiV2 runner] / [SmsAiV2 dispatch]) consolidating pre-Layer-6's 7 prefixes. Plus locked: G1 latency capture at background-dispatch entry (sub-1ms gap vs webhook-entry, accepted); G5 conversation_close 11-field hard cap (PM2-tail readability ceiling); G6 enum + `isV2ErrorClass` type guard (operator refinement on CC's proposal — prevents future drift); G7 per-iter cache stats + accumulated rollup at close. **Pre-flight audit corrections (CC re-grep):** operator's stated baseline of "7 sparse log prefixes" understated emission-site count (7 distinct PREFIXES but ~37 EMISSION SITES across 5 v2 modules + webhook); "cron has 14+ lines per request, aspire to that" was conservative (cron actually emits 22 lines BUT mostly error-on-failure — happy-path is 0-5 lines; v2 already exceeds cron in line volume). Re-framed "aspire to cron richness" as structure + persistence + consumability, not raw volume. **Audit-isolation pre-implementation (Q1-Q4 lock):** CC-side grep of scripts/, docs/, *.json/*.yml/*.yaml/*.conf/*.toml configs, src/ outside known surfaces — zero non-doc consumers. Operator VPS-side check of /var/log/ + /etc/ — also zero (no Datadog/Logtail/fluent-bit/vector; cPanel crons don't parse PM2; agent360 doesn't touch app logs). Safe to rename. **Implementation (9-step locked order):** 1. NEW src/lib/sms-ai/observability.ts (~110 LoC: SmsAiV2ErrorClass enum + isV2ErrorClass type guard + CacheStats + emptyCacheStats + formatLogFields key=value formatter + formatErrorMessage normalizer); 2. agent-runner.ts MOD (destructure {text, source} from buildV2SystemPrompt, cacheStats accumulator, replace per-iter + dispatch + retry + forced-final + api-error log lines with formatLogFields, retire 4 done lines, set errorClass on error paths); 3. background-dispatch.ts MOD (startedAt at entry, LOG_PREFIX → [SmsAiV2], single conversation_close line per inbound with 10-11 fields, sendAndLogChunks returns { anySuccess } for accurate reply_sent, dispatch-throw catch emits TWO lines to keep close at cap, 6 transport-error sites consolidated under [SmsAiV2] event= discriminator); 4. tool-dispatcher.ts MOD (dot-delimited outlier → [SmsAiV2 dispatch] event=auto_send_trigger, 5 per-tool summaries normalize latency=Xms → latency_ms=X via formatLogFields, key-load error emits error_class=tool_key_missing); 5. system-prompt.ts MOD (buildV2SystemPrompt returns BuiltSystemPrompt = { text, source } instead of bare string — runner needs source for conversation_close without re-deriving; 3 log emissions consolidated under [SmsAiV2] event=prompt_loaded\|prompt_load_error; header docstring updated); 6. feature-flag.ts MOD (both load-error sites consolidated under [SmsAiV2] event=flag_load_error; success path emits no log line — conversation_close provides the per-inbound positive signal); 7. route.ts MOD ([SmsAiV2 routing] → [SmsAiV2] event=routing; v2Err catch → event=routing_error error_class=flag_dispatch_threw; outer fire-and-forget .catch → event=dispatch_thrown). 8. Tests: 184 const-assignments in system-prompt.test.ts destructured via single-regex sed; 6 existing log-shape assertions updated to new shapes; NEW 5-test `conversation_close line shape` regression suite in background-dispatch.test.ts (pins success line / api_error line / prompt_source=fallback / reply_sent=false on all-chunks-failed / dispatch_thrown TWO-line emission); helpers extended with RunAgentResultOverrides for cacheStats + promptSource + errorClass. 9. Docs: SMS_AI_V2_ROLLBACK.md PM2-grep example updated (operationally critical — runbook is post-deploy verification path); FILE_TREE.md background-dispatch description + NEW observability.ts entry; SMS_AI_V2_PROMPT_OBSERVATIONS.md historical-traces note explaining Layer 6 prefix change. **Hard rules respected:** NO new deps, NO schema, NO migrations, NO admin UI, NO behavior changes (routing/agent loop/tool execution/prompt loading/decision logic byte-identical). All emissions stdout-format only. **Forward-facing audit:** zero v1/v2 string references in customer SMS bodies, admin UI labels, or operator-visible toasts (re-verified). **Scope:** 1 NEW prod + 6 prod MOD + 4 test MOD + 3 doc MOD. ~600 LoC net (most in observability.ts NEW + CHANGELOG entry). **Gates:** tsc 3 baseline preserved (customer-accept-service.test.ts:44,49,73 TS2556 pre-existing, untouched per Memory #29 Targeted-discipline); lint 0 new on changed files (98 warnings baseline preserved); vitest 3470/3470 (was 3465 baseline; +5 = NEW conversation_close regression suite); build clean. **Tier 2 (future):** audit_log persistence will mirror conversation_close as a structured row using the same 10-11 field schema as the canonical Tier 2 schema (observability.ts header documents this). **Post-deploy verification:** `ssh root@31.220.60.157 'pm2 logs smart-details --lines 200 --nostream 2>&1 \| grep -E "\[SmsAiV2\]\|\[SmsAiV2 runner\]\|\[SmsAiV2 dispatch\]" \| tail -40'` — expect structured event= discriminators on every line; cache_reads > 0 on iter 2+ within same conversation if Phase C prompt is cacheable; cache_creates non-zero on every iter = prompt-drift investigation needed. Not a 13-item entry (Workstream A out-of-scope follow-up arc; Tier 2/3 still pending). |
| 2026-06-18 | 153 | Fix: lifecycle-engine Phase 4 SLA cron PGRST201 disambiguation | Production code change (1 prod file MOD, 1 test file MOD, 1 regression test added) | _(this commit)_ (branch `fix/sla-cron-pgrst201-fk-hint`) | Targeted Type 1 audit + fix (Memory #29). **Bug:** `/api/cron/lifecycle-engine` Phase 4 SLA scan (AC-12 pending-appointment alerts) was throwing `PGRST201: Could not embed because more than one relationship was found for 'appointments' and 'quotes'` on every business-hours run since 2026-06-07 — the date `20260607202559_appointments_ac12_schema_additions.sql` added the forward FK `appointments.quote_id → quotes.id` alongside the existing reverse `quotes.converted_appointment_id → appointments.id`. PostgREST cannot auto-pick the relationship when both FKs exist. **Operational impact:** pending-accept appointments past `AC12_SLA_THRESHOLD_MINUTES` (60min) went un-alerted to staff for the full window — silent SLA violation pipeline. **Audit (read-only, ~15 min):** A1 — FK direction (forward `appointments_quote_id_fkey` correct; query is `from('appointments').not('quote_id', 'is', null)` wanting THE SOURCE quote, consumer in `fireSlaAlert` reads `quote.quote_number` + `quote.items` — the source quote shape); A2 — only ONE `quote:quotes(...)` embed in file; A3 — sibling cron files clean (`grep -rn ... src/app/api/cron/` empty for the pattern); A4 — repo-wide grep for actual PostgREST embed syntax `^\s*(quote\s*:\s*)?quotes\s*\(` (after filtering comment-prose false positives across 5 initial matches in `global-search`/`pos/jobs`/`pos/jobs/[id]/checkout-items`/`convert-service`) found this is the ONLY site; A5 — inverse direction (`quotes → appointments` embeds) also clean. **Fix (1 site, 1 edit):** `route.ts:266` `quote:quotes(...)` → `quote:quotes!appointments_quote_id_fkey(...)`. **D1/D2/D3 LOCKED upstream by operator.** **Test fix path (3 attempts, Path A'' landed):** sla-cron.test.ts had pre-existing TS2556 spread-arg errors on lines 33+45 (sendSms + isWithinBusinessHours mock factories). Path A (`Parameters<typeof X>`) cleared TS2556 but introduced TS2554 (typed-arg spread mismatched mocks' inferred 0-arg signatures). Path A' (`never[]` with `as never[]` cast) still failed TS2556 because `never[]` is array-not-tuple and the mocks have fixed-arity rest. Path A'' (operator-authorized): mocks changed to bare `vi.fn()` (inferred `Mock<any[], any>` — symmetric with `renderSmsTemplateMock` + `logAuditMock`), default impls moved to `beforeEach.mockResolvedValue` / `.mockReturnValue`, `.mockClear()` → `.mockReset()` for `sendSmsMock` to prevent impl compounding. All 3 factories now use uniform `(...args: unknown[]) => mock(...args)` pattern. Inline doc comment in beforeEach explains the choice. **Tests (+1 → 3465):** new `regression guards` describe block with two assertions — positive (FK-hinted embed `quote:quotes!appointments_quote_id_fkey(` present in route source) AND negative (no bare `quote:quotes(` embed elsewhere — anti-duplication guard). Source-string structural test (operator preference over full mock-the-supabase-client harness for bug-localization scope). **Hard rules respected:** NO changes to SLA threshold / cooldown / audit_log shape / business-hours logic. NO new dependencies. NO migrations (schema is correct; bug was missing FK hint in embed string, not a schema gap). NO scope creep into broader lifecycle-engine or 4 sibling files (audit confirmed they're clean). **Scope:** 2 files MOD (route.ts + sla-cron.test.ts) + 2 doc MOD (CHANGELOG + this ledger row). ~10 LoC net. **Gates:** tsc 5 baseline → 3 (pre-existing `customer-accept-service.test.ts:44,49,73` TS2556 unchanged — out of scope per Memory #29 Targeted-discipline), lint 0 new errors on changed files (baseline 98 warnings preserved), vitest 3464 → 3465 (+1 regression test), build clean. **Post-deploy verification (operator):** `ssh root@31.220.60.157 'pm2 logs smart-details --lines 200 --nostream 2>&1 \| grep -E "Lifecycle SLA\|PGRST201"'` — expect zero PGRST201 lines + either `[Lifecycle SLA]` info logs (candidates found) or quiet (no candidates / outside business hours). Not a 13-item entry (production-cron bug fix). |
| 2026-06-17 | 151 | Workstream A — SMS AI v2 Layer 5 Phase A: flip `sms_ai_v2_globally_enabled` to true | Migration + docs only, merged to main; NO code, NO test changes | _(this commit)_ (branch `feat/sms-ai-v2-layer-5-phase-a-global-flip`) | Phase A of the 4-phase Layer 5 cutover. Single idempotent UPDATE flips the global SMS AI v2 flag → `true`; all inbound SMS now routes through v2 (was: 3-phone allowlist). v1 fallback code remains in source at `route.ts:536+` but unreachable on live traffic. **Motivation:** 2026-06-17 ~22:00 PST diagnostic — `+13109461688` sent 7 inbound SMS (21:02-21:10) with `is_ai_enabled=true` + `status='open'`, got ZERO AI replies; allowlisted `+13107564789` got v2 reply in 4 seconds. v1 silently dropping non-allowlisted traffic was a real production gap. **Pre-flip audit** (no code changes, audit-only): confirmed `feature-flag.ts`/`webhook routing`/`background-dispatch`/`agent-runner` all unchanged from Layer 4 ship; surfaced 5 non-blocking concerns (C1-C5: audience gates still apply post-flip; legacy specialty pivot + `staff_notification_inbound_specialty` template become unreachable — deferred to Phase C/D; rate-limit shared; Session #150 reactivation runs first). Operator authorized after audit. **Migration:** `supabase/migrations/20260617223520_flip_sms_ai_v2_globally_enabled.sql` — single UPDATE with `AND value = 'false'::jsonb` idempotence guard; applied via `supabase db push --linked` per locked workflow. **Post-flip verification (`supabase db query --linked`):** globally_enabled=`true` (updated 22:35:50), kill_switch=`false` (unchanged 2026-05-19), enabled_phones=`["+13107564789","+14243396994","+13103101445"]` (unchanged 2026-05-20). **New doc:** `docs/dev/SMS_AI_V2_ROLLBACK.md` — emergency rollback runbook (kill_switch flip = instant full v1 fallback; global_enabled re-flip = back to allowlist-only); documents Layer 5 phase timeline so future operators know which rollback path is viable at each phase. **Allowlist intentionally left intact** — redundant post-flip but harmless; Phase D removes. **No source code touched** (per session prompt hard rules) — no edits to `messaging-ai.ts`, `route.ts`, `agent-runner.ts`, `background-dispatch.ts`, `tool-dispatcher.ts`, `feature-flag.ts`, `system-prompt.ts`, `tools.ts`, `customer-context.ts`. No template deletion. No admin UI changes. No new permission keys. **Scope:** 1 new migration + 1 new doc + 2 doc MOD (CHANGELOG + ROADMAP-13-ITEMS) + this ledger row. **Gates:** tsc 0, lint baseline preserved, vitest baseline preserved (no test changes — migration + docs only), build clean. **Phase B (operator-driven, post-deploy):** monitor PM2 `[SmsAiV2 routing]` log line on non-allowlisted inbounds + query `messages` table for `sender_type='ai'` rows on non-allowlisted phones for 30-60 min. Green → schedule Phase C (v1 code deletion); anomaly → kill_switch rollback per runbook. **Phase C** (pending): delete `getAIResponse()`, `messaging-ai.ts`, specialty pivot block, `[AUTHORIZE_ADDON]` parse block, legacy 5-query customer-context block. **Phase D** (pending): delete `staff_notification_inbound_specialty` SMS template. Not a 13-item entry (Workstream A out-of-scope follow-up). |
| 2026-06-03 | 145 | Chore: ROADMAP-13-ITEMS audit + #118 ledger backfill + Roll-up/Status-Table drift reconciliation + Current Status section | Documentation only, merged to main; NO code, migrations, tests, or permission keys | _(this commit)_ (branch `chore/roadmap-13-items-audit-and-118-backfill`) | Housekeeping audit cross-referencing the documented status across 13 items against the session ledger and CHANGELOG entries through #144 (per Memory #11 evidence discipline). **Three trivial fixes applied this session:** (1) **#118 backfill** — added missing ledger row for the Sale-vs-Quotes shared-component parity sweep (`audit/sale-vs-quotes-shared-component-parity-sweep`, branch tip `31199db7`, merge `c79dcf91`, 2026-05-28). The audit DID land on main as #118 with `docs/dev/SALE_VS_QUOTES_PARITY_SWEEP.md` and 6 enumerated gaps (G1–G6), but the ledger row was never added; the renumbering note inside #119's row ("renumbered #118 → #119") was a misread of the parallel-session collision — both #118 (sweep) and #119 (panel-parity fix) shipped that day. (2) **15e drift reconciliation** — the Status Table marked 15e ✅ CLOSED (2026-05-27) but the Roll-up still said "In progress (1): 15e" and the "Total items" line listed 8 done items while claiming 9; both reconciled. (3) **"Last session updated" header refresh** — was #124 (2026-05-29), now reflects #145 with the prior #144 entry preserved as "Earlier" prefix per existing convention. **New section added:** "Current Status — Evidence-Based (as of 2026-06-03, post-Session #145)" with per-item evidence table (every DONE/NOT STARTED/CLOSED/ABSORBED verdict cites a session number + commit hash), recent-activity summary (sessions #110–#144 cluster characterization), and Target C operator-decision surface. **Target C — surfaced for operator decision, NOT silently applied:** (a) 5 additional missing audit-ledger rows (#112, #125, #126, #127, #128, #135 — all read-only audits with full CHANGELOG entries; backfilling all 5 would take ~30 min as a sibling session); (b) Memory #12 "Booking multi-service SHELVED INDEFINITELY" framing — Q-Arch-2 architectural lock is captured in CLAUDE.md Rule 22 + the #140 ledger row, but the ROADMAP-13 has no formal "shelved" entry for booking-multi-service since it was never a 13-item member to begin with; (c) older session-number gaps (#12, #15, #16, #18, #38, #56, #83, #84 — left as-is; likely renumbering / abandoned / parallel-session collisions from the early-sprint era); (d) the `Document version: v4.0 (2026-05-27)` block could be bumped to v4.1 to make the post-#124 follow-up arcs (catalog, Sale-vs-Quotes parity, vehicle/classifier, public-booking Path B, Q-D) discoverable from the doc-version header, but that's a content-scoping decision deferred here. **Big-picture finding:** every session #110 → #144 (35 sessions across 2026-05-27 → 2026-06-03) ends with "Not a 13-item entry" — the 13-item scope itself has been idle since 2026-05-17 (15f Layer 8f, the last implementation sub-layer). All recent work has been on out-of-scope follow-up arcs. **Per-item verdicts (evidence-based):** 9 DONE (1, 6, 12, 15a–c, 15e, 15f, 15g) + 1 CLOSED (5) + 1 ABSORBED (15d) + 10 NOT STARTED (2, 3, 4, 7, 8, 9, 10, 11, 13, 14) + 0 IN PROGRESS + 0 BLOCKED. **Scope:** 0 prod files, 0 migrations, 0 tests, 0 permission keys; 2 doc files MOD (ROADMAP-13-ITEMS.md + CHANGELOG.md). **Gates (sanity check, all expected no-op since no code touched):** tsc 0 errors, lint 0err/97warn baseline preserved, 2869/2869 tests, build clean. Not a 13-item entry (housekeeping). |
| 2026-06-03 | 144 | Fix (Q-D): `services.pricing_model` immutability — documentation + admin UI tooltip | Documentation + UI surfacing, merged to main; NO behavior change | _(this commit)_ (branch `fix/q-d-pricing-model-immutability-doc-and-tooltip`) | Closes **Q4** from `CATALOG_CRUD_WIRING_AUDIT.md` (the only deferred Informational item from that audit) and Q-Arch-D from `PUBLIC_BOOKING_ARCHITECTURAL_AUDIT.md` (709befa5). **Q-Arch-D LOCKED KEEP-IMMUTABLE — operator decision: the omission is BY DESIGN, surface it.** Pre-session and post-session, `src/app/admin/catalog/services/[id]/page.tsx` `onSaveDetails` PUT payload OMITS `pricing_model` from the update set and the Edit page renders no pricing_model selector — model is fixed at creation time. **Rationale:** changing model post-creation would orphan `service_pricing` tier rows (incoherent tier-name vocabularies across `vehicle_size` / `scope` / `specialty`) AND risk inconsistency in historical `quote_items` / `appointment_services` / `transaction_items` rows whose `price_at_booking` was computed under the OLD model's rules. **Operator workflow:** delete and recreate the service under the new model. **Three documentation-only surface changes:** (1) `Info` tooltip next to Edit page's "Pricing" tab title — mirrors canonical admin tooltip pattern at `marketing/coupons/new/page.tsx:1555-1564` (Memory #2 reuse) + adds keyboard-accessibility layer (`tabIndex={0}` + `group-focus-within:opacity-100` + `aria-describedby="pricing-model-immutable-tip"` + `id` match + `role="tooltip"` + `sr-only` accessible name); locked wording — "Cannot be changed after creation. To use a different pricing model, delete and recreate the service. This keeps tier rows, ticket history, and price calculations consistent." (2) Multi-line code comment at PUT-payload site explaining the omission + linking to `CATALOG_CRUD_WIRING_AUDIT.md Q4` + `Q-Arch-D LOCKED (KEEP-IMMUTABLE)` so future readers don't "fix" it. (3) CLAUDE.md Rule 22 amended with the immutability convention + an "Adding a new immutable-after-create constraint" canonical pattern (omit + comment; tooltip; source-string regression test). **Tests (+5 → 2869):** NEW `src/app/admin/catalog/services/[id]/__tests__/pricing-model-tooltip.test.ts` — source-string pins (mirrors `services-summary-adoption.test.ts` precedent vs. mounting the 2200-line Edit page with disproportionate mocks). Pins: `Info` lucide import / tooltip wrapper + locked wording / a11y attrs / PUT-payload regression guard / Q-Arch-D + audit-reference comment. **Scope:** 1 prod MOD + 1 NEW test + 3 doc MOD (CLAUDE.md / CATALOG_CRUD_WIRING_AUDIT.md / CHANGELOG.md) + this ledger row. ~50 prod lines net. No migrations, no new permission keys, **NO behavior change** — the PUT route's payload still excludes `pricing_model` exactly as pre-session. **Gates:** tsc 0, lint baseline preserved, 2869/2869, build clean. **CATALOG_CRUD_WIRING_AUDIT.md Q4** marked RESOLVED. **NOT in scope** (Memory #29 Targeted discipline): catalog audit's other items (S1 slug-on-rename, S2 sale-price CHECK trap, M5 racy slug pre-check, architectural admin-API vs browser-client refactor) — all separate sessions. Not a 13-item entry (catalog audit follow-up). |
| 2026-06-02 | 143 | Fix: Step 1 classifier-output rules — refined exotic/classic-only pre-select + Mustang year propagation + stale comment | Production code change, merged to main | _(this commit)_ (branch `fix/step1-classifier-output-application-rules`) | Closes **Finding 1 + Finding 2** from `STEP1_SIZE_CLASS_AND_MUSTANG_CLASSIC_AUDIT.md` (e807f543). Both LATENT bugs surfaced (not introduced) by #142. **Q-A.4 LOCKED Option (iii) — refined rule, not replaced:** classifier may pre-select `size_class` ONLY when detecting `'exotic'` or `'classic'` (flow-routing to SpecialtyVehicleBlock); mundane automobile sizes AND non-automobile `specialty_tier` seeds silently dropped from UI state. **Finding 1:** `effectiveSizeClass` non-specialty branch tightened to `manualSizeClass` only; `effectiveSpecialtyTier` to `manualSpecialtyTier` only; manual-clear useEffect guarded to fire only on `classifierSpecialty`. **Finding 2:** `classify(mk, mdl, cat)` extended to `classify(mk, mdl, cat, yr)`; debounce useEffect deps gained `year`. Wrapper + endpoint already accepted year from #142 (Memory #11 verified). Mirrors `vehicle-form-dialog.tsx:227` canonical pattern. **Stale comment** at `:228` replaced; CLAUDE.md Rule 19 "filter to 3" question OUT OF SCOPE. **Tests (+10 → 2864):** NEW `classifier-output-rules.test.tsx` (mundane no-auto-select × 3, exotic/classic DOES pre-select × 3 incl. Ford Mustang 1965 → classic, non-automobile specialty_tier purely manual × 2, year propagation × 2). **Scope:** 1 prod MOD + 1 NEW test. ~50 prod lines net. **Gates:** tsc 0, lint 0err/97warn, 2864/2864, build clean. **STEP1_SIZE_CLASS_AND_MUSTANG_CLASSIC_AUDIT.md** Findings 1+2 RESOLVED. **CLAUDE.md Rule 22** updated. Not a 13-item entry (audit follow-up). |
| 2026-06-02 | 142 | Fix: Vehicle classifier restoration — C1 architectural refactor + S1 error signal + T9 contract test + M1 doc + Mi1+Mi2 cleanup | Production code change, merged to main | _(this commit)_ (branch `fix/vehicle-classifier-restoration-and-hardening`) | Closes **5 findings** (C1 Critical + S1 Significant + M1 Moderate + Mi1 Minor + Mi2 Minor) from `docs/dev/VEHICLE_CLASSIFIER_BEHAVIOR_AUDIT.md` (commit `5e3d3388`). Public booking Step 1 "Add a New Vehicle" path restored for anonymous customers (the audit's production-blocking acute bug — RV + Airstream stuck-spinner). **Q-1 LOCKED → Option B (architectural refactor)** rather than Option A (RLS hotfix migration) — one canonical classifier execution path, no drift. **Q-2 LOCKED → `console.warn`** only for S1 telemetry. **Q-3 LOCKED → Layer 3 specialty_tier is INTENTIONALLY user-input** — operator-facing surfaces now explicit. **C1 architectural refactor (Shape B chosen — full server-side execution via dedicated endpoint):** canonical `resolveVehicleClassification` in `vehicle-categories.ts` retains its parameter-injection signature unchanged — server-side callers (POS, voice agent, customer portal POST/PATCH, `findOrCreateVehicle`) continue calling directly with admin client. Browser-side callers route through new wrapper. NEW `src/app/api/classify-vehicle/route.ts` (public GET endpoint mirroring `/api/vehicle-makes` pattern; admin-client server-side, RLS bypassed). NEW `src/lib/utils/classify-vehicle-client.ts` (thin wrapper `classifyVehicleClient(make, model?, year?)` with defensive `CLASSIFIER_TIMEOUT_MS = 10_000` AbortController so hung fetch can't trap spinner forever — T9 requirement). step-vehicle.tsx + vehicle-form-dialog.tsx drop direct browser-Supabase classifier calls and route through wrapper. **Pre-flight Memory #11 verification:** classifier queries ONLY `vehicle_makes` (Layers 2-5 use hardcoded constants); refactor blast radius bounded. **Why dedicated endpoint not extending `/api/vehicle-makes`:** existing endpoint returns combobox array `{id, name}`, classifier needs make→classification object — different query + response shapes. **S1 error signal hardening:** `VehicleClassification` gains optional `classifier_reason: 'no_match' \| 'query_failed'`. Set on 0-row Layer-1 (`'no_match'`), disambiguation fall-through (`'no_match'`), Supabase `error` non-null response (`'query_failed'` — the RLS-denial-equivalent path that was the audit's smoking gun), and `catch` block throws (`'query_failed'`). Omitted on confident results. Backward compatible. Wrapper emits `console.warn` on `'query_failed'` for operator devtools telemetry. **M1 documentation + microcopy:** explicit comment at Layer-3 site in vehicle-categories.ts documenting intentional manual-pick design. New microcopy under non-automobile specialty tier picker: "Please select the size that matches your {category} — affects service pricing." Framed as required input, NOT detection failure. **Mi1 dead useEffect removed:** the empty-branches useEffect at step-vehicle.tsx removed cleanly. Auto-detect already happens via `effectiveSizeClass`/`effectiveSpecialtyTier` derived values. **Mi2 two-data-path drift closed:** pre-#142 `vehicle_makes` was exposed via TWO browser paths (direct browser-client query AND `/api/vehicle-makes`). Post-#142 ALL browser classifier traffic flows through `/api/classify-vehicle`. **T9 regression-locking contract test:** NEW `classifier-spinner-lifecycle.test.tsx` (6 tests, 5 failure-mode scenarios mirrors #136 T8's structural-guard pattern). Asserts `aria-busy=false` on the height-reserved spinner container within bounded time across: (1) confident success, (2) no_match, (3) query_failed (+ verifies S1 console.warn fires with "query_failed" + the make), (4) HTTP error × 2 sub-tests (non-2xx response + fetch rejection), (5) never-resolve via `CLASSIFIER_TIMEOUT_MS` AbortController (THE production stuck-spinner bug class). If any future refactor removes the timeout or breaks the lifecycle, the test fails. **S1 unit tests extended (+4):** `vehicle-categories.test.ts` pins `classifier_reason` contract — omitted on confident, `'no_match'` on 0-row, `'query_failed'` on throw, `'query_failed'` on `{data:null, error:RLS}` non-throw (the RLS-denial-equivalent path explicitly tested). **Tests (+10 → 2854):** 6 T9 + 4 S1. **Scope:** 3 NEW prod files + 4 MOD prod files + 2 test files. Well under Memory #8 ceiling. No migrations, no new permission keys, no admin/POS/voice-agent/quote-builder edits. **Gates:** tsc 0, lint 0err/97warn (baseline), 2854/2854, build clean (790 pages). **VEHICLE_CLASSIFIER_BEHAVIOR_AUDIT.md** C1+S1+M1+Mi1+Mi2 marked RESOLVED. **CLAUDE.md Rule 22** updated with classifier DB-access-is-server-side rule + Layer-3 manual-pick clarification. **FILE_TREE.md** updated with new helper + endpoint + test. **NOT in scope:** RLS policy changes (Option A path explicitly rejected), model-aware specialty_tier detection (Q-3 confirmed intentional), POS/voice-agent/portal API classifier surfaces (server-side, already correct), other Path B work (the architectural audit Concerns 1+2 are closed via #140/#141). Not a 13-item entry (vehicle-classifier audit follow-up). |
| 2026-06-02 | 141 | Fix (Path B Session 2 / Concern 2): save-to-account transparency for vehicles — silent-save toast | Production code change, merged to main | _(this commit)_ (branch `fix/path-b-session-2-save-to-account-transparency`) | Closes **Concern 2 (Significant)** from the architectural audit (`docs/dev/PUBLIC_BOOKING_ARCHITECTURAL_AUDIT.md`, commit `709befa5`). Path B Session 2 of 3. The audit corrected the operator's stated premise — vehicle data IS persisted via `findOrCreateVehicle` (`vehicle-helpers.ts:188-204`) to the same `vehicles` table the portal reads. **The gap was transparency, not persistence.** **Q-PB-S2 LOCKED in-session as Option 1 (transparency-only)** after pre-flight surfaced two architectural blockers against the prompt's original "toggle + opt-out" shape: (1) `vehicles.customer_id` is `NOT NULL` (DB_SCHEMA.md:3040) and the dedup `idx_vehicles_customer_make_model` UNIQUE index is scoped by customer_id — no "vehicle without account linkage" data path exists; toggle's OFF branch would require a schema migration (out of scope per session prompt's "NO migrations"). Anonymous bookings already create a customer record before linking the vehicle, so there's no "stateless vehicle" path anywhere in the data model. (2) Address-save is already Phase Mobile-1.1 LOCKED with silent-save+diff UX (D18 in audit); adding a parallel "save toggle" to the address step would re-litigate the operator-locked design. **Operator post-flight directive:** match Mobile-1.1's silent-save+toast precedent (Memory #2), reuse `sonner` (already shared), fold both save events into one mount-effect, include "View →" link to `/account/vehicles` for authenticated sessions as customer's agency-preservation escape hatch (review or delete the saved vehicle from portal). **Server:** new `src/lib/utils/vehicle-save-action.ts` — `resolveVehicleSaveAction({customerId, vehicleId, vehicleCreated})` returns `{silently_saved: true, vehicle_id, customer_id}` ONLY when `findOrCreateVehicle` returned `created: true` AND there's customer+vehicle linkage; null otherwise. Sync helper (no DB calls) — unlike `resolveMobileAddressAction` which queries customers + may run an UPDATE, the vehicle work is already done by `findOrCreateVehicle` upstream. `/api/book/route.ts` captures `vehicleResult.created` (the helper already exposes this discriminant; no API surface change) and emits `vehicle_save_action` alongside `mobile_address_action` in the response. **Client:** widened the existing Mobile-1.1 useEffect in `booking-confirmation.tsx` to a three-branch dispatch on `[vehicleSaved, addressSaved]`: (1) **combined** "We've saved your vehicle and address to your account." — ONE toast (not two stacked), with `action: {label: 'View →', onClick: → /account/vehicles}` when `isPortal===true`; (2) **vehicle-only** "We've saved your vehicle to your account." — same action conditional; (3) **address-only** Mobile-1.1's existing wording byte-stable ("We've saved your address to your profile.", no action button) — the test suite asserts this literally so any future widening must be deliberate operator decision, not accidental drift. "View →" gated on `isPortal===true` (anonymous bookings route through `/signin` otherwise). **Why one effect, three branches (not three effects):** Memory #2 — a customer with BOTH events on one booking sees ONE combined acknowledgment, not two stacked toasts. Combined-message branch is anti-regression-locked in tests. **Why no opt-out:** structurally impossible without a schema migration. Q-PB-S2's pragmatic shape: surface the save, give the customer the escape hatch (portal delete affordance via "View →" link). Agency preserved without breaking NOT NULL invariant or dedup index. **Tests (+16 → 2844):** new `vehicle-save-action.test.ts` (8 — null cases × 5: vehicleCreated=false / null customerId / null vehicleId / both ids null / vehicleCreated=false with both ids populated anti-regression; silently_saved case × 3: standard hit / id pass-through / discriminator-always-true contract); new `booking-confirmation-toast.test.tsx` (8 — combined toast WITH View action when isPortal=true / combined toast WITHOUT action when anonymous / vehicle-only with action / vehicle-only without action / vehicle-only when address has diff but not silently_saved / address-only Mobile-1.1 wording byte-lock / neither no-op / neither no-op with non-null but silent_saved=false). **Scope:** 1 NEW helper + 3 prod files MOD (route.ts, booking-wizard.tsx, booking-confirmation.tsx) + 2 NEW test files. ~115 prod lines net. No migrations, no new permission keys. **Gates:** tsc 0, lint 0err/97warn (baseline preserved), 2844/2844, build clean (790 pages). **PUBLIC_BOOKING_ARCHITECTURAL_AUDIT.md** Concern 2 marked RESOLVED with Path B Session 2 marker. **CLAUDE.md Rule 22** updated with silent-save transparency pattern note (paired with Mobile-1.1). **FILE_TREE.md** updated with the new helper + tests. **NOT in scope:** Path B Session 3 (Q-Arch-7 signup acknowledgment + 3 deferred T6.4 minor siblings — name updates, saved payment, notes propagation — bundled by Q-Arch-8 as optional). Concern 1 single-primary constraint locked unchanged per Q-Arch-2. No POS/admin/quote-builder touches. Not a 13-item entry (public-booking audit follow-up arc). |
| 2026-06-02 | 140 | Fix (U-B.5 / Path B Session 1): W5 prereq + W7 addon vehicle_compat enforcement on public booking — two-layer defense | Production code change, merged to main | _(this commit)_ (branch `fix/u-b-5-prereq-and-addon-vehicle-compat-public`) | Closes **W5 (Moderate)** + **W7 (Minor)** from Unit B audit (`docs/dev/PUBLIC_BOOKING_NAV_AND_OPTION_WIRING_AUDIT.md`), reframed by the expanded architectural audit (commit `709befa5`) as Path B Session 1. **Q-Arch-1/2/3 LOCKED upstream:** public booking is intentionally a SUBSET of POS — "self-service customer with a known service; anything requiring staff judgment goes elsewhere"; single-primary stays; W5+W7 are real defects the subset philosophy doesn't excuse. **Q-W5-UX LOCKED in-session Option 1:** show prereq-blocked primary with "Custom Quote" badge + replace configure panel with `<RequestQuoteCard>` (preserves discoverability over filter-out); reuse `request_type='staff_assessed_service'` per operator guidance — no strong reason to split since staff triage path is identical (customer SMS body + staff notification template are reason-agnostic). **Public-booking SUBSET semantics (vs POS):** unlike POS — which gates prereqs by SATISFACTION (history/same-ticket) and offers manager override — public booking checks ONE axis only: prereq vehicle-compatibility. NO manager override on this surface, by design; the customer's escape hatch is the RequestQuoteCard CTA. POS prereq enforcement unchanged (canonical via #121/#122/#130). **W5 two-layer:** Layer 1 — `src/lib/data/booking.ts` extended with `service_prerequisites` embed (PostgREST FK via `service_id`, nested `prerequisite_service` join via `prerequisite_service_id` — mirrors POS shape) in BOTH `getBookableServices` + `getBookableServiceBySlug`; `step-service-select.tsx` computes `serviceRequiresQuote(svc, vehicleCategory)` widening W3's `staff_assessed`-only branch — five prior `selectedService.staff_assessed` gating points (configure-panel substitution, desktop sidebar Continue button + price summary, mobile sticky footer + spacer) now switch on unified `selectedRequiresQuote` derived bool; ServiceCard takes `requiresQuote` prop for the badge. Layer 2 — new `_prereq-enforcement.ts` (`assertPrereqsCompatible` + `prereqIncompatibleErrorMessage` — single-offender names prereq inline, multi-offender comma-joins; both close with "Please request a quote." matching W3's imperative); `/api/book/route.ts` fetches prereq rows via dedicated `service_prerequisites` query after staff_assessed + before price validation. Empty/null `vehicle_compatibility` on prereq = compatible-with-all (implicit default matches `route.ts:343`). **W7 two-layer:** Layer 1 — booking.ts addon sub-select gains `vehicle_compatibility`; `step-service-select.tsx` filters `addonSuggestions` by `addonAllowedForVehicle(...)` BEFORE rendering (filter-out pattern, NOT keep-visible-suppress — addons are optional so there's no value in showing a "you can't add this" affordance). Layer 2 — new `_addon-vehicle-compat.ts` (`checkAddonsVehicleCompatible` + `addonVehicleIncompatibleErrorMessage` — first-fail by array order mirroring `_mobile-eligibility.ts`; closes with "Please remove it and try again." — DIFFERENT closer than W5 because addons are resolvable client-side). **Shared addon row fetch — now serves FOUR consumers:** `addonServiceRows` (introduced U-B.3, extended U-B.4) now also selects `vehicle_compatibility`. One round-trip serves W2 (mobile_eligible) + W3 (staff_assessed) + W4 (is_taxable persistence) + W7 (vehicle_compatibility). Zero extra queries for no-addon case. **Two distinct error-closer wordings (locked):** W5's "Please request a quote." matches W3 byte-for-byte (customer needs staff escalation); W7's "Please remove it and try again." is the actionable closer (customer can resolve by unchecking). Test files lock both wordings. **Tests (+30 → 2828):** new `prereq-enforcement.test.ts` (16 — no prereqs / null category / all-compatible / empty-compat / null-compat / single offender / mixed list / all-offenders / null prereq_service join / standard↔automobile mapping symmetry × 4 non-automobile categories / wording single + multi + defensive empty + imperative-closer-match-W3); new `addon-vehicle-compat.test.ts` (14 — empty addons / null category / empty-compat / null-compat / single category match / multi-category match / mixed compatible / first-fail by order / single incompatible / all 4 non-automobile categories anti-regression / wording exact + punctuation + closer-imperative-not-quote-request). Total within expected +18-25 range — slightly over because both helpers got vocabulary anti-regression coverage (the standard↔automobile mapping is easy to invert). **Scope:** 4 prod files (2 NEW helpers + booking.ts data layer + route.ts wire-up + step-service-select.tsx client wire-up) + 2 NEW test files. ~250 prod lines net. No migrations, no new permission keys. **Gates:** tsc 0, lint 0err/97warn (was 98; net -1 — unifying 5 `selectedService.staff_assessed` checks into one `selectedRequiresQuote` derived bool eliminated a redundant raw read), 2828/2828, build clean (790 pages). **CLAUDE.md Rule 22 updated** with W5+W7 two-layer enforcement (family of five now: W1 classification + W2 mobile_eligible + W3 staff_assessed + W5 prereq vehicle_compat + W7 addon vehicle_compat; W4 is_taxable persistence is sibling but separate axis). **PUBLIC_BOOKING_NAV_AND_OPTION_WIRING_AUDIT.md** W5 + W7 marked RESOLVED. **PUBLIC_BOOKING_ARCHITECTURAL_AUDIT.md** Path B Session 1 marked complete. **FILE_TREE.md** updated with new helpers + tests. **NOT in scope:** Concern 2 (save-to-account transparency — Path B Session 2 is the explicit follow-up), POS / quote-builder prereq enforcement (already canonical via #121/#122/#130), multi-primary work (Path B locked — Q-Arch-2 single-primary stays), F2 RV/Boat/Aircraft (separate session; uses staff_assessed pattern), pricing_model mutability (Q-D still open). Not a 13-item entry (public-booking audit follow-up arc). |
| 2026-06-02 | 139 | Fix: Quote-request SMS bundle — Pattern B (per-`request_type` slug lookup) + footgun hardening + universal customer template + sendSms self-send chokepoint | Production code change + 1 migration, merged to main | _(this commit)_ (branch `fix/quote-request-sms-bundle`) | Closes the four-concern bundle from `docs/dev/QUOTE_REQUEST_SMS_AUDIT.md` (Targeted audit run between #138 and this session). **Concern 1 (Pattern B):** new migration seeds `booking_staff_notify_quote_request` (staff slug for `staff_assessed_service` `request_type`, same two staff phones as `booking_staff_notify_specialty`); route's `STAFF_SLUG_BY_REQUEST_TYPE` map owns per-variant slug routing. **Concern 2 (footgun):** both `recipients = [biz.phone]` fallback sites in `route.ts` replaced with `[]` + `console.warn` describing which slug had empty `recipient_phones` — closes audit's S2 (admin clears all phones) and S3 (fresh DB / forgotten seed) trigger scenarios. **Concern 3 (universal customer SMS):** new `quote_request_received_customer` template (category `quote`, recipient_type `customer`, required `first_name` + `request_subject`, optional auto-injected `business_name` + `business_phone`). Both variants dispatch it after the audit_log row in a separate try/catch from the staff SMS. `request_subject` resolves per-variant: `staff_assessed_service → service_name`, `specialty_vehicle → "specialty vehicle"`. **EXPLICIT BEHAVIOR CHANGE** for `specialty_vehicle`: pre-#139 sent NO customer SMS, post-#139 it WILL. Operator approved per QUOTE_REQUEST_SMS_AUDIT Target E. **Concern 4 (sendSms self-send chokepoint):** added defense-in-depth guard in `src/lib/utils/sms.ts` BEFORE the Twilio fetch — when `normalize(to) === normalize(TWILIO_PHONE_NUMBER)` returns `{ success: false, error: 'Self-send blocked: ...' }` + `console.warn`. Skipped when env unset (test envs). Backward compatible — failure shape mirrors existing `Invalid phone number format` rejection; no `sms_delivery_log` row written for blocked self-sends. **Tests (+31 → 2798):** new `sms-self-send.test.ts` (10 — chokepoint contract: E.164/non-E.164/10-digit/11-digit self-match blocking, malformed-env defense, non-match passthrough, missing-env passthrough, empty-env passthrough, no delivery log on block, chained order with normalization); new `specialty-callback/__tests__/route.test.ts` (21 — Concern 1 slug-per-variant, Concern 1 recipient-phones routing without biz.phone fallback, Concern 2 empty/null recipient_phones drops + warn, S2/S3 trigger scenarios, biz.phone never-fallback regression guard, Concern 3 service_name → request_subject, "specialty vehicle" → request_subject behavior-change verification, customer phone routing, first_name derivation from whitespace tokens, customer-template-inactive skip, staff-send-failure does not block customer SMS, response contract 400/200 shapes). Total range expected: +18-25, came in at +31 because Concern 2's three S2/S3 scenarios + Concern 3's seven behavioral pins are all narrow contract assertions — no padding. **Scope:** 1 new migration + 1 sms-contracts source edit (+ regen of palette.ts + generated-contracts.ts) + 1 route refactor + 1 sms.ts addition + 2 new test files. Updated CHANGELOG, CLAUDE.md (Rule 9 chokepoint note + Rule 22 W3 update marking the follow-on closed), QUOTE_REQUEST_SMS_AUDIT.md (resolution mapping table), FILE_TREE.md. **Gates:** tsc 0, lint 0err/98warn (baseline preserved), 2798/2798, build clean (790 pages). **Not in scope:** F2 RV/Boat/Aircraft suppression (templates are forward-compatible — adding the new request_type is a one-line addition to `STAFF_SLUG_BY_REQUEST_TYPE` + new seed migration), U-B.5 prereq enforcement, Q-D pricing_model doc work, deploy script `.next` cleanup. Not a 13-item entry (public-booking audit follow-up arc). |
| 2026-06-01 | 138 | Fix (U-B.4 / W4): services `is_taxable` on booking deposit — line-item persistence mirror (audit + fix, Option A) | Production code change, merged to main | _(this commit)_ (branch `fix/u-b-4-is-taxable-mirror-products-pattern`) | Closes **W4 (Moderate)** from Unit B audit. **Option 1 structure (audit + fix in one session):** Phase 1 audit + stop-and-report on Q-C-1 Option A vs Option B fork; operator locked Option A. **Phase 1 findings:** 3 hardcoded-false sites in `/api/book/route.ts` (primary `:603`, addon `:623`, mobile fee `:648` — only the mobile fee has prior CDTFA Pub 100 justification). Effect was PERSISTENCE only — admin Transaction Detail shows `---` for taxable items because the deposit's persisted `is_taxable: false` mistypes the column. POS finalization unaffected — `/api/pos/appointments/[id]/load` already looks up live `services.is_taxable` at drain time, so tax collection at completion has always been correct. **Products' pattern has TWO shapes** — Pattern A (POS-side line-item persistence via `calculateItemTax` + `is_taxable` flag wiring) is the reusable subset for booking deposit; Pattern B (full e-commerce checkout in `create-payment-intent`) applies to immediate-pay-in-full, not the booking deposit's partial-prepayment model. 14 prod files touch `is_taxable` significantly — full Pattern B mirror would exceed Memory #8; Option A mirror was the scoped subset. **Q-C-1 LOCKED Option A:** per-row `is_taxable` reflects underlying `services.is_taxable`; `tax_amount` stays `0` on items + the deposit transaction because no tax is collected at deposit time (CA CDTFA Pub 100 — tax tied to service completion). **Phase 2 fix (1 prod file, ~10 lines net):** `/api/book/route.ts` primary line item → `is_taxable: serviceRow.is_taxable as boolean`; addon line items → `is_taxable: addonMetaById.get(addon.service_id)?.is_taxable ?? false` via a `Map` lookup from `addonServiceRows` (the addon-fetch from U-B.3 extended to include `is_taxable` — one query feeds W2/W3/W4 now); mobile fee at `:648` stays `is_taxable: false` with expanded in-source comment flagging it as the ONE legitimate hardcoded-false post-W4 (CDTFA Pub 100). Defensive `?? false` default acceptable per operator: POS re-reads canonical at finalization. **Tests (+8 → 2767):** new `deposit-tax-persistence.test.ts` (8) mirrors `modifier-persistence.test.ts` harness style (Stripe + combo-resolver + card-detail mocked; supabase builder extended for `transactions` + `transaction_items` + `payments` + `services.in()`). Coverage: primary taxable + primary non-taxable (anti-overshoot guard); mixed-flag addon case (one taxable + one non-taxable proves per-row lookup); missing addon row defensive default; CDTFA mobile-fee always-false regression pin (anti-overshoot against blanket "use serviceRow flag everywhere"); `tax_amount=0` invariant on items + deposit transaction (anti-overshoot against future Option-B drift); no-deposit path writes zero `transaction_items` (boundary pin). Scope: 1 prod file + 1 new test file (well under ≤4 budget). No migrations / new permission keys / no client UI changes. **Gates:** tsc 0, lint 0err/98warn (unchanged from #137 baseline), 2767/2767, build clean. **CLAUDE.md Rule 22 updated** with W4 services-deposit-persistence note — family of four now (W1 classification + W2 mobile_eligible + W3 staff_assessed + W4 is_taxable persistence). **PUBLIC_BOOKING_NAV_AND_OPTION_WIRING_AUDIT.md** W4 marked RESOLVED with Option A annotation. **FILE_TREE.md** updated with new test file. **NOT in scope:** Option B path (Step 4 tax UI, payment intent tax computation, receipt/SMS tax line, admin tax-rate setting) — separate session if operator wants customer-visible tax UX. `pos/utils/tax.ts` "services are tax-free" stale comment — doc-only update deferred. U-B.5 prereq enforcement, W5/W7, Concern C — separate fix-arc waves. Not a 13-item entry (public-booking audit follow-up arc). |
| 2026-06-01 | 137 | Fix (U-B.3 / W3): `staff_assessed` → "Request a Quote" CTA on public booking Step 2 — two-layer defense + shared form base | Production code change, merged to main | _(this commit)_ (branch `fix/u-b-3-staff-assessed-request-quote-cta`) | Closes **W3 (Significant)** from Unit B audit. **Q-B-1 LOCKED:** inline callback form (mirror `<SpecialtyVehicleBlock>` shape) + generalized `/api/public/specialty-callback` endpoint with `request_type` discriminator. Verified cleanly extensible pre-implementation. **Two-layer defense (mirrors W1/W2):** Layer 1 — `step-service-select.tsx` ServiceCard shows "Custom Quote" badge in place of price label when `staff_assessed=true`; when SELECTED, the configure panel + price summary + Continue button (desktop sidebar + mobile sticky footer + mobile spacer) are all suppressed, replaced by `<RequestQuoteCard>` inline. Service stays VISIBLE; deep-link via `?service=slug` continues to resolve (data layer NOT filtered). Layer 2 — new `_staff-assessed.ts` helper invoked in `/api/book/route.ts` after classification check + before price validation; returns 400 + `"{name} requires a custom quote and cannot be booked directly online. Please request a quote."` for primary-or-addon `staff_assessed=true`. Mirrors `_classification.ts` + `_mobile-eligibility.ts` byte-symmetrically (primary precedence; addon check by array order). **Addon-fetch refactor:** the existing W2 mobile_eligibility check inside-conditional addon-fetch moved UPWARD so both W2 + W3 reuse one `addonServiceRows` query (selects `mobile_eligible` AND `staff_assessed`). **Endpoint generalization:** `request_type: 'specialty_vehicle' \| 'staff_assessed_service'` discriminator with `'specialty_vehicle'` as the BC default. Per-type audit-event labels (`specialty_callback_requested` / `staff_assessed_quote_requested`), per-type staff SMS prose. `<SpecialtyVehicleBlock>` updated to pass `request_type` explicitly. SMS template caveat — existing `booking_staff_notify_specialty` slug's chip contract is vehicle-specific; `staff_assessed_service` uses the endpoint's existing raw-prose fallback path (operator-customizable template deferred to follow-up migration). **Mid-session course correction (operator's Memory #2/#29 reuse principle):** CC initially built `<RequestQuoteCard>` as near-duplicate of `<SpecialtyVehicleBlock>`; operator interjected "extract shared base, NOT duplicate." Extracted new `<QuoteRequestForm>` shared base (~210 lines) owning form state, phone validation, submit handler, success state, Call CTA, divider, four input fields. Both `<SpecialtyVehicleBlock>` (refactored, ~100 lines now) and `<RequestQuoteCard>` (~95 lines) are thin presentational wrappers — composition over duplication. `<SpecialtyVehicleBlock>`'s external API preserved byte-stable. Side effect: phone CTA color migrated from hardcoded `bg-lime-600` to `bg-accent-brand` design tokens. **Tests (+15 → 2759):** new `staff-assessed.test.ts` (11 — primary precedence, addon array order, wording lock with punctuation/ampersand, empty-array contract); `step-service-select-render.test.tsx` (+4 — Custom Quote badge present/absent, selected-card suppresses Continue + renders RequestQuoteCard markers). Scope: 7 prod files (1 over ≤5 budget — the shared-base refactor IS the minimum-scope fix satisfying Memory #2/29). ~290 prod lines net. No migrations, no new permission keys, no admin/POS/quote-builder edits. **Gates:** tsc 0, lint 0err/98warn (was 99 baseline — **net -1**; two ESLint disable comments added at JSX prop pass-throughs where `<QuoteRequestForm>` wraps `formatPhone`/`phoneToE164` internally per `docs/dev/PHONE_LINT.md`), 2759/2759, build clean. **CLAUDE.md Rule 22 updated** with staff_assessed two-layer enforcement (family of three: W1/W2/W3). **PUBLIC_BOOKING_NAV_AND_OPTION_WIRING_AUDIT.md** W3 marked RESOLVED. **FILE_TREE.md** updated with `_staff-assessed.ts`, `quote-request-form.tsx`, `request-quote-card.tsx`, `staff-assessed.test.ts` + SpecialtyVehicleBlock refactor note. **NOT in scope:** F2 RV/Boat/Aircraft suppression (the RequestQuoteCard generic name is the seed for its future reuse), U-B.4 `is_taxable` (next session), U-B.5 prereq enforcement, W4/W5/W7/Concern C, admin/POS/quote-builder `staff_assessed` (operator-trust boundary), migration for `booking_staff_notify_quote_request` SMS sub-slug. **Session-number label note:** prompt was "U-B.3" but #136 reused the label for the vehicle-form arc (labeling drift between #135's vehicle-forms audit and the Unit B audit fix arc); THIS session is the canonical Unit B W3 fix. Not a 13-item entry (public-booking audit follow-up arc). |
| 2026-05-31 | 136 | Fix (U-B.3): vehicle-form behavior coherent arc — reset semantics + visual stability + contract alignment + T8 regression-locking test | Production code fix bundle (16 defects closed), merged to main | _(this commit)_ (branch `fix/u-b-3-vehicle-forms-behavior-coherent-arc`) | ONE coherent session per #135 audit's "splitting NOT recommended — defects entangled." Closes 16 of 24 defects: B1 (reset semantics both forms) + B2 (classifier spinner row height-reserved, operator finding #2) + B2-P (portal advisory banner reserved) + B3 (PATCH `?? undefined` collapse fixed → null now writes NULL, closes silent DB inconsistency after specialty→automobile category change) + B4 (PATCH null-handling symmetry) + B5/B5-P (classifier race-cancellation via `classifyRequestIdRef` in both forms) + B6 (FormField opt-in `reserveErrorSpace` prop — zero blast radius for the other ~54 consumers) + B7/B9 (validation timing standardized — both forms now real-time + on-touched) + B10 (portal Year required indicator) + B11 (vin/license_plate/notes surfaced on portal, Q3) + B15 (portal required-field client validators via RHF per-field `validate:` + server defense-in-depth check) + B22 (saved-vehicle card uses `getSpecialtyTierLabel()`) + B30 (color case timing converged — identity-on-display + titleCase-on-submit) + B31 (year max aligned to 2099 across both schemas). **T8 contract test** (`vehicle-forms-reset-contract.test.tsx`, new, 6 tests) is the durable star deliverable — mirrors Track B's #120 structural-guard pattern; runs SAME reset-semantics assertions against BOTH forms with same fixture data so future refactors that partially fix one but break the other fail immediately. Scope: 6 prod files modified + 4 new test files (~310 prod lines net; 1 file over ≤5 budget — `form-field.tsx`'s opt-in prop IS the minimum-scope B6 fix). Tests +21 → 2744. **8 informational/a11y/mobile defects deferred** (B12/B17/B19/B24/B25/B26/B27/B28/B29) — separate a11y-polish pass per audit recommendation. No POS / admin / quote-builder form edits (NO-UNIFICATION #128 honored). No migrations / new permission keys. Classifier (#131 Layer 2) untouched. Gates: tsc 0, lint 0err/97warn (none new), 2744/2744, build clean. Not a 13-item entry (vehicle-form arc). |
| 2026-05-30 | 134 | Fix (U-B.2): classification filter on public booking Step 2 — client + server defense in depth | Production code change, merged to main | _(this commit)_ (branch `fix/u-b-2-classification-filter-public-booking-step2`) | Closes **W1 (Significant)** from Unit B audit. **Q-A LOCKED:** only `classification IN ('primary', 'both')` may appear as standalone primary on Step 2; `addon_only` services must never appear as standalone options (they stay valid as add-ons — that's their whole point). **Two-layer defense-in-depth (mirrors W2's pattern from #133):** Layer 1 — data layer (`src/lib/data/booking.ts`): both `getBookableServices` (Step 2 list) and `getBookableServiceBySlug` (deep-link) gained `.in('classification', PRIMARY_BOOKABLE_CLASSIFICATIONS)` on the Supabase query — operator-preferred source-query location, smaller payload, no leak risk. Addon sub-select intentionally unfiltered. Layer 2 — server validation (`src/app/api/book/route.ts`): `checkPrimaryClassification` invoked between service-not-found check and price validation; returns 400 with `"{name} cannot be booked as a standalone service. Please select a different service."` if addon_only. Catches tampered/replayed POSTs, deep-links bypassing the wizard, AND operator misconfiguration. **Helper extraction (`src/app/api/book/_classification.ts`, new — 87 lines):** mirrors `_mobile-eligibility.ts` byte-symmetrically. Exports `isPrimaryBookable(c)` predicate (single source of truth), `PRIMARY_BOOKABLE_CLASSIFICATIONS = ['primary', 'both']` constant (imported by `booking.ts` so client + server share one rule — drift guard), `checkPrimaryClassification(primary)` runtime check, `primaryClassificationErrorMessage(name)` wording lock. `'both' === primary on Step 2` is explicit in the predicate, not magic. Underscore prefix excludes from Next.js route resolution; helper unit-testable without Supabase/Stripe/Twilio. **Cross-package import `@/lib → @/app`** unusual but justified — existing precedent in codebase; alternative third-file extraction would be over-engineering for a 3-line rule. **Tests (+11 → 2723):** new `classification.test.ts` (11 — predicate × 3 classifications, constant drift guard × 3, check × 3 outcomes, message lock × 2); `modifier-persistence.test.ts` fixture +1 line (`classification: 'primary'` + `mobile_eligible: true` for W2 symmetry). Scope: 3 prod files + 1 new prod helper + 1 new test + 1 fixture line; ~120 prod lines net. Gates: tsc 0, lint 0err/97warn (none new in touched files), 2723/2723, build clean + 790/790 static pages. **CLAUDE.md Rule 22 updated** with the two-layer classification enforcement note. **NOT in scope** per audit's locked fix arc: W3 (Q-B), W4 (Q-C), W5/W7 (U-B.5), Concern C pricing_model mutability (Q-D), admin/POS/quote-builder classification handling (#121's `useValidatedServiceAdd`). Not a 13-item entry (public-booking audit follow-up arc). |
| 2026-05-30 | 133 | Fix (U-B.1 + E5): Step 2 Back button + server mobile_eligible defense + special_requirements display + pushState navigation | Production code change, merged to main | _(this commit)_ (branch `fix/u-b-1-step2-back-server-mobile-special-pushstate`) | Bundle of four no-decision-needed findings from the Unit B audit (merge `1eeade10`, `docs/dev/PUBLIC_BOOKING_NAV_AND_OPTION_WIRING_AUDIT.md`): **N1** (Step 2 explicit Back button mirroring `step-schedule.tsx:285`), **W2** (server `mobile_eligible` defense via extracted pure helper `_mobile-eligibility.ts` — mirrors `_pricing.ts` pattern; route checks primary + batch-fetches addons), **W6** (`services.special_requirements` surfaced on Step 2 service card + Step 4 order summary; new `serviceSpecialRequirements` prop on `StepConfirmBook`), **E5** (`updateUrl` switched from `replaceState` → `pushState` for step transitions; `isInitial` flag preserves replaceState for mount + confirmation cleanup; new `popstate` listener rehydrates wizard via refactored `getInitialState(paramsArg?)` reading fresh `window.location.search`). N1's wiring suppresses Back during edit-from-Step-4 mode to avoid stacking two backward affordances (existing "Back to Booking" remains the only path during edit). Step 4 edit-from-summary pencils already cover Steps 1/2/3 — no new pencil needed. `handleCategoryChange` is dead code (defined, never called) and left as-is per Memory #19 scope discipline. **Tests (+15 → 2712):** new `_mobile-eligibility.test.ts` (7 — primary ineligible / addon ineligible / primary precedence / first-by-array-order / message wording lock); new `step-service-select-render.test.tsx` (6 — N1: renders when `onBack` provided, hidden when omitted, click invokes handler; W6: renders when set, hidden when null/empty). Step 4 W6 + E5 popstate end-to-end tests skipped — implementation mirrors tested paths and `<StepConfirmBook>` / `<BookingWizard>` mounts would require disproportionate mocking. Scope: 4 prod files modified + 1 new prod helper + 2 new test files; ~165 prod lines net (well under 250 target). Gates: tsc 0, lint 0err/97warn (none new in touched files), 2712/2712, build clean + 790/790 static pages. **NOT in scope** per audit's locked fix arc: W1 (gates on Q-A canonical-rule confirmation), W3 (gates on Q-B), W4 (gates on Q-C), W5/W7 (separate session U-B.5), Concern C pricing_model mutability (gates on Q-D). Not a 13-item entry (public-booking audit follow-up arc). |
| 2026-05-30 | 132 | Fix (Unit A): year text input supersedes #131 dropdown+Other + model case preservation | Production UI correction, merged to main | _(this commit)_ (branch `fix/unit-a-year-text-input-and-model-case`) | Two small contained changes, same files as #131. **Issue 2 (DESIGN REVERSAL):** operator reconsidered #131's dropdown+Other... pattern ~3 hours after ship — replaced with a single 4-digit text input on both customer-facing forms. Rule: `/^(19\|20)\d{2}$/` (1900–2099); the "19/20" prefix IS the range constraint. `step-vehicle.tsx` year UI collapsed from `Select+yearOtherMode+customYearInput` state machine (~70 lines) to one `<Input type="text" inputMode="numeric" maxLength={4}>` with onChange digit-strip + validate + onBlur final-error; `Select` import removed (no remaining Selects in file). `vehicle-form-dialog.tsx` collapsed dual-mode to one Input wired through RHF `register('year', { setValueAs, validate })`. `getCustomerVehicleYearOptions()` + `CUSTOMER_VEHICLE_YEAR_INPUT_MIN/_MAX` constants (all #131) removed as dead code; `validateCustomerVehicleYear()` repurposed with new rule (export name byte-stable so the design pivot doesn't ripple). Operator-facing surfaces (POS, admin) untouched per scope — `getVehicleYearOptions()` still 1980→currentYear+2 (Memory #19 DRY-within-trust-boundary preserved). **Issue 4 (model case):** root cause traced before fix-shape commitment (Memory #11). Found at TWO locations, NO CSS: `step-vehicle.tsx:435` called `titleCaseField()` on EVERY keystroke (display bug — "CBR600RR" → "Cbr600rr"); `vehicle-form-dialog.tsx:211` called it on SAVE payload (persist bug — display preserved via RHF `register` but the submitted body was mangled). Fixes: identity transform on step-vehicle's onChange (`setModel(e.target.value)`); trim-only on dialog's save (`(data.model ?? '').trim()`). Color's `titleCaseField` stays per scope ("Make has a separate case story from model — DO NOT touch"). Server-side confirmed clean (no model lowercasing in `/api/customer/vehicles` or `/api/book`). **Tests (+10 → 2697):** vehicle-year-helpers.test.ts rewritten (17 tests — new validator rule across accepts/empty/non-4-digit/non-19-20-prefix branches + operator-range-NOT-narrowed regression); step-vehicle.test.tsx +8 (model display + submit transforms locked via predicate-mirror, including a "legacy titleCaseField WOULD mangle these" regression that documents the bug shape if anyone re-introduces it). Scope: 3 prod files + 2 test files updated; ~42 prod lines net (heavy removal — dropdown state machine collapsed). No POS / admin / quote-builder edits. No migrations / new permission keys. Supersedes #131 Issue 2 within 24 hours. Layer 2 classifier hardening from #131 untouched (orthogonal). Gates: tsc 0, lint 0err/97warn (none new in touched files), 2697/2697, build clean. Not a 13-item entry (vehicle-form arc). |
| 2026-05-30 | 131 | Fix: Layer 2 classifier hardening (corrects #129 Y-1 insufficiency) + year dropdown 2028→2000 with Other write-in | Production code fix, merged to main | _(this commit)_ (branch `fix/classifier-layer2-hardening-and-year-dropdown`) | Two related changes bundled (same files, same review unit). **Issue 1 — F1 returned with a different trigger after #129 deployed.** Operator confirmed: typing a MODEL after picking a non-automobile category still silently reset the form to automobile. #129 C1's `mdl.trim()` gate only covered ONE of the three silent-default paths in `resolveVehicleClassification` (the dual-category empty-model case at `:302-305`); the OTHER two paths — 0-row `vehicle_makes` lookup (`:691`, the common one for niche RV/boat makes due to data drift) and DB error (`:712-714`) — still fired override when model was typed. **Layer 2 structural fix: `category_confident: boolean` flag added to `VehicleClassification` (additive, no breaking change).** Set `true` only on positive evidence (single-row match OR dual-category disambiguation via known model keyword); `false` on every fall-through. `disambiguateCategory` refactored to return `{ category, matched }` so its no-keyword fallback is distinguished from a real match. Surfaced a 4TH silent path during the work (dual-category + model that matches no category keyword) — now also dev-warns + flagged false. **`step-vehicle.tsx`:** setCategory gate switched from `mdl.trim()` to `result.category_confident`; SURFACED a SECOND silent-override #129 missed in `buildSelection()` (`effectiveCat = classification?.vehicle_category ?? category` — submitted automobile even when setCategory gate kept RV on screen), now also gated on `category_confident === true`. **`vehicle-helpers.ts` + POS create route:** override-mismatch warns suppressed on `!category_confident` (was producing false-positive log noise for every Winnebago / niche-make case). **`vehicle-form-dialog.tsx`:** TypeScript-compat with new field; advisory was already correctly scoped to `size_class` independent of category confidence (Session 29 anti-gaming). **Issue 2 — Customer-facing year dropdown 2028→2000 (29 entries) + "Other..." write-in (1900–2028).** New helpers `getCustomerVehicleYearOptions()` + `validateCustomerVehicleYear()` + constants exported from `vehicle-make-combobox.tsx`. Public booking step-vehicle gains the Other-mode state + 4-digit text input + Back-to-list button (mirroring the portal dialog's pattern). Portal dialog dropdown switched to new helper; RHF `validate:` block added to year `register()` to enforce 1900–2028 client-side. **Operator-facing surfaces (POS, admin) NOT touched** per session's explicit exclusion — separate helper keeps the narrow customer-facing range from bleeding into operator forms. **Tests (+26 → 2687):** vehicle-categories.test.ts +12 (Layer 2 confident-flag suite covering all 4 silent paths + 3 confident paths + EXOTIC_MAKES regression preservation); step-vehicle.test.tsx rewritten (8 tests — two predicate mirrors locking BOTH the setCategory gate AND the buildSelection gate, with explicit operator-regression case); new vehicle-year-helpers.test.ts (11 — dropdown shape + validator bounds + operator-range-NOT-narrowed regression); pre-existing shape test updated for new field. Scope: 6 prod files modified + 1 new test file + 2 test files updated; ~245 prod lines net (within ≤250 target per Memory #8). No migrations / new permission keys / POS / admin / quote-builder form edits. **Supersedes #129 C1's insufficient Y-1 fix** — F1 now closed at the architectural level. Gates: tsc 0, lint 0err/97warn (none new in touched files), 2687/2687, build clean. Not a 13-item entry (vehicle-form arc). |
| 2026-05-30 | 130 | Fix: V1+V2 prereq vehicle-compat block (Option A) — server flag + client error + selectPricingTierForVehicle clarification | Production code fix, merged to main | _(this commit)_ (branch `fix/v1-v2-prereq-vehicle-compat-block`) | Closes **V1 (Critical) + V2 (Significant)** from `docs/dev/VEHICLE_TAXONOMY_AUDIT.md` (#126). Operator's daily blocker: tapping a vehicle-type-incompatible prereq in the prerequisite dialog (e.g. RV-only "RV Interior Clean" on a sedan ticket) used to fail with the misleading "no price configured for this vehicle size" toast — a generic pricing-config error hiding the real cause (the prereq isn't available for the ticket's vehicle category at all). **Option A locked (transparency over filtering):** the dialog continues to render every configured prereq, incompatible ones are NOT filtered out — they render dimmed with a `<Ban>` icon + `(For RV vehicles only)` hint, and clicking surfaces a precise category-specific toast (and keeps the dialog open so the operator can pick a compatible prereq or use Manager Override). **Defense in depth — server AND client ship:** (V2, `check-prerequisites/route.ts`) each prereq carries `is_compatible_with_vehicle: boolean` + `compatible_categories: VehicleCategory[]` computed from the prereq's `vehicle_compatibility` JSONB against the ticket vehicle's `vehicle_category` (mapped via `categoryToCompatibilityKey` — bridges "standard ↔ automobile"); response top-level gains `ticket_vehicle_category`. Incompatible prereqs are NOT filtered out. (V1, `use-validated-service-add.tsx`) `handleAddPrerequisite` gates on the server flag BEFORE `selectPricingTierForVehicle`; precise toast: *"RV Interior Clean is only available for RV vehicles; this ticket's vehicle is an Automobile. Use Manager Override to add Ozone Odor Treatment without this prerequisite."* Multi-category lists read "RV or Boat vehicles"; preposition is "an Automobile" / "a Motorcycle". Manager Override unaffected (commits the DEPENDENT service with override note, never invokes prereq auto-add). **`selectPricingTierForVehicle` clarification:** doc comment now explicitly says cross-category awareness is an UPSTREAM concern via `vehicle_compatibility`; no return-type refactor (would touch all 6 Rule 22 call sites — over budget; caller misinterpretation is fixed at the call site). **Tests (+12 → 2649):** new `check-prerequisites/__tests__/route.test.ts` (8: all 5 categories, standard↔automobile bridge, null vehicle, empty/missing compat list, Option-A no-filter contract); `use-validated-service-add.test.tsx` (+5: incompatible block, Override bypass, non-automobile preposition, multi-category "or" allowed list, compatible regression); `picker-engine.test.ts` (+1: cross-category null contract across all 5 size_classes). **CLAUDE.md Rule 22** updated with the vehicle-compat axis note. Gates: tsc 0, lint 0err/97warn (none new in touched files), 2649/2649, build clean. **Session-number collision** (Memory #3): prompt said #127; #127+#128 taken by parallel audits → I claimed #129 → another parallel session merged its #129 first → I renumbered to #130 across CHANGELOG / ROADMAP / commit message / in-source comments. Scope: 4 prod files modified + 1 new test file + 2 extended. No migrations, no new permission keys. Not a 13-item entry (vehicle-taxonomy fix arc). |
| 2026-05-30 | 129 | Fix: vehicle classifier corrective (C1) + dev-warn ride-along (Q7) + portal classifier opt-in (C3) | Production code fix, merged to main | _(this commit)_ (branch `fix/vehicle-classifier-corrective-and-portal-opt-in`) | Three related changes bundled per the #128 NO-UNIFICATION verdict (four context-driven vehicle-form patterns are intentional). **C1 (`step-vehicle.tsx` +5 net):** `classify()` refuses category auto-override when `!mdl.trim()` — the resolver silently defaults to automobile in three paths (`vehicle-categories.ts:302-305/691/712-714`), which was overwriting customer's explicit RV/motorcycle/boat/aircraft picks (PUBLIC_BOOKING_FLOW_AUDIT.md F1, operator-confirmed). Classifier still runs; the Honda-motorcycle correction still fires when model is typed. Y-1 shape per audit. **Q7 (`vehicle-categories.ts` +12):** dev-only `console.warn` (NODE_ENV-gated) at all three silent-default paths — 0-row lookup, dual-category empty model (existing warn now env-gated), and DB error catch. F4 data-drift telemetry per PUBLIC_BOOKING_FLOW_AUDIT.md + S9 of VEHICLE_FORM_UNIFICATION_AUDIT.md. **C3 (`account/vehicle-form-dialog.tsx` +66; `api/customer/vehicles/[id]/route.ts` +23):** portal vehicle dialog mounts an opt-in debounced classifier (requires both make AND model — same C1 gate) and renders an inline amber advisory ("Specialty / Exotic vehicle detected ...") when classifier resolves to exotic/classic; PATCH route gains classifier parity with POST (mirror — closes the edit-time re-classification gap). Server remains the authoritative writer; Session 29 anti-gaming preserved (customer-facing dropdown still 3-value, classifier wins for specialty detection). Closes Q4 from VEHICLE_FORM_UNIFICATION_AUDIT.md + Q3 from PUBLIC_BOOKING_FLOW_AUDIT.md. **Tests +12 → 2647:** `vehicle-categories.test.ts` +9 (Q7 warn per path + production suppression + C1 baseline resolver behavior); new `step-vehicle.test.tsx` +5 (C1 override-gate predicate mirror across 4 non-auto categories + Honda-motorcycle preservation). Render-level integration tests deferred — predicate mirror + resolver tests pin both contract ends; render harness for step-vehicle/dialog would require disproportionate `createClient`/`fetch`/debounce mocking with no additional evidence over the unit-level locks. Scope: 4 prod files modified + 1 new test (~115 prod lines net; well under 200-line target per Memory #8). No reducer / migration / schema / permission change. No POS / admin / V1-prereq overlap (Session #127 isolation respected). Gates: tsc 0, lint 0err/97warn (none new in touched files), 2647/2647, build clean. Not a 13-item entry (vehicle-form arc). |
| 2026-05-29 | 124 | Fix: Admin Services Edit add-on dropdown + parent-self exclusion + named UNIQUE-constraint messages (sibling to #123) | Production code fix, merged to main | _(this commit)_ (branch `fix/admin-services-edit-addon-dropdown-and-parent-self-exclusion`) | Closes all three open questions from #123's audit (`docs/dev/PREREQ_SERVICE_DROPDOWN_AUDIT.md`) — Target 4 sibling, pre-existing parent-self latent, UNIQUE-constraint specificity — bundled per operator lock. **(1) Add-on edit dropdown un-disabled (mirrors #123 byte-for-byte):** removed `disabled={!!editingAddon}` at `page.tsx:1714`; new `src/app/admin/catalog/services/[id]/addon-helpers.ts` with `getEditAddonOptions(allServices, addons, editingAddonServiceId, parentServiceId)` returning current value PLUS unused non-primary services that are not the parent (mirrors prereq helper + `classification !== 'primary'` clause); wired `describeSupabaseError` into the `saveAddon` catch. **(2) Parent-self exclusion (defense in depth) BOTH eligibility filters + BOTH edit helpers:** `addonEligibleServices` + `prereqEligibleServices` now have `s.id !== serviceId &&` (ADD mode); `getEditPrereqOptions` + `getEditAddonOptions` now take `parentServiceId` and exclude it (EDIT mode); the current-selection clause always takes precedence (CHECK forbids parent = editing row's prereq/addon, so the helper never accidentally re-introduces it). **(3) Named UNIQUE-constraint messages** in `CONSTRAINT_MESSAGES` (`supabase-error.ts`): prereq → "That prerequisite is already configured for this service."; add-on → "That add-on is already configured for this service." Live constraint names verified against `DB_SCHEMA.md` (add-on carries Postgres' 63-char double-underscore truncation); regexes use permissive `.*key` tail. **Tests +10 → 2635:** new `addon-helpers.test.ts` (7); `prereq-helpers.test.ts` +1 parent-self + 5 existing updated for 4th arg; `supabase-error.test.ts` +2. Scope: 3 prod files modified + 1 new prod file + 1 new test + 2 extended; ~90 prod lines net. No reducer/migration/schema/permission change. **Closes the prereq/add-on edit UX cluster completely** — no more "we found another disabled field" sequels expected on that page. Gates: tsc 0, lint 0err/94warn (none new in touched files), 2635/2635, build clean. Not a 13-item entry. |
| 2026-05-29 | 123 | Audit + fix: Admin Services Edit prereq-service dropdown — un-disabled + error-surfaced (half-built) | Audit + production code fix, merged to main | _(this commit)_ (branch `audit/admin-services-edit-prereq-service-dropdown`) | Operator-reported: prereq-service dropdown greyed in edit mode. **Classification (a) half-built convention.** `git blame` traces `disabled={!!editingPrereq}` (`page.tsx:1807` + paired allServices fallback at `:1810`) to the initial bulk Phase 1 commit `846ece126` (2026-02-01) — no comment, no design discussion. Schema `service_prerequisites` PK on `id`, UNIQUE `(service_id, prerequisite_service_id)`, CHECK `service_id <> prerequisite_service_id`; **zero FKs reference `service_prerequisites.id`**; runtime queries only by parent `service_id`; `savePrereq()` (`:903-941`) already passes `prerequisite_service_id` in the UPDATE payload. Not (b) schema, not (c) data-integrity, not (d) permissions. **Fix in-session, 3 files / ~25 prod lines:** (1) removed `disabled={!!editingPrereq}`; (2) new `src/app/admin/catalog/services/[id]/prereq-helpers.ts` — pure `getEditPrereqOptions(allServices, prerequisites, editingPrereqServiceId)` returns current value PLUS every unused service (preserves selection while excluding other already-used prereqs that would collide with UNIQUE at save); (3) wired `describeSupabaseError` (C1/S3 / #111) into `savePrereq()` catch so UNIQUE-23505 + CHECK-23514 violations surface as actionable text. **Test +5 → 2625:** unit-tested the helper's invariants (current value always present, unused included, other-used excluded, degenerate, defensive duplicate). Mirrors #119 — no admin-page render harness (~2150-line component disproportionate). **Targets 3 & 4 surfaced (NOT in scope):** sibling gap — add-on edit dropdown at `page.tsx:1708` carries the identical pattern, recommend sibling session; pre-existing latent — eligibility filters do not exclude the parent service (`CHECK` 23514 now surfaces via `describeSupabaseError` on prereq path; add-on still generic). Add+delete confirmed working (`:1649/1681/1684`); the disable was friction, not a blocker. Deliverable `docs/dev/PREREQ_SERVICE_DROPDOWN_AUDIT.md`; 3 open operator Qs. Gates: tsc 0, lint 0err/94warn (none new in touched files), 2625/2625, build clean. Not a 13-item entry. |
| 2026-05-29 | 122 | Fix: Track A gate-order — prereq dialog primary, manager-PIN only on override | Production UX-correctness fix, merged to main | _(this commit)_ (branch `fix/track-a-prereq-primary-gate-order`) | Corrects a gate-order bug in the #121 `useValidatedServiceAdd` helper: an `addon_only` service that ALSO has prerequisites fired the **manager-PIN (add-on gate) FIRST**, then the prereq dialog — backwards (operator-reported on "Paint Correction Prep"). **LOCKED contract:** (1) prerequisite check PRIMARY — unmet → `PrerequisiteWarningDialog` (add a prereq, or manager-override behind its OWN Override button), the add-on gate never fires; satisfied → commit. (2) add-on-only gate CONDITIONAL — fires ONLY when the service has **no prerequisites configured** AND is `addon_only` AND solo (no `primary`/`both` anchor). Rationale: when prereqs exist they ARE the gate; a satisfied/overridden prereq implicitly authorizes the add-on, so a separate add-on PIN would be redundant and wrongly preempt the prereq dialog. **2 prod files (~40 lines):** `use-prerequisite-check.ts` now returns `hasPrerequisites` (endpoint already reported it; was folded into `canAdd`) so the helper knows whether the add-on gate applies; `use-validated-service-add.tsx` `runValidations` runs the prereq check FIRST and gates the add-on warning on `!hasPrerequisites && isAddOnSolo`; `handleAddOnSoloOverride` commits directly (no re-check — solo warning only appears when prereqs are already clear). No reducer/migration/schema/permission change; `PrerequisiteWarningDialog`/`ManagerPinDialog`/endpoint/Track B untouched; the 3 consumer surfaces (catalog-browser/quote-builder/register-tab) unchanged. **Tests (+5 → 2620):** the 4 add-on-solo "no network" assertions across engine + 3 per-surface tests **locked the wrong behavior** → rewritten (prereq check runs first, so the network IS called before the add-on gate); new per-surface "addon_only WITH unmet prereqs → PREREQ dialog (not add-on PIN)" + engine "addon_only WITH satisfied prereqs → commits, no PIN". Gates: tsc 0, lint 0err/97warn (none new), build clean. Not a 13-item entry (POS prereq/gating arc). |
| 2026-05-28 | 121 | Fix Track A: `useValidatedServiceAdd` shared helper — prereq + add-on gating across Sale/Quotes/register-tab | Production code fix + canonical-engine refactor, merged to main | _(this commit)_ (branch `fix/track-a-useValidatedServiceAdd-shared-helper`) | Closes **G5** (`SALE_VS_QUOTES_PARITY_SWEEP.md`) + the never-built add-on-only gating + the register-tab no-add-time-validation gap from `POS_PREREQ_ENFORCEMENT_AND_GATING_AUDIT.md` + `POS_SALE_VS_QUOTES_PARITY_AUDIT.md` Target 5. **New hook `src/app/pos/hooks/use-validated-service-add.tsx`** (canonical add-time gate, CLAUDE.md Rule 22): add-on-only gate (`classification==='addon_only'` && no `primary`/`both` anchor on the order → **warn + manager-PIN override**, reuses `pos.override_prerequisites`) → prerequisite check (wraps `use-prerequisite-check`) → commit via caller `onAdd`; owns BOTH warning dialogs + the prerequisite auto-add orchestration (de-duplicated from catalog-browser + quote-builder). **Sale `catalog-browser` refactored behind it byte-behavior-identically** (the reference; `addServiceChecked`→hook's `addService`, prereq handlers deleted). **Quotes `quote-builder`** routes search/picker through the hook (quote context) + the browse path via new `customerIdOverride`/`vehicleIdOverride`/`serviceIdsOverride` props on `<CatalogBrowser>` (the G5 fix — was hardwired to `useTicket()`). **`register-tab`** favorites + picker route through the hook (gained the prereq check + gating it never had). **`service-detail-dialog`** runs the validation check in callback (quote) mode too (was dispatch-only) so quote browse-via-detail-dialog is no longer unchecked. Add-on-only rule = warn-and-allow w/ manager override (operator decision). No reducer/migration/schema change. **Tests (+17 → 2601 Track A alone; 2615 combined w/ Track B #120):** `use-validated-service-add.test.tsx` (8 — engine), `catalog-browser-validation.test.tsx` (4 — Sale add-on+prereq+parity-guard + Quote-context G5 proof), `register-tab-validation.test.tsx` (3), `quote-builder-validation.test.tsx` (2 — search). Gates: tsc 0, lint 0err/97warn (none new), build clean. No Track B file overlap. Not a 13-item entry (Sale-vs-Quotes parity arc). |
| 2026-05-28 | 120 | Fix Track B: Quotes-panel parity wiring (G2/G3/G4) + structural guard test | Production code fix, merged to main | _(this commit)_ (branch `fix/track-b-quotes-panel-parity-wiring`) | Closes 3 Sale-vs-Quotes gaps from the #118 sweep (`SALE_VS_QUOTES_PARITY_SWEEP.md`) by mirroring Sale exactly; no reducer/migration changes (sweep Target 5: `SET_CUSTOMER`/`SET_VEHICLE` identical both sides). **G2 vehicle-edit unreachable → reachable:** `editingVehicle` state + `onEditVehicle` on `<CustomerVehicleSummary>` (pencil only renders when prop passed, `customer-vehicle-summary.tsx:116`) + `editVehicle` on `<VehicleCreateDialog>` w/ edit-branch `onCreated` dispatching `SET_VEHICLE` (`blockedByPayment:false`). **G3 reprice-failure silent → surfaced:** panel `toast.warning` watcher (mirror `ticket-panel.tsx:123-143`) + amber "No <size> pricing" badge in `quote-item-row.tsx` (mirror `ticket-item-row.tsx:138-151`); reducer already set `repriceFailed`. **G4 `CustomerTypePrompt` never shown → mounted:** opened from `handleSelectCustomer`/`handleCustomerCreated` when `customer_type` null, sequenced into the vehicle selector on dismiss (quote-flow adaptation; Sale has no vehicle step), reuses #119 `handleCustomerTypeChanged`. **STRUCTURAL GUARD (durable win)** `src/app/pos/__tests__/sale-vs-quotes-shared-prop-parity.test.tsx` — source-contract test: every prop Sale wires on each of the 7 shared components must be wired in Quotes (minus documented allowlist `disabled`); also pins `CustomerTypePrompt` mounted both + `repriceFailed` surfaced both. **Verified teeth** (removing `onEditVehicle` → fails naming it). + `quote-item-row-reprice-badge.test.tsx` (G3 badge). **G6 (swipe+undo) DEFERRED** (Minor; needs `RESTORE_ITEM` action). Scope: 2 prod files + 2 test files; no Sale changes, no Track A files. Gates: tsc 0, lint 0err/94warn (none new), **2598/2598 (+14)**, build clean. Not a 13-item entry (parity arc). |
| 2026-05-28 | 119 | Fix: wire onCustomerTypeChanged in Quotes pill mount (silent customer demotion) | Production code fix, merged to main | _(this commit)_ (branch `fix/quotes-customer-type-pill-callback-wiring`) | Resolves the Critical finding from #117 (`POS_CUSTOMER_TYPE_PILL_PARITY_AUDIT.md` → RESOLVED). Shared `CustomerTypeBadge` PATCHes the permanent customer record on tap then calls `onTypeChanged` for the host to sync local state; Sale wired it (`ticket-panel.tsx:413`), **Quotes never did** → every Quotes tap silently re-tagged the customer globally while the pill showed stale state. **Fix (~9 lines, 1 prod file, mirror Sale exactly):** added `handleCustomerTypeChanged` to `quote-ticket-panel.tsx` (dispatch `SET_CUSTOMER` with `{ ...quote.customer, customer_type: newType }`), wired `onCustomerTypeChanged` on the `CustomerVehicleSummary` mount, added `CustomerType` import. **No reducer change** (`quote-reducer.ts:361` already handles `SET_CUSTOMER`). Sale path + `CustomerTypeBadge` + `/api/pos/customers/[id]/type` untouched. Product decision LOCKED: global+permanent, no confirm dialog. **Tests (+4 → 2584):** new `customer-type-badge.test.tsx` — tap persists (PATCH) + signals `onTypeChanged` with cycled value; full cycle order; **stale-state regression** (no callback → PATCH fires but badge can't advance = the bug); display-follows-prop. Panel wiring guaranteed by tsc + manual verification (no QuoteTicketPanel harness — ~14 children/4 contexts, disproportionate to a ~9-line fix). Gates: `tsc --noEmit` 0, lint clean on touched files, `npm test` 2584/2584, `next build` clean. No migrations, no new keys. **Deploy soon** (data-integrity). |
| 2026-05-28 | 118 | Sweep: POS Sale-vs-Quotes shared-component parity (enumerates ALL gaps in one pass; locks the upcoming Track-A/Track-B fix arc) | Read-only diagnostic SWEEP (no app code), merged to main | `31199db7` (merge `c79dcf91`, branch `audit/sale-vs-quotes-shared-component-parity-sweep`) | **Ledger row backfilled in Session #145 (2026-06-03)** — this session shipped 2026-05-28 with `docs/dev/SALE_VS_QUOTES_PARITY_SWEEP.md` and a clean merge to main, but the ledger row was never added; #117 → #119 in the ledger as a result. The renumbering note inside the #119 row ("renumbered #118 → #119") was a misread of the parallel-session collision — the sweep DID land as #118 on main, then the panel-parity fix landed as #119. Fifth Sale-vs-Quotes artifact on 2026-05-28 — the prior four (#114–#117) each found ONE gap by stumbling onto a symptom; this sweep deliberately enumerates ALL of them in one pass so the upcoming fix arc is the last one. **7 components mounted in BOTH panels** (`ticket-panel.tsx` / `quotes/quote-ticket-panel.tsx`): `CustomerVehicleSummary`, `CustomerLookup`, `CustomerCreateDialog`, `VehicleSelector`, `VehicleCreateDialog`, `PrerequisiteRemovalDialog`, `ManagerPinDialog`. Their callback props are **not** byte-for-byte — `CustomerVehicleSummary` alone has 3 prop deltas in Quotes. **Gaps found (1 Critical in-flight + 4 Significant + 1 Minor):** **G1** customer-type pill `onCustomerTypeChanged` omitted (silent demote; in-flight as #119); **G2** vehicle edit unreachable in Quotes (`onEditVehicle` omitted on the quote summary + no `editVehicle` passed to `VehicleCreateDialog` + no `editingVehicle` state; operator cannot correct a quote vehicle's `size_class` inline → silent mispricing); **G3** reprice failure fully silent in Quotes (`quote-reducer.ts:393-423` sets `repriceFailed` but Quotes surfaces it nowhere — no panel toast watcher, no amber badge in `quote-item-row.tsx`); **G4** `CustomerTypePrompt` never shown in Quotes (Sale-only mount; quote `handleSelectCustomer`/`handleCustomerCreated` skip the `if (!customer.customer_type)` prompt); **G5** quote browse prereq wrong-context (`<CatalogBrowser>` `useTicket()` already documented in #116; Significant; Track A); **G6** no swipe-to-delete + undo in Quotes (`RESTORE_ITEM` absent from quote reducer; Minor). **Target 3 (context sweep):** NONE of the 7 panel-mounted shared components read `useTicket()` — all take customer/vehicle via props. The only wrong-context shared component is `catalog-browser`, mounted by `quote-builder` (not the panel) = G5. **Target 5 (reducer parity):** reducers are at functional parity for every shared concern; `SET_CUSTOMER`/`SET_VEHICLE` identical both sides → G1–G4 require NO reducer change (G6 alone needs a `RESTORE_ITEM` quote action). **Fix-arc (two tracks, disjoint code areas, can run in parallel):** Track A — service-add validation: the `useValidatedServiceAdd` helper (prereq + add-on gating across Sale `catalog-browser`, Quotes `quote-builder` both views, `register-tab`); closes G5 + add-on gating + register-tab no-check (delivered as #121, gate-order corrected as #122). Track B — Quotes-panel parity: one session wiring the props/handlers Sale already has, all in `quote-ticket-panel.tsx` + `customer-vehicle-summary` wiring + `quote-item-row` + `vehicle-create-dialog`; G1 (in-flight = #119) is the first commit; G2/G3/G4 follow (delivered as #120 with structural-guard test `sale-vs-quotes-shared-prop-parity.test.tsx`); G6 optional/deferred. Both tracks landed within the next ~2 days (#119–#122). **Informational (by design, not gaps):** `disabled` prop (quotes have no checkout), guest-checkout (quotes need a customer), `ENTER/EXIT_EDIT_MODE` (Sale job-edit vs quote load+autosave), `pos-vehicle-needed` gate skipped in quotes (vehicle-optional by design — `catalog-browser.tsx:359` gates behind `if (!onAddService)`), mobile/validity/draft actions (quote-only features). No source/migration/test changes. Not a 13-item entry (POS Sale-vs-Quotes parity arc). |
| 2026-05-28 | 117 | Audit: POS customer-type pill Sale-vs-Quotes parity + persistence | Read-only audit (no app code), merged to main | _(this commit)_ (branch `audit/pos-customer-type-pill-sale-vs-quotes-parity`) | 4th Sale-vs-Quotes parity gap today. Pill (Unknown/Enthusiast/Professional) cycles in Sale, shows "Customer type cleared" in Quotes. **Root cause = missing prop, classification (c) half-built wiring (NOT wrong-context):** shared `CustomerTypeBadge` (via `CustomerVehicleSummary` in both surfaces) PATCHes the record then calls `onTypeChanged` for the host to sync LOCAL state. Sale wires `onCustomerTypeChanged` (`ticket-panel.tsx:413`→`:357-360` `SET_CUSTOMER`); **Quotes omits it** (`quote-ticket-panel.tsx:830-840`) → undefined no-op (`customer-type-badge.tsx:70`) → quote local `customer.customer_type` never updates → badge renders stale value → every tap repeats one transition (`nextType`, `:27-32`); operator's customer was `professional`→null→"cleared" toast (`:75`). **Persistence: YES both surfaces** — badge PATCHes `/api/pos/customers/[id]/type` → direct `customers.customer_type` write + audit row (`route.ts:38-69`); live DB 732 enthusiast/285 professional/370 null. Pill = GLOBAL permanent record change (Admin→Customers reflects it), not quote-scoped → Quotes stale-UI can silently demote a customer. **Fix:** mirror Sale (~3 lines, `quote-ticket-panel.tsx`; `quote-reducer.ts:361` already handles `SET_CUSTOMER` — no reducer change). **Bundle** with the Quotes-parity fix session as an independent commit (NOT part of the prereq/gating shared add-with-validation helper — different code area). Optional hardening: confirm step (pill mutates the permanent record). Deliverable `docs/dev/POS_CUSTOMER_TYPE_PILL_PARITY_AUDIT.md`; 3 open operator Qs. No source/migration/test changes. |
| 2026-05-28 | 116 | Audit: POS Sale-vs-Quotes prereq + add-on gating parity | Read-only audit (no app code), merged to main | _(this commit)_ (branch `audit/pos-sale-vs-quotes-prereq-gating-parity`) | Pins the Sale-vs-Quotes add-path mechanism difference for the parity fix. **Corrects #115's overgeneralization** ("quote paths dispatch directly"): `quote-builder.tsx` DOES call `checkPrerequisites` in all add paths since 2026-03-14 (`4afe5ed94`/`99ba7d2c5`). **Defect is context, not absence:** quote BROWSE view delegates to `<CatalogBrowser>` whose check is hardwired to the SALE-ticket context (`catalog-browser.tsx:76-80` `useTicket()`), not the quote; quote search/picker use correct quote context (`quote-builder.tsx:221-225`, `:314/:340/:354`). Wrong context → over-fires on empty sale ticket, silently under-fires when a Sale ticket already holds the prereq (the "doesn't fire" symptom). Reducers pure (store `prerequisiteNote` only). **Surface matrix:** Sale/catalog-browser ✓ correct; Sale/register-tab favorites ✗ zero check (re-verified 0 matches); Quotes search/picker ✓ correct; Quotes browse ⚠ wrong context. Hook is surface-agnostic (`use-prerequisite-check.ts:28-32`) — only CatalogBrowser hardcodes `useTicket()` (overrides vehicle SIZE for pricing but not prereq context). Add-on gating absent everywhere (per #115). **Rec:** one shared add-with-validation helper (addon-gate → prereq-check → dispatch) routed through Sale + Quotes (both views) + register-tab; fixes quote-browse context + register-tab no-check + gating together (Rule 11/22). Blast radius ~3 components + 1 hook; Sale path must stay behavior-identical. Deliverable `docs/dev/POS_SALE_VS_QUOTES_PARITY_AUDIT.md`; 3 open operator Qs (chief: confirm quote repro — search vs browse, sale ticket open?). No source/migration/test changes; fix is a separate session. |
| 2026-05-28 | 115 | Audit: POS prereq-enforcement regression + add-on gating + create-service tier persistence | Read-only audit (no app code) | _(this commit)_ (branch `audit/pos-prereq-enforcement-regression-and-addon-gating`) | Diagnoses 3 issues that surfaced same-day as #114; decides fix-forward vs revert. **#114 caused none** — behavior-preserving tier-selection refactor; all 3 are pre-existing gaps surfaced by creating/testing the new `addon_only` "Paint Correction Prep". **(1) Tiers:** live DB shows 3 `vehicle_size` tiers (sedan $160/truck $190/suv $220), no exotic/classic; root cause `new/page.tsx:231-237` hard-codes 3 inserts (exotic/classic only via `scope` model or Edit page). **(2) Add-on gating:** never built — no `classification`/`addon_only` check in any POS add path (grep: test fixtures only); `addon_only` addable solo on every screen. **(3) Prereq enforcement:** catalog-browser (all paths via `addServiceChecked→checkPrerequisites`) + `ServiceDetailDialog` (`:199-200`) DO check, unchanged by #114; `register-tab.tsx` has ZERO prereq handling in any path (favorites + picker dispatch `ADD_SERVICE` directly), pre+post #114; client fails open (`use-prerequisite-check.ts:77-78,109-110`); detection endpoint + hook untouched, no `picker-engine` import. **T4:** Issues 2+3 share register-tab's no-validation direct-dispatch (if that was the repro); Issue 1 independent. **T5: FIX-FORWARD** — revert reintroduces the wanted $75→$110 fix and fixes nothing; surgical fixes = shared prereq-checking add helper for register-tab + add-on gate at add-time + exotic/classic in create form + stop silent fail-open. **T6:** #114's 15 tests cover prereq PRICING not DETECTION/register-tab/gating. Deliverable `docs/dev/POS_PREREQ_ENFORCEMENT_AND_GATING_AUDIT.md`; 4 open operator questions (chief: confirm exact repro screen). No source/migration/test changes. **DO NOT merge.** |
| 2026-05-28 | 114 | Fix: POS prerequisite size-aware pricing via canonical `selectPricingTierForVehicle()` (Item 15f-adjacent) | Production code change | _(this commit)_ (branch `fix/pos-prerequisite-size-aware-pricing-canonical-tier-selector`) | Fixes the Critical bug from `POS_PREREQUISITE_PRICING_AUDIT.md` (now RESOLVED): prereq auto-add used `prereqPricing[0]` (always sedan/first tier) instead of the `size_class`-matched tier — Suburban (`suv_3row_van`) prereq priced **$75** not **$110**. A drift bug: the `tier_name === size_class` selection existed correctly in 4 add paths, skipped by the 2 prereq paths. **D1 (DRY):** extracted canonical **`selectPricingTierForVehicle(pricing, size_class)`** into `picker-engine.ts` (CLAUDE.md Rule 22 — handles single non-size-aware, row-based Pattern B `tier_name` match, single column-based Pattern A; returns `null` on no-match). Routed **all 6** selection sites through it: 2 prereq (the fix — `catalog-browser.tsx` + `quote-builder.tsx` `handleAddPrerequisite`) + 4 behavior-preserving (`routeServiceTap`, `catalog-browser` direct/unchecked, `register-tab`). **D2 (no-match):** prereq sites **block** with warning toast (add neither prereq nor dependent add-on); normal sites preserve fall-through to manual picker. **Tests (+15 → 2580):** 9 `selectPricingTierForVehicle` unit + 6 integration (`prerequisite-size-aware-pricing.test.ts` — ticket+quote reducers land $110 for `suv_3row_van`; old `[0]`→$75; every size→own price; no-match→null). All 64 pre-existing picker-engine/reducer/component tests green (behavior-preservation proof). **Gates:** tsc 0; lint 0 errors / 97 warnings (baseline); `npm test` 2580/2580 (2565 + 15); build clean. NO migrations, no new keys. Booking wizard unaffected (no prereq logic). Forward-only. **DO NOT merge — operator verifies in POS (add Paint Correction Prep to a Suburban ticket → prereq reads $110), then merges + deploys.** |
| 2026-05-28 | 113 | Harden + version-control the VPS deploy script (infra) | Infra / repo hygiene (no app code) | _(this commit)_ (branch `chore/harden-deploy-smartdetails-script`) | Brought the previously-unversioned `/usr/local/bin/deploy-smartdetails` into the repo at `scripts/deploy/deploy-smartdetails.sh` (executable `100755`) + `scripts/deploy/README.md`; `/usr/local/bin/deploy-smartdetails` to become a **symlink** to it (future hardening via `git pull`). Hardened after the 2026-05-27 transient-`npm ci` incident (first failure in 100+ runs; `--silent` hid the error + a hardcoded "lockfile drift" message sent the operator on a ~10-min phantom chase). **H1** diagnosable npm ci (tee → timestamped `…-npm-ci.log`, `${PIPESTATUS[0]}` recovers npm's real exit code vs tee's 0); **H2** honest probability-ordered fail message (transient → re-run; drift → `npm install`+commit; disk → `df -h`); **H3** retry-once (5s backoff; transient absorbed, genuine drift/disk fails BOTH → exit 2; `if run_install_attempt` disables `set -e` for attempt 1 only); **H4** `rm -rf .next` before build; **H5** phase log to `/var/log/deploy-smartdetails/<ts>.log` (fallback `/tmp/`, printed at end, 30-day prune). **Preserved:** GUARD 1-4, all 9 steps, exit codes 0-5 (npm-ci failure stays exit 2; two old npm `fail` lines consolidated into one retry-aware `fail … 2`). **Verification:** `bash -n` clean; shellcheck unavailable (documented); retry/PIPESTATUS/`set -e` logic validated against an isolated stub harness (clean→0, one-transient→absorbed→0, double-fail→exit 2). NO app code / migrations / app deploy. Session **#113** because #112 is earmarked in the #111 writeup for the deferred catalog S1 (slug-on-rename). **DO NOT merge — operator reviews + merges, then one-time VPS migration: back up `/usr/local/bin/deploy-smartdetails` → `ln -sf …/scripts/deploy/deploy-smartdetails.sh /usr/local/bin/deploy-smartdetails` → verify on next real deploy.** |
| 2026-05-27 | 111 | Catalog C1 (Create Service slug) + S3 error-surfacing helper | Production code change | _(this commit)_ (branch `fix/catalog-c1-create-service-slug-and-s3-error-surfacing`) | Fixes the Critical "Failed to create service" bug from `CATALOG_CRUD_WIRING_AUDIT.md` (root cause: `services.slug` UNIQUE NOT NULL, no default/trigger; create form never set slug → 23502, masked by generic toast). Architecture LOCKED browser-client+RLS. **Scoped down per Memory #8 + context budget: C1 + S3 helper-applied-to-services/new this session; broader S3 sweep = #111b.** **3 production files:** (1) `lib/utils/validation.ts` — `serviceCreateSchema` gains `slug: slugSchema.optional()`. (2) `services/new/page.tsx` — slug default + name→slug auto-gen `useEffect` (overridable "URL Slug" input via `slugManuallyEdited`) + uniqueness pre-check + slug in payload + catch now uses `describeSupabaseError`. Mirrors proven `products/new` flow (M5 TOCTOU left as-is). (3) NEW `lib/utils/supabase-error.ts` — `describeSupabaseError(err, fallback)` maps named constraints (sale-price, slug-unique) + SQLSTATE (23502 w/ column, 23505, 23514, 23503) → friendly text, else raw msg, else fallback; generalizes `categories/page.tsx:252`. **Tests (+11):** `supabase-error.test.ts` (8 helper + 3 schema-slug). **Gates green:** tsc 0; lint 0 errors / 97 warnings (baseline); `npm test` **2565/2565** (2554 + 11); build clean. NO migrations, no new keys. **DO NOT merge — operator verifies + manually creates a real service in Admin → Services → Add New (the proof), then merges + deploys.** **#111b:** apply `describeSupabaseError` to `services/[id]` (save-details + save-pricing), `products/new`, `products/[id]`. Then #112 = slug-on-rename (S1) + sale-price CHECK trap (S2). |
| 2026-05-27 | 110 | Item 15e Phase 2C-β-2 corrective — Supabase 1:1 cardinality shape in `has_active_job` (production-blocking fix) | Production code change | _(this commit)_ (branch `fix/item-15e-phase-2c-beta2-supabase-cardinality-jobs-shape`) | **Production-blocking:** #109's `has_active_job` derivation assumed `appointment.jobs` is an array, but `jobs.appointment_id` has a UNIQUE constraint (migration `20260329000002`) → PostgREST infers **1:1** and returns `jobs` as a **single object (or null), NOT an array** → `.some()` threw `(intermediate value).some is not a function`, crashing **admin Customers + Appointments** on load (~30 min after #109 deploy). **Pre-flight found local main mid-merge of the unrelated `audit/catalog-services-products-tiers-addons-crud-wiring` branch (1fdf4cca) with a raw CHANGELOG conflict; that branch is fully on origin → operator approved `git merge --abort` (lost nothing), restoring clean main at `78521314`.** **Fix (shape-defensive, both surfaces):** (1) NEW `src/app/admin/appointments/has-active-job.ts` — `withHasActiveJob` EXTRACTED from `page.tsx` (Next.js pages can't carry arbitrary named exports — the build failed when exported inline), now normalizes via `asRelationArray(v)=Array.isArray(v)?v:v?[v]:[]`; `page.tsx` imports it. (2) MOD POS `GET /api/pos/appointments/[id]/route.ts` — same normalization before `.some()`. **Sibling audit (mandatory):** the 3 `jobs:jobs!appointment_id` derivations (admin ×2 via shared mapper, POS ×1) were the only bugs; `transaction-detail.tsx:128` (`transaction.jobs ?? []`) CLEARED — `jobs.transaction_id` is NOT unique → genuine 1:many array → correct. **Tests (+10):** POS `get.test.ts` +3 (single-object scheduled/completed, null); NEW `__tests__/with-has-active-job.test.ts` +7 (object/null/undefined/array shapes + stripping) — closes the gap where #109 only mocked arrays. **CLAUDE.md:** added "Supabase relation cardinality" Key Pattern (UNIQUE-FK embeds → single object; normalize before iterate). **Gates green:** tsc 0; lint 0 errors / 97 warnings (baseline); `npm test` **2554/2554** (2544 + 10); build clean. NO migrations, no new permission keys. Untouched: lifecycle-sync/executor, unmaterialize endpoints, modal, catalog arc, D43-D50. **DO NOT merge — operator merges + deploys (production hotfix path).** |
| 2026-05-27 | 109 | Item 15e Phase 2C-β-2 — admin un-materialize intercept + POS "Revert to Pending" button + has_active_job (**Item 15e CLOSED**) | Production code change | _(this commit)_ (branch `feat/item-15e-phase-2c-beta2-admin-intercept-pos-button-and-has-active-job`) | Wiring half of 2C-β; **completes Phase 2C and CLOSES Item 15e.** Built on merged β-1 (`b7c4e385`). **5 production files / ~130 lines** (3 are mechanical: 1 type field + 2 select-derivations) — over the ~3-file count but well under the ~300-line threshold, judged one reviewable unit. **Pre-flight caught β-1 was pushed-not-merged (session #108 branch unmerged); flagged + operator merged β-1 first, then re-ran this session off the β-1 merge.** (1) MOD `src/lib/appointments/types.ts` — added optional `has_active_job?: boolean` to `AppointmentWithRelations` (undefined=not-populated/don't-intercept, false=no-active-job, true=intercept). (2) MOD POS `GET /api/pos/appointments/[id]/route.ts` — joins `jobs:jobs!appointment_id(id, status)`, derives `has_active_job` (any job status ∉ completed/closed/cancelled), strips raw `jobs` from the response. (3) MOD `src/app/admin/appointments/page.tsx` — **discovery: there is NO admin GET route**; the admin dialog's data comes from a direct browser Supabase query, so both month + day selects gained the `jobs` join + a module-level `withHasActiveJob()` mapper (derives the flag, drops the raw array). (4) MOD `appointment-detail-dialog.tsx` — Save intercept after the existing cancel short-circuit: opens the shared `<UnMaterializeConfirmationDialog context="admin">` ONLY when status changed AND `isEarlierState(new,current)` (revert; excludes cancel/no_show) AND `has_active_job===true`; every other save byte-identical. (5) MOD `job-detail.tsx` — header "Revert to Pending" button gated on `usePosPermission('appointments.cancel')` + non-terminal `job.status` + `job.transaction_id==null` (hidden when 409 guaranteed); click → `posFetch` the appointment → mount modal `context="pos"`; onSuccess → `onBack()` to Jobs queue. `transaction_id` added to `JobDetailData` (present via `JOB_SELECT *`). NO new permission key (reuses `appointments.cancel`). **Tests (+10):** POS GET `has_active_job` derivation (5 — none/scheduled/completed/mixed + jobs-array stripping); admin intercept render test (5 — revert+active→modal & no-onSave; revert+no-active→onSave; later-state→onSave; unchanged→onSave; cancelled→cancel-short-circuit). β-1 populate-rerun invariant still green. POS button visibility = operator manual Gate 5 (no render harness for the 2000-line job-detail; gating is simple inline logic). **Gates green:** `npx tsc --noEmit` 0 errors; `npm run lint` 0 errors / 97 warnings (baseline unchanged); `npm test` **2544/2544** (2534 + 10); `npm run build` clean. NO migrations. Confirmed untouched: unmaterialize endpoints + shared modal (β-1/2C-α), admin/POS PATCH (2A), populate, walk-in creation, D43-D50/D45-D48. Deploy: ships Phase 1+2+2C as one release. **DO NOT merge — operator verifies gates + diff + manual UI (Gates 3-6: admin byte-identical, admin intercept, POS button matrix, GET field), then merges.** **Item 15e CLOSED.** Open follow-ups: Item 15h (full bidirectional sync), Item 13 (Detailer Mobile Link). |
| 2026-05-27 | 108 | Item 15e Phase 2C-β-1 — un-materialize shared confirmation modal + dryRun preview + populate-rerun invariant (UI foundation half) | Production code change | _(this commit)_ (branch `feat/item-15e-phase-2c-beta1-shared-modal-and-dryrun`) | First half of 2C-β, SPLIT per Memory #8 (full 2C-β = 6-7 production files: types `has_active_job` + 2 GET widenings + shared modal + admin dialog intercept + POS job-detail button + dryRun on 2 endpoints). 2C-β-1 = reusable modal + its dryRun server support + the load-bearing invariant test; **modal is INERT until 2C-β-2 mounts it** (foundation-then-wiring pattern, as 2A/2C-α). **4 production files (3 are small dryRun changes + 1 new modal):** (1) MOD `src/lib/appointments/lifecycle-sync.ts` — added `dryRun?:boolean` to `UnMaterializeOptions` + early preview-return in `executeUnMaterialize` (runs guards not_found/transaction_linked/terminal + collects data enumeration, then returns 200+data WITHOUT mutating — no revert/delete/storage/audit; a blocked job still surfaces 409 in dry-run so the modal blocks immediately). (2)(3) MOD both unmaterialize endpoints (`/api/pos/appointments/[id]/unmaterialize`, `/api/appointments/[id]/unmaterialize`) — accept `dryRun` in body schema + forward to executor. (4) NEW `src/components/appointments/un-materialize-confirmation-dialog.tsx` — shared modal reused by BOTH surfaces (canonical-engine); `context:'admin'|'pos'` selects endpoint URL + auth wrapper (`adminFetch` vs `posFetch`); on open runs dryRun → renders exact deletion enumeration (photos+"image files removed", add-on requests, timer progress, intake notes) OR block message (payment attached/terminal); type-to-confirm input (exact whitespace-trimmed "DELETE", Revert disabled until valid) for in_progress/pending_approval; execute re-POSTs without dryRun; NO webhook (silent revert); dark-aware; reuses Dialog primitive. **Tests (+14):** `lifecycle-sync.test.ts` +6 (4 dryRun-preview cases incl. dryRun-still-409-on-transaction/terminal + **2 populate-rerun re-materialization-invariant tests** — post-un-materialize appt is `pending` so populate's confirmed/in_progress filter (mirrors populate/route.ts:65) excludes it; + the appt-update-before-job-delete ordering assertion); NEW modal component test (7 — dry-run-on-open per context/endpoint/wrapper, enumeration accuracy, free-zone Revert executes with confirmString=DELETE, confirm-required gating (wrong-case stays disabled), payment-block hides Revert); endpoint dryRun-passthrough (+1 each). **Gates green:** `npx tsc --noEmit` 0 errors; `npm run lint` 0 errors / 97 warnings (baseline unchanged); `npm test` **2534/2534** (2520 + 14); `npm run build` clean. NO migrations. Confirmed untouched: admin/POS appointment PATCH, populate, job lifecycle endpoints, admin dialog, job-detail, D43-D50/D45-D48. **Item 16 (Mobile Detailer Remote Job Management) added to roadmap this session.** Deploy: NO (modal inert until 2C-β-2). **DO NOT merge — operator verifies gates + diff, then merges; 2C-β-2 branches off merged 2C-β-1.** **Phase 2C-β-2 (next):** `has_active_job` widening (types + 2 GET) + admin Save intercept + POS "Revert to Pending" button (all mount this modal). Then Item 15h (full bidirectional sync). |
| 2026-05-27 | 107 | Item 15e Phase 2C-α — un-materialize server foundation (lifecycle-sync seam + 2 endpoints) | Production code change | _(this commit)_ (branch `feat/item-15e-phase-2c-unmaterialize-via-lifecycle-sync-seam`) | Server half of Phase 2C, SPLIT per Memory #8 (full 2C = 6-7 production files / >300 lines: helper + 2 endpoints + shared modal + dialog intercept + job-detail button + has_active_job widening). 2C-α = server foundation only; **purely additive — ZERO existing production files touched; endpoints inert until 2C-β wires UI** (mirrors 2A's inert PATCH). (1) NEW `src/lib/appointments/lifecycle-sync.ts` — the canonical seam Item 15h extends. `jobStatusForAppointmentStatus(newApptStatus, currentJobStatus, hasJob)` forward mapping (Phase 2C implements ONLY `delete_job` for `→ pending` with existing job; all else `none` incl. walk-in `in_progress`-appt+`scheduled`-job pairing left untouched); `isEarlierState()` ranks forward appt axis (pending<confirmed<in_progress<completed; cancelled/no_show unranked) for the 2C-β admin Save intercept; `executeUnMaterialize(supabase, appointmentId, {confirmString, actor, source, ipAddress})` executor. **Guards:** `not_found` 404 (missing appt OR no job — a plain status edit goes through PATCH), `transaction_linked` 409 (`jobs.transaction_id` set — Decision 3), `terminal` 409 (completed/closed/cancelled jobs — defensive, beyond the brief's enum, per Rule 4 cover-all-scenarios), `confirm_required` 422 WITH data enumeration (photoCount/addonCount/timerSeconds/hasIntakeNotes) when `job.status IN (in_progress, pending_approval)` and `confirmString !== 'DELETE'` (exact case-sensitive — Decision 8/9). **Re-materialization invariant WITHOUT a migration (Decision 10):** UPDATE appointment→`pending` FIRST, then DELETE job — Supabase JS has no multi-statement txn, but this ordering makes the only unsafe state (materializable appt + absent job) impossible; a failed DELETE leaves the benign recoverable `pending`+job state, never re-materialization. CASCADE removes `job_photos`/`job_addons` rows; Storage objects (main + `_thumb`, bucket `job-photos`) deleted best-effort (failures logged, never roll back DB — Decision 4); audit row (`action:'delete'`, `entityType:'job'`, `details.reason:'un_materialize'`+`previous_job_status`, `source` = surface); **NO webhooks** (Decision 5). (2) NEW `POST /api/pos/appointments/[id]/unmaterialize` — HMAC + `checkPosPermission('appointments.cancel')`. (3) NEW `POST /api/appointments/[id]/unmaterialize` — `getEmployeeFromSession` + `requirePermission('appointments.cancel')`. Both thin wrappers passing through executor `httpStatus`/`error`/`data` (401/403/404/409/422/200); **NO new permission key** (Decision 2 — but runtime `appointments.cancel` grants all 4 roles incl. detailer per audit). Two endpoints not one because admin cookie-auth ≠ POS HMAC-auth (audit finding); both call the one shared executor. **Tests (+31):** `lifecycle-sync.test.ts` (20 — forward-mapping incl. walk-in, `isEarlierState`, executor guards/storage/audit + **CRITICAL re-materialization-invariant ordering test** asserting appt→pending precedes job-delete and final status ∉ {confirmed,in_progress}); POS endpoint (6) + admin endpoint (5) auth/permission/pass-through. **Gates green:** `npx tsc --noEmit` 0 errors; `npm run lint` 0 errors / 97 warnings (baseline unchanged); `npm test` **2520/2520** (2489 + 31); `npm run build` clean (both routes compiled). NO migrations. Confirmed untouched: admin/POS appointment PATCH, populate, all job lifecycle endpoints, admin dialog, D43-D50/D45-D48. Deploy: NO (endpoints inert until 2C-β). **DO NOT merge — operator verifies gates + diff, then merges; 2C-β branches off merged 2C-α.** **Phase 2C-β (next):** shared `un-materialize-confirmation-dialog.tsx` + admin Save intercept + POS "Revert to Pending" button + `has_active_job` widening + end-to-end populate-rerun invariant test. Then Item 15h = full bidirectional sync. |
| 2026-05-27 | 106 | Item 15e Phase 2 follow-up — status sync gap + un-materialize scope (read-only diagnostic audit) | Audit done | _(this commit)_ (branch `audit/phase-2-status-sync-and-unmaterialize`) | Diagnostic audit (`docs/dev/ITEM_15E_PHASE_2_STATUS_SYNC_AUDIT.md`) of the lifecycle-sync gap surfaced in Phase 2B testing: editing `appointment.status` does NOT propagate to `jobs.status`. No source/migration/test changes — 1 audit doc + 3 doc updates. **Verified the gap live** (service-role REST): test appt `2099ffa8` status=`pending` but its job `4b02e4f2` status=`scheduled`, `transaction_id`=NULL. **The divergence is SYSTEMIC** — a scan of all 7 non-terminal jobs found 6 diverge from a 1:1 mapping, incl. a live **`cancelled → scheduled`** (cancelled appt, still-active job → admin appt-cancel does NOT cancel the linked job) + three `completed → scheduled/intake`; only `confirmed → scheduled` is the correct materialized pairing. **Existing sync = ONE partial edge:** POS job-cancel → `appointment.status='cancelled'` (`api/pos/jobs/[id]/cancel/route.ts:145-154`). Nothing else syncs: appt-status edits (admin PATCH `api/appointments/[id]/route.ts:109`; POS PATCH `api/pos/appointments/[id]/route.ts:275` — only `assigned_staff_id` syncs at `:301-308`, never status) never touch jobs; job complete (`complete/route.ts:87`) / start-work (`start-work/route.ts:52`) / generic PATCH (`[id]/route.ts:113`) never touch appointments; admin appt-cancel (`api/appointments/[id]/cancel/route.ts:77`) never cancels the job. **No cron writes either status** (lifecycle-engine `:668` is execution-context; booking-reminders writes only `reminder_sent_at`). **Enums not 1:1** (`AppointmentStatus` 6 `types.ts:9` vs `JobStatus` 7 `types.ts:1043`) → directional/lossy mapping. **Job creation:** `populate` (`populate/route.ts:64,160`) materializes `confirmed`/`in_progress` appts → `jobs.status='scheduled'`, dedup on UNIQUE `jobs.appointment_id` (`20260329000002_*:21`); walk-in (`pos/jobs/route.ts:389,478`) makes a synthetic `in_progress` appt + `scheduled` job (designed mismatch). **Schema cascade (Option B feasible):** delete jobs → `job_photos`/`job_addons` CASCADE (`phase8_jobs_schema.sql:80,129`), `lifecycle_executions.job_id` SET NULL (`20260428000005_*:24`); timer/intake_notes are columns on the row. **TWO non-cascade obligations:** `job_photos` Storage objects orphan (must delete explicitly); `jobs.transaction_id` (FK-out SET NULL) leaves the transaction unlinked → **block un-materialize when `transaction_id` set**. **Re-materialization invariant:** un-materialize MUST revert `appointment.status='pending'` atomically or `populate` recreates the job. **Display sources confirmed:** POS Today card + job-detail header read `jobs.status` (`job-queue.tsx:849,917`; `job-detail.tsx:790`); admin list + Phase-2B Schedule pill read `appointment.status` (`day-appointments-list.tsx:78`; `job-queue.tsx:~1128`) — so the same materialized appt shows two labels across POS scopes. **Webhooks:** appt PATCH fires `appointment_confirmed`/`_completed`; job-complete fires job-level notifications (NOT the appt webhook); job-cancel sends SMS directly (NOT the n8n webhook) — full sync (Path B) carries a HIGH webhook-duplication risk needing single-owner+suppress policy. **Permission reality ≠ seed:** runtime `appointments.cancel` granted to ALL FOUR roles (cashier+detailer), not admin-only as seeded → reusing it lets detailer hard-delete jobs (operator decision). **Three paths scoped:** A (narrow un-materialize, ~3-4 files/~250-350 lines, 1 session; does NOT fix cancelled→/completed→ divergences); B (full bidirectional sync via `lifecycle-sync.ts` mapping + wiring all writers, ~6-9 files/2-3 sessions, fixes all); C (phased: Path A now as Phase 2C behind a thin `lifecycle-sync.ts` seam + Item 15h for Path B). **Recommended: Path C** — un-materialize is a strict subset of bidirectional sync, so building it first as a shared helper is not throwaway; full sync's webhook/race complexity deserves its own session. **10 open operator decisions enumerated** (Target 12). Deploy: NO. **DO NOT merge — operator + Claude lock path + decisions, then implementation session.** |
| 2026-05-27 | 105 | Item 15e Phase 2B — mount the reused admin dialog in the POS Jobs Schedule scope + appointment-status pill (COMPLETES Phase 2) | Production code change | _(this commit)_ (branch `feat/item-15e-phase-2b-jobqueue-mount-and-wire`) | Wiring half of Phase 2, built on merged 2A (`b12148f8`). Replaces Phase 1B's placeholder toast with the full tap→fetch→edit→save→refetch workflow. **1 production file (`job-queue.tsx`) + 1 test file; NO migrations; NO new helper file (status pill kept inline per operator's call).** In `src/app/pos/jobs/components/job-queue.tsx`: (1) imports the reused dialog via `@/app/admin/appointments/components/appointment-detail-dialog` (the dual-context component parameterized in 2A) + POS `CancelAppointmentDialog` (Item 15b) + `PosAppointment`/`AppointmentWithRelations`/`AppointmentUpdateInput`/`Employee`/`AppointmentStatus` types + `APPOINTMENT_STATUS_LABELS`. (2) New state: `selectedAppointmentId`, `selectedAppointment` (typed `AppointmentWithRelations`), `detailEmployees` (`Pick<Employee,'id'|'first_name'|'last_name'|'role'>[]`), `loadingAppointment`, `cancelTarget` (typed `PosAppointment`). **Type bridge resolved without unsafe casts:** the GET payload is structurally `PosAppointment` (nested `service` nullable) but typed-in as `AppointmentWithRelations` via the `any` json boundary; the dialog reads `service?.name` so null is safe; `onCancel`'s `AppointmentWithRelations` arg is assignable to the cancel dialog's `PosAppointment` prop (non-null → nullable widening). (3) Three `usePosPermission` gates (`appointments.reschedule`/`.cancel`/`.add_notes`) — existing keys, NO new keys; server PATCH re-checks each so gates are UX-only. (4) `handleScheduleCardTap(id)` mirrors `change-time-button.tsx` (Rule 11): parallel `posFetch` of `GET /api/pos/appointments/[id]` + `/api/pos/staff/available`; on failure → error toast + reset (no mount). (5) `handleSaveAppointment(id, data)` → `PATCH /api/pos/appointments/[id]` via `posFetch`; on `ok` closes dialog + `fetchSchedule()` + success toast + returns `true` (dialog self-closes); on `!ok` error toast + returns `false` (dialog stays open). Webhook firing is server-side (2A's PATCH, Decision 2) — 2B adds NO webhook logic. (6) `handleCancelAppointment(appt)` closes detail dialog, opens POS cancel dialog (its own notify-off flow). (7) `ScheduleScopeList`: prop `onTapPlaceholder` → `onSelectAppointment:(id)=>void` + `busyAppointmentId`; tapped card gets `aria-busy` + `opacity-60 pointer-events-none` while loading; **appointment-status pill** added near the indigo "Schedule" badge via new module-level `getAppointmentStatusPillClasses(status)` (POS-local, dark-aware per Rule #10 — pending=amber, confirmed=blue, in_progress=indigo, else gray; label from `APPOINTMENT_STATUS_LABELS`). (8) Mounts `<AppointmentDetailDialog … mobileModalMode="pos" modifierVariant="pos" onEditInPos={no-op} />` (the no-op suppresses the admin deep-link button since we're already in POS) + `<CancelAppointmentDialog>`. **STATUS_DOT_COLORS NOT lifted** (Phase 2B decision — admin's text-less calendar dots vs POS's labelled pills are different visual treatments; over-coupling avoided; stays admin-only). **Flag-gated:** flag OFF → `effectiveScope` pinned to Today, toggle never renders, Schedule unreachable, no dialog can mount → byte-identical to pre-2B. **Tests** (`job-queue-schedule-scope.test.tsx`, +8; the one stale 1B placeholder-toast UI test updated to assert tap-fetches-appointment): tap→GET+staff fetch; success→dialog mounts w/ `mobileModalMode/modifierVariant='pos'` + `onEditInPos` fn (+populate-never-fires re-assert); fetch-fail→error toast + no mount; onSave→PATCH; save-success→close+refetch; save-fail→keep-open+toast; onCancel→cancel dialog opens w/ same appt; status pill per status. Dialogs mocked (capture props), mirroring the existing `JobTimeline` mock. **Phase 1B's 6 load-bearing invariant tests still pass** (Schedule scope never triggers populate). **Gates green:** `npx tsc --noEmit` 0 errors; `npm run lint` 0 errors / 97 warnings (baseline unchanged); `npm test` **2489/2489** (2481 + 8); `npm run build` clean (admin dialog compiles into the POS bundle; an initial build hit a stale-`.next` chunk error — `rm -rf .next` + rebuild was clean). Confirmed untouched: Phase 1A `schedule`/`populate` routes, Phase 2A PATCH endpoint, admin dialog + admin PATCH route, POS Appointments tab, `job-detail.tsx`, walk-in creation, D43–D50/D45–D48. **Phase 2 COMPLETE.** Deploy: NO. **DO NOT merge — operator verifies tests + diff + manual (flag-OFF regression; flag-ON tap→edit→save→cancel; webhook fire on pending→confirmed), then merges.** Phase 3 = retire POS Appointments tab + nav redirect. |
| 2026-05-27 | 104 | Item 15e Phase 2A — shared type lift + dialog parameterization + dark-mode pass + combined POS PATCH endpoint (foundation half of Phase 2) | Production code change | _(this commit)_ (branch `feat/item-15e-phase-2a-shared-lift-dialog-parameterize-pos-patch`) | First half of Phase 2 per Memory #8 split; builds the dual-context foundation so Phase 2B can mount the admin `AppointmentDetailDialog` in the POS Jobs Schedule scope. **5 production files (exactly as scoped) + 1 test file; NO migrations.** (1) NEW `src/lib/appointments/status-transitions.ts` — `STATUS_TRANSITIONS` lifted out of admin `appointments/types.ts`. (2) NEW `src/lib/appointments/types.ts` — `AppointmentService` + `AppointmentWithRelations` lifted (note: structurally equivalent to POS `PosAppointment`; convergence deferred). (3) MOD `src/app/admin/appointments/types.ts` — now re-exports the 3 lifted symbols; keeps admin-only `STATUS_DOT_COLORS` in place (NOT lifted — Phase 2B decides if the Schedule pending-pill needs it). **5 admin importers of `AppointmentWithRelations` (`admin/page.tsx`, `appointments/page.tsx`, `cancel-appointment-dialog.tsx`, `day-appointments-list.tsx`, `appointment-calendar.tsx`) confirmed UNCHANGED via the re-export; verified no external importer of `STATUS_TRANSITIONS` exists besides the dialog (the waitlist route defines its own `VALID_STATUS_TRANSITIONS`; the POS list route only mentions the type in a comment).** (4) MOD `appointment-detail-dialog.tsx` — 3 new optional props with admin defaults (`mobileModalMode='admin'` → `<EditMobileModal>`; `modifierVariant='admin'` → `<ModifierSummary>`; `onEditInPos?` → overrides the admin `router.push(returnTo=/admin/appointments)` deep-link); 2 import paths switched to shared lib; full `dark:` variant pass across the ~370-line light-only body (info `<dl>`, service rows, mobile card, enable-mobile button, cancellation block, notes textareas, Edit-in-POS button) matching existing POS gray-scale conventions (`gray-500→dark:gray-400`, `gray-900→dark:gray-100`, `bg-gray-50→dark:bg-gray-800`, `border-gray-200→dark:gray-700`, `bg-white→dark:bg-gray-900`, `bg-red-50→dark:bg-red-950`). **Admin behavior byte-identical** — props default to admin, `dark:` is a no-op outside `.dark`, types moved via re-export. iOS-zoom `text-base` bump (Risk #9) intentionally SKIPPED in 2A (it would change admin small-screen rendering → not byte-identical; revisit POS-side in 2B). (5) MOD `src/app/api/pos/appointments/[id]/route.ts` — ADDED a `PATCH` handler beside the existing GET (the brief framed it "NEW file"; the file already existed for the Item-15c GET, so PATCH was appended). Mirrors admin PATCH field-for-field with 3 deliberate diffs: **(a)** HMAC `authenticatePosRequest` + per-field `checkPosPermission` (`appointments.reschedule` for date/time/employee, `.update_status` for status, `.add_notes` for notes); **(b)** webhooks FIRE — `appointment_confirmed`/`appointment_completed`/`appointment_rescheduled` per **Decision 2** (NOT suppressed, unlike the POS reschedule/cancel endpoints); **(c)** `STATUS_TRANSITIONS` enforced server-side per **Decision 3** (e.g. `completed`→`pending` → 400; terminal states reject any change). Overlap check + jobs.assigned_staff_id sync + full joined `PosAppointment` response mirror the POS reschedule endpoint. **No new permission keys, no role-defaults migration** (Decisions 4/5). (6) NEW `__tests__/patch.test.ts` (+17) — named `patch.test.ts` to match the sibling `get.test.ts` per-method convention (brief said `route.test.ts`, which would be ambiguous next to `get.test.ts`); covers 401/400-invalid-body/400-no-fields/403×3-per-field/404/invalid-transition/valid-transition/confirmed-webhook/completed-webhook/409-overlap/rescheduled-webhook/notes-only-no-webhook/full-shape+pos-audit/detailer-sync/empty-employee-clear. **Gates green:** `npx tsc --noEmit` 0 errors; `npm run lint` 0 errors / 97 warnings (baseline unchanged); `npm test` **2481/2481** (2464 + 17); `npm run build` clean. **The new POS PATCH is INERT — nothing mounts/calls it until Phase 2B**, so POS deploy risk ≈ 0; admin PATCH route untouched. **Phase 2B (next session):** wire `ScheduleScopeList` onTap → fetch via `GET /api/pos/appointments/[id]` → mount the dialog (`mobileModalMode="pos"` `modifierVariant="pos"` `onEditInPos` no-op) → `onSave` to the new PATCH via `posFetch` → `onCancel` to POS cancel dialog → `fetchSchedule()` refresh; + Schedule status pill (decide `STATUS_DOT_COLORS` lift). Placeholder toast still shows in POS until then. Deploy: NO. **DO NOT merge — operator verifies tests + diff + manual admin byte-identical regression, then merges.** |
| 2026-05-27 | 103 | Item 15e Phase 2 — AppointmentDetailDialog reuse verification (read-only audit) | Audit done | _(this commit)_ (branch `audit/item-15e-phase-2-reuse-verification`) | Read-only feasibility audit: *if/how* the admin `AppointmentDetailDialog` can be reused in POS Jobs Schedule scope (Phase 2 "reuse admin dialog" lean). No source/migration/test changes — 1 audit doc (`docs/dev/ITEM_15E_PHASE_2_REUSE_VERIFICATION.md`) + 3 doc updates. **Verdict: `reuse-with-moderate-adjustments`.** Component reusable (save/cancel are prop callbacks → already decoupled from admin write API; only 1 of 20 imports admin-coupled: `../types`). **PATCH auth incompat:** `/api/appointments/[id]` uses cookie Supabase auth (`getEmployeeFromSession`→`auth.getUser()`), which POS (HMAC `X-POS-Session`) never satisfies → 401; admin route NOT POS-reusable, broadening not recommended. **Only real server work:** POS has reschedule/cancel/services/mobile/load but NO status-change + NO notes endpoint → recommend one combined `PATCH /api/pos/appointments/[id]` (`authenticatePosRequest` + per-field `checkPosPermission`). **No role-defaults migration** — `appointments.update_status`/`add_notes`/`reschedule` already grant correct matrix to POS roles; `checkPosPermission` reads same `permissions` table. **Phase 1B already compatible** — Schedule scope's `ScheduleScopeList` overrides the List/Timeline `viewMode` toggle (not even rendered in Schedule scope), so Phase 2 needs no Timeline support for Schedule rows. **Mechanical adaptation:** parameterize 3 hardcoded admin assumptions ("Edit in POS" `router.push(returnTo=/admin/appointments)`; `EditMobileModal mode="admin"`; `ModifierSummary variant="admin"` — latter two already support `'pos'`); add `dark:` variants to light-only dialog body (Rule #10 — no-op in admin, no regression); lift `STATUS_TRANSITIONS`+`AppointmentWithRelations` to `src/lib/appointments/` with admin re-export (low blast radius — only dialog imports `STATUS_TRANSITIONS`; 5 admin files import the type, re-export keeps them untouched). **Template:** `pos/jobs/components/change-time-button.tsx` already fetches full `PosAppointment` from `GET /api/pos/appointments/[id]` + mounts a reused dialog inside Jobs. **Pending pill:** `PosScheduleEntry.status` already present (no endpoint widening). **Honest fallback:** slim POS dialogs (`reschedule-/cancel-appointment-dialog.tsx`) already POS-wired/dark/in-Jobs — less work, narrower (reschedule+cancel only). Est ~3–4.5h, likely own session (Memory #8). 3 open operator decisions: rich-reuse-vs-slim, POS status webhook policy, allowed status transitions on upcoming rows. **DO NOT merge — operator + Claude lock Phase 2 plan from findings.** |
| 2026-05-27 | 102 | Item 15e Phase 1B — client wiring: Today/Schedule scope toggle + populate gate + feature flag (COMPLETES Phase 1) | Production code change | _(this commit)_ (branch `feat/item-15e-phase-1b-client-wiring`) | Client-side completion of Phase 1, built on merged 1A (`ab59d9a3`). Wires 1A's Schedule endpoint into the POS Jobs queue behind feature flag `pos_jobs_unified_schedule` and lands the CLIENT-SIDE load-bearing invariant: Schedule scope NEVER triggers populate. **2 production files + 1 seed migration + 1 test file** (within Memory #8). (1) MOD `job-queue.tsx` — Today/Schedule scope toggle (mirrors view-mode toggle; `scope` persisted to localStorage `pos-jobs-scope`; rendered only when flag enabled); `effectiveScope = scheduleScopeEnabled ? scope : 'today'` so flag-OFF is byte-identical to pre-1B; Schedule scope hides date-nav + summary + filter pills + view toggle; read-only `ScheduleScopeList` renders `PosScheduleEntry` rows (same card visual language, "Schedule" badge, NO job chrome) with a Phase-2 placeholder toast on tap (no state mutation). **THREE defense layers on the populate gate, all `scope==='today'`-conditional:** (A) `init()` useEffect → Schedule calls `fetchSchedule()` + returns, never populate; (B) Refresh button → Schedule re-fetches endpoint, no populate; (C) function-level guard atop `populateFromAppointments` (`scopeRef.current !== 'today'` early-return) — refuses to materialize even if invoked outside the gated effect. `pollJobs` + interval also gated (no live-poll in Schedule). `scopeRef` keeps `scope` out of the populate callback's deps (no stale-closure churn on the dedup set). (2) MOD `constants.ts` — `FEATURE_FLAGS.POS_JOBS_UNIFIED_SCHEDULE`. (3) NEW migration `20260527000000_pos_jobs_unified_schedule_flag.sql` — DATA seed only (flag row, enabled=false, `ON CONFLICT DO NOTHING`); NO schema change. (4) NEW `job-queue-schedule-scope.test.tsx` (+9): 6 load-bearing invariant tests (Schedule mount/toggle/refresh never populate; Schedule→Today DOES populate; date-nav hidden in Schedule; flag-OFF pins Today) + 3 scope-toggle UI tests; CRITICAL-INVARIANT header comment present. **Gates green:** tsc 0 errors; lint 0 errors / 97 warnings (baseline unchanged); `npm test` **2464/2464** (2455 + 9); build clean. Confirmed untouched: admin appointment dialog, POS Appointments tab UI, GET `/api/pos/appointments`, walk-in creation, `job-detail.tsx`, D43–D50, D45–D48 helpers; no new permission keys. **Phase 1 (1A + 1B) COMPLETE.** Deploy: NO. **DO NOT merge — operator verifies tests + diff + manual (flag OFF regression; flag ON Today/Schedule), then merges.** Phase 2 = Schedule reschedule/cancel + lazy-materialize + `update-appointment.ts` extraction. |
| 2026-05-27 | 101 | Item 15e Phase 1A — Schedule scope endpoint + populate gate (server foundation) | Production code change | _(this commit)_ (branch `feat/item-15e-phase-1a-schedule-endpoint-and-populate-gate`) | First implementation phase of the Item 15e retire arc, SPLIT from the planned Phase 1 per Memory #8 (full Phase 1 = 5 production files / ~345 lines, exceeds the >3-files/>300-lines threshold; operator approved 1A-now / 1B-next). **1A = server foundation only.** Three production files: (1) NEW `GET /api/pos/jobs/schedule` (`src/app/api/pos/jobs/schedule/route.ts`) — pure-read endpoint returning FUTURE appointments (default tomorrow→+30d, hard floor clamps any `from` ≤ today up to tomorrow) excluding cancelled/no_show/completed and excluding already-materialized appointments (two-step appointments→jobs dedup, mirroring populate); `appointment_services` SELECT includes `quantity` proactively (D48) so Phase 2 needs no widening; perm `appointments.view_today` (Decision #13, no new keys); ZERO `jobs` writes. (2) MOD populate gate (`src/app/api/pos/jobs/populate/route.ts`) — short-circuits BEFORE any DB query when `targetDate > getTodayPst()`, logs `[populate] skipped — future date {date}`, returns `{created:0, skipped:'future_date'}`; inline today-default DRYed to `getTodayPst()` so gate + default share one source (no behavior change). (3) NEW type `PosScheduleEntry` (`src/app/pos/jobs/components/schedule-types.ts`, `scope:'schedule'` discriminator) — placed beside `job-queue.tsx`/`JobListItem` (brief's `pos/components/jobs/` path does not exist). +20 tests (12 schedule + 8 populate-gate), both files include the CRITICAL-INVARIANT comment; server-side half of the load-bearing populate gate is fully covered (future date → zero DB touch / zero upsert; today/past → materializes). **NO feature flag, NO client changes, NO `job-queue.tsx` touch in 1A** — the new endpoint is inert until Phase 1B wires the scope toggle, so flag-OFF behavior is moot/unchanged. Corrected the brief's `:411-413` populate-trigger ref: the primary trigger is the `init()` useEffect at `job-queue.tsx:380-386` (dedup `:357-358`), not the Refresh button — Phase 1B gates both. **Gates green:** `npx tsc --noEmit` 0 errors; `npm run lint` 0 errors / 97 warnings (baseline unchanged); `npm test` **2455/2455** (2435 + 20); `npm run build` clean (`/api/pos/jobs/schedule` compiled as dynamic route). NO migrations. Confirmed untouched: admin appointment dialog, POS Appointments tab UI, GET `/api/pos/appointments`, walk-in creation logic (`pos/jobs/route.ts:383-415`), `job-detail.tsx`, D43–D50 SMS-AI v2, D45–D48 helpers. **Phase 1B (next session):** `job-queue.tsx` Today/Schedule scope toggle + `pos_jobs_unified_schedule` feature flag + the client-side load-bearing invariant test (Schedule scope never triggers populate). Deploy: NO. **DO NOT merge — operator verifies tests + diff, then merges; Phase 1B branches off merged 1A.** |
| 2026-05-27 | 100 | Item 15e (new framing) audit — POS Jobs Unified Operations View + retire-vs-complete evaluation | Audit done | _(this commit)_ (branch `audit/item-15e-pos-jobs-unification`) | Read-only diagnostic audit. NO `src/` changes, NO migrations, NO test changes. Deliverable: `docs/dev/ITEM_15E_POS_JOBS_UNIFIED_OPERATIONS_AUDIT.md` (Phase 0 retire-vs-complete + Targets 1–12 + risk matrix). Second audit for Item 15e under the reframed scope; references the preserved capability inventory at `ITEM_15E_ADMIN_APPOINTMENT_CAPABILITY_REFERENCE.md` rather than re-deriving it. **Pivotal finding:** the unification is ~70–80% ALREADY BUILT — POS Jobs is already the day-of surface for booked appointments AND walk-ins. Every job is 1:1-linked to an `appointments` row (`jobs.appointment_id`); walk-ins are synthetic `channel='walk_in'` appointments (`api/pos/jobs/route.ts:383-415`); booked appointments auto-materialize into jobs (`populate/route.ts:40-55`); the Jobs queue merges both with an origin badge (`job-queue.tsx:654-665`); `job-detail.tsx` (1968 lines) already exposes status/mobile/modifier/payment-mismatch/notes/customer-vehicle/reassign/payment-link/cancel/change-time. The real problem is DUPLICATION: both `/pos/jobs` + `/pos/appointments` are sibling tabs (`bottom-nav.tsx:196-206`) and the Appointments list has no channel filter (`api/pos/appointments/route.ts:83`), so every walk-in + every populated booked appointment double-surfaces. **Leans (operator locks in Target 12):** (0) RETIRE + ABSORB; re-home the forward-looking multi-day schedule as a Jobs "Upcoming" scope (the one real gap — must NOT prematurely materialize jobs); (3) view model = lifecycle-segmented + origin badge/filter + Today/Upcoming scope; (4) ONE polymorphic detail = extend `job-detail.tsx`, NOT a new `<PosAppointmentDetailDialog>` (that reference-audit recommendation is now OBSOLETE); (6) service-edit stays on canonical POS ticket flow (Item 15f Layer 8e); (7) WK Sessions 2–4 run in PARALLEL (surfaces adjacent — WK orphans are receipt transactions, not job walk-ins which already carry customer_id); (10) soft-deprecate the tab (flag → banner → redirect → remove), NO data migration. **Biggest delta from reference audit:** most of its "NEW work" (`update-appointment.ts` extraction, new dialog, helper mounting) is already done in job-detail or re-scoped away — POS drives `job.status`, not `appointment.status` (Target 12 #9 confirms whether ANY appointment-status editing is needed in POS). 5-phase plan ~9–14h CC if retire wins; ~13 operator decisions. Central technical RISK: premature job materialization when navigating Jobs to future dates — Phase 1 must gate `populate` to today/active. D48 quantity fold-in narrowed to the upcoming-appointment detail path only (job-detail renders `job.services` JSONB, not `appointment_services`). Gates: none — doc-only. Deploy: NO. **DO NOT merge — operator reviews + locks Target-12 decisions before implementation fires.** |
| 2026-05-27 | 99 | Item 15e reframing prep — rename audit to capability reference + ROADMAP scope update (15e becomes 'POS Jobs Unified Operations View'; 15d absorbed) | Documentation update | _(this commit)_ (branch `docs/item-15e-reframe-prep`) | Doc-only session. Reframes Item 15e from "POS Appointments Modal Parity" to "POS Jobs Unified Operations View" per operator direction (2026-05-27 conversation, immediately following the original 15e audit merge `1927d4b2`). Renames `docs/dev/ITEM_15E_POS_APPOINTMENTS_MODAL_PARITY_AUDIT.md` → `docs/dev/ITEM_15E_ADMIN_APPOINTMENT_CAPABILITY_REFERENCE.md` (via `git mv` for full history preservation); PREPENDS preamble note documenting which sections remain valid (Targets 1, 3, 5, 8 — capability inventory + reuse patterns + D48 interactions + walk-in baseline) vs. obsoleted (Targets 2, 6, 7, 9, 10, 11 — gap analysis + 15c interaction + 15d framing + implementation plan + verification plan + operator decisions). ROADMAP updated: Item 15e renamed to "POS Jobs Unified Operations View" with "🔍 auditing" status preserved; Item 15d marked 🔀 absorbed; Roll-up updated to reflect absorption (15e moved from Not-started to In-progress; 15d Deferred→Absorbed; Total-items line deferred→absorbed). New audit `ITEM_15E_POS_JOBS_UNIFIED_OPERATIONS_AUDIT.md` lands in session #100 (next CC session). No source code changes, no migrations, no test changes. Gates: none — doc-only. Deploy: NO. **DO NOT merge — operator reviews + merges.** |
| 2026-05-27 | 98 | Item 15e audit — POS Appointments Modal capability parity with Admin | Audit done | _(this commit)_ (branch `audit/item-15e-pos-appointments-modal-parity`) | Read-only diagnostic audit. NO `src/` changes, NO migrations, NO test changes. Deliverable: `docs/dev/ITEM_15E_POS_APPOINTMENTS_MODAL_PARITY_AUDIT.md` (11 targets + 27-row capability parity matrix + 3-phased implementation plan + 11 operator decisions). **Findings:** Admin Appointment Detail Dialog = 17 mutable + 9 readonly capabilities; POS Appointments view = 5 (date/start/end/detailer/cancel). Net parity gap = 13 ADDs + 1 conditional. **Architecture:** Pattern C across the board (shared lib function consumed by both Admin + POS routes); 5 helpers already shared (`<EditMobileModal>`, `<ModifierSummary>`, `<PaymentMismatchBanner>`, `composeLineItems`, `formatChannelLabel`); new work needed: extract `/api/appointments/[id]/route.ts:11-187` admin PATCH body into `src/lib/appointments/update-appointment.ts` + add POS sibling endpoint at `/api/pos/appointments/[id]/route.ts` PATCH. **16th D46/D48 visual surface flagged:** 3 POS endpoint SELECTs (`/api/pos/appointments`, `/api/pos/appointments/[id]`, `/api/pos/appointments/[id]/reschedule` response) need `quantity` widening — fold-in proposed as Phase A. **Item 15e original Q5 reconciliation:** roadmap called for `useServicePicker` hook in-dialog; conflicts with Item 15a/15f Layer 8e closure (bespoke modal deleted; canonical service-edit = POS deep-link). Audit recommends Option A: "Edit Services in Sale" deep-link button → `/pos?source=appointment&id=…&returnTo=/pos`. **Phased plan (~6-9 hours CC):** Phase A endpoints + helper extraction + D48 fold-in; Phase B `<PosAppointmentDetailDialog>` UI rebuild; Phase C service-edit affordance + integration tests + closing. **Item 15c interaction:** preserved — `<RescheduleAppointmentDialog>` stays for POS Jobs Card narrow entry; POS Appointments adopts new full detail dialog. **Workstream K interaction:** zero conflict — POS adopts shared `formatChannelLabel(channel, 'pos')` for walk-in pill; WK Sessions 2-4 untouched. **Item 15d implication:** 15d stays deferred; 15e ships the appointment detail surface 15d's row click will route to. **Operator decisions (11 surfaced):** modal scope lock; notify-on-status-change policy (α recommended: default OFF symmetric with Item 12 + 15b); manual Send Confirmation (defer to separate audit); service-edit affordance (Option A deep-link); D48 fold-in timing (include in Phase A); `STATUS_TRANSITIONS` lift to `src/lib/appointments/`; admin channel-label cleanup (defer); 15d revival call (keep deferred); POS Jobs Card regression check; status permission key (mirror admin `appointments.update_status`); iPad responsive layout (single-col mobile, sm:2-col landscape). **Captured for future (NOT 15e scope):** Admin Dialog tier-render gap (`appointment-detail-dialog.tsx:289-313` missing `attachTierMetaToItems` + `renderTierToken`); Admin channel-label local map drift (`:37-42`); `useServicePicker` hook adoption-gap visibility. **Hard rules honored:** no `src/` changes, no migrations, no test changes, every finding cites `file:line` evidence, audit produces options for Target 11, NOT a pre-picked plan; no admin-side capability recommendations (one-direction audit). **Gates:** none — doc-only. **Deploy: NO.** **DO NOT merge — operator reviews + locks decisions before implementation fires.** |
| 2026-05-27 | 97 | ROADMAP bump v3.9 → v4.0 — captures SMS-AI v2 fidelity arc Issues 39-46 closure + Issue 47 + misleading-docstring follow-up | Documentation update | _(this commit)_ (branch `docs/roadmap-bump-v4-0`) | Doc-only session. Updates `docs/dev/ROADMAP-13-ITEMS.md`: header version v3.9 → v4.0; Snapshot table date 2026-05-19 → 2026-05-27; "Last session updated" stamp refreshed; Total items roll-up corrected (15f + 15g now reflected — 9 done / 1 closed / 1 deferred / 10 not-started); Workstream A SMS AI v2 status updated (fidelity arc 26-46 closed via D43-D50; Layer 5-6 + admin UI wiring next); Issue 47 captured (soft-friction "Want to go with that?" pattern, MONITOR status); misleading docstring at `system-prompt.ts:12` captured as doc-fix follow-up. No source code changes, no migrations, no test changes. Gates: none — doc-only. Deploy: NO. **DO NOT merge — operator reviews + merges.** |
| 2026-05-27 | 96 (D50) | D50 — Issue 45 follow-up — universal prohibition of "Want me to send a quote?" friction question | Production code change | _(this commit)_ (branch `fix/issue-45-friction-question-universal-prohibition`) | Small surgical follow-up to D49 driven by empirical evidence from operator's 2026-05-27 verification. D49 closed Issue 45 architecturally (Critical Rule 17 + auto-send + tool description retighten), but Scenario 1 verification revealed the friction question "Want me to send you a quote?" re-emerging at **discovery-phase** after an add-on pitch. Root cause: D49's prohibition was scoped to the moment when auto-send preconditions weren't met, leaving a discovery-phase loophole. **Empirical evidence:** operator's Scenario 1 conversation `Customer: "How much to wash Exterior on my 2018 Suburban" → Agent: "Express Exterior Wash for your 2018 Suburban is $110. Includes... Engine Bay Detail bundles in for $125 ($50 off) if you want that added. Want me to send you a quote?"` (friction question after discovery-phase add-on pitch). **All 6 operator-locked decisions implemented:** (1) approach = universal-scope prohibition (no contextual limits); (2) wording principle = "either preconditions met (auto-send) OR continue discovery without asking permission"; (3) Rule 20 example refactored from "if you want" → "if you'd like to add it" (Refactor A; Refactor B reserved as escalation); (4) scope = Rule 17 + Rule 20 example only (no new rules, no renumber); (5) explicit test coverage for universal prohibition + discovery-phase invariants; (6) operator's universal-scoping principle captured as standing rule in observations doc Section 1. **System-prompt changes (`src/lib/sms-ai/system-prompt.ts`):** (1) Rule 17 tightened at lines 253-270 — single-sentence prohibition replaced with expanded block including universal-scope clause ("in ANY conversational position — not at discovery-phase, not at close-phase, not after an add-on pitch, not anywhere"), two-path framing (Preconditions met → auto-fire; Preconditions NOT met → continue discovery without elicitation), ❌ WRONG discovery-phase example verbatim from operator's 2026-05-27 verification, ✅ RIGHT counterpart showing correct pattern (present pricing + add-on, no permission ask). (2) Rule 20 add-on example at line 356 refactored — "if you want" → "if you'd like to add it" — with inline operator-facing parenthetical explaining the rationale (pattern-matchable hook removed) and cross-referencing Critical Rule 17. D49's three preconditions UNCHANGED. D49's Pattern 3 example + Issue 27 safety + Rule 16 cross-ref + auto-send reply phrasing all PRESERVED. **Latent permission-asking cue audit:** grep for `if you want|if you'd like|would you like me|want me to|should I` returned 2 matches — `:335` Rule 20 example REFACTORED; `:596` last name capture "What name should I put on the quote?" PRESERVED (asks for INFORMATION not permission). No other matches. **Friction-question deletion verified (post-D50):** 6 grep matches across `system-prompt.ts` (5) + `tools.ts` (1), ALL in forbidding contexts. D49 invariant preserved + 2 D50 additions in forbidding contexts (Rule 17 ❌ WRONG example + Rule 20 refactor explanation). Zero prescriptive matches. **Tests +8 net new:** 7 new D50 system-prompt tests (universal clause language + two-path framing + ❌ WRONG example + ✅ RIGHT example + "commitment arrives naturally" pin + Rule 20 refactor pin + friction-question variant-form forbidding-context invariant) + 1 canonical-form invariant test (preserves D49 count=3 + documents why D50 variant additions use "send you a quote?") + 1 pre-existing D49 test substring relaxed ("that friction step is deleted" → "friction step is deleted" — D50 capitalized sentence start). **D45/D46/D47/D48/D49 surfaces UNAFFECTED:** helper files byte-identical; 15 D46 visual surfaces byte-identical; D47 Rules 8 + 9 + `get_services` response shape byte-identical; D48 schema + 4 customer-facing surfaces byte-identical; D49 Rule 17 three preconditions + Pattern 3 + Issue 27 safety + auto-send reply phrasing all preserved; `NOTIFICATION_LABEL_OVERRIDES` byte-identical; no renumbering (22 Critical Rules unchanged in count + position); no new rules; no new tools; no endpoint changes; no observability log changes; no admin UI changes; no migrations. **Operator's prompt-discipline universal-scoping rule captured** in `docs/dev/SMS_AI_V2_PROMPT_OBSERVATIONS.md` Section 1: "When fixing emergent LLM behaviors, define the prohibition at the universal level (NEVER do X anywhere) — not the contextual level (NEVER do X at this specific moment). LLMs find loopholes in contextual prohibitions." D49 → D50 is the canonical case study. **Gates green:** `npx tsc --noEmit` 0 errors, `npm run lint` 0 errors / 97 warnings (baseline unchanged), `npm test` **2435/2435** (was 2427 pre-D50; +8 net new), `npm run build` compiled successfully in 15.0s (788 dynamic pages). **Deploy: YES** via `deploy-smartdetails` post-merge — observable on next discovery-phase conversation. **Customer SMS body content UNCHANGED:** quote link arrives the same way; only the agent's discovery-phase conversational behavior changes (no more friction question after pricing + add-on presentation). **Operator manual verification (2 scenarios per session brief):** (A) discovery-phase friction-question SHOULD NOT fire — customer asks "How much for Express Exterior Wash on my 2018 Suburban?" → agent replies pricing + add-on with "if you'd like to add it" → agent ends turn WITHOUT friction question (BOTH "send a quote" AND "send you a quote" variants must be absent); (B) close-phase auto-send STILL works — customer follows up "Cool, send it" → agent auto-fires with "Sending the quote now — check your texts!" PM2 shows `[SmsAiV2.send_quote_sms.auto_send_trigger]`. Both pass → D50 successful. If Scenario A still shows friction question post-deploy, escalate to Refactor B (drop Rule 20 example conditional language entirely) in a follow-up. **DO NOT merge — operator merges after verifying tests + reading the diff.** Issue 45 now closed via D49+D50 sequence. |
| 2026-05-27 | 95 (D49) | D49 — Option A proactive auto-send for send_quote_sms (CLOSES Issue 45) | Production code change | _(this commit)_ (branch `fix/issue-45-auto-send-confirmation`) | Implementation session for the Issue 45 audit (`docs/dev/ISSUE_45_AUTO_SEND_AUDIT.md`, audit branch merged at `d5efe521`). Single combined session per audit Target 10. **Closes Issue 45** — eliminates the redundant "Want me to send a quote?" friction step that added 30-90 sec funnel latency per quote-send flow with 100% pass-through rate in operator's 4 captured Q-tests (Q-0086/Q-0087/Q-0090/Q-0091). **All 7 operator-locked decisions implemented:** (1) Option A proactive auto-send; (2) reply phrasing "Sending the quote now — check your texts!" tool-result-agnostic per Issue 27 safety; (3) friction step DELETED entirely; (4) 7-pattern matrix encoded; (5) observability log added; (6) Rule slot 17 with renumber 17-21 → 18-22; (7) three preconditions enforced (commit signal + total stated + no mid-flux). **System-prompt changes (`src/lib/sms-ai/system-prompt.ts`):** new Critical Rule 17 at lines 221-255 (~35 LOC: 3 preconditions, reply phrasing, Pattern 3 conditional-commitment example, Issue 27 post-failure correction clause, double-mention of "NEVER ask 'Want me to send a quote?'" prohibition, Rule 16 architectural-parallel cross-ref); Rule 2 ✅ RIGHT example updated (`Cool, send it` + `Sending the quote now` — past-tense `Quote sent!` removed); renumber 17-21 → 18-22 (Never pitch mobile → 18; After notify_staff → 19; Tool-grounded add-ons → 20; Quote first → 21 with trigger reworded to reference Rule 17; instructions_for_agent → 22); 5 inline cross-references mechanically updated (`:93`, `:132`, `:205`, `:322`, `:395`); tool usage guide trigger reworded to configuration-finalized framing; Quote-send intent recognition paragraph reframed (commit phrases are examples; implicit commitment qualifies; reminder of 3 preconditions); Booking flow Steps 1-3 reworded (Step 1 references Rule 17 + forbids friction question, Step 2 calls out same-turn pairing, Step 3 explains tool-result-agnostic rationale; old "Sent the quote to your phone — tap the link" canonical handoff removed). **Tool description retighten (`src/lib/sms-ai/tools.ts:231`):** "AND **asked to be texted a quote**" clause DELETED (the strongest of 6 latent cues per audit Appendix A); new "WHEN TO CALL (Issue 45 / D49, 2026-05-27)" paragraph references Rule 17 preconditions, forbids friction question, pins reply phrasing + Issue 27 safety; D43 multi-tier guidance paragraph preserved unchanged. **Observability log (`src/lib/sms-ai/tool-dispatcher.ts:500-518`):** new `[SmsAiV2.send_quote_sms.auto_send_trigger]` console.log line capturing conversationId, servicesCount, services, tiers, quantities, sizeClass. Informational only — does NOT gate dispatch. Enables post-deploy operator visibility for false-fire monitoring. RuntimeContext shape UNCHANGED (extending to include in/outbound messages would require runner refactor, out of D49 scope). **D45/D46/D47/D48 + Issue 46 surfaces UNAFFECTED:** helper files byte-identical; 15 D46 visual surfaces byte-identical; D47 Rules 8 + 9 + `get_services` response shape byte-identical; D48 schema + 4 customer-facing surfaces byte-identical; `NOTIFICATION_LABEL_OVERRIDES` byte-identical; no endpoint changes; no new tools; no migrations. **Other-tool regression check passed:** auto-send rule scope-limited to `send_quote_sms` only; `create_appointment` + `send_info_sms` + `approve_addon` + `decline_addon` retain "Only call when explicitly confirmed" gate. Side-effecting-tools gate loop in tools.test.ts updated to exclude `send_quote_sms` (new framing); other 2 tools unchanged. **D36 + D43 idempotency preserved:** 60-sec guard + `(service_id, tier_name, quantity)` triple-key fully cover auto-send double-fire scenarios per audit Target 6. NO guard tuning needed. **Friction-step deletion verified:** 4 grep matches remain, ALL in forbidding contexts (2 in Rule 17 body, 1 in Booking flow Step 1, 1 in tools.ts description). Zero prescriptive matches. **Tests +20 net new:** 15 new D49 system-prompt tests (Rule 17 headline + placement + 3 preconditions + reply phrasing + Pattern 3 + Issue 27 safety + friction-question deletion + Rule 16 cross-ref + Rule 2 update + tool usage guide reword + Booking flow Step 1 + intent recognition reframe + only-in-forbidding-contexts pin + rule count = 22) + 6 new D49 tool tests (Rule 17 reference + 3 preconditions + friction-question forbidden + auto-send reply + clause removed + D43 multi-tier preserved) + 3 pre-existing tests updated to reflect D49 changes (rule count 21 → 22; Booking flow line-wrap; canonical handoff line) + 8 pre-existing renumber-pin tests updated (Rule 21 → 22 in 5 sites; Rule 19 → 20 cross-ref; Rule 20 → 21 trigger; Booking flow cross-ref pin) + 2 pre-existing tests adjusted (side-effecting-tools gate loop excludes send_quote_sms; D43 regression-safety pin updated to D49 framing). **Gates green:** `npx tsc --noEmit` 0 errors, `npm run lint` 0 errors / 97 warnings (baseline unchanged), `npm test` **2427/2427** (was 2407 pre-D49; +20 net new), `npm run build` compiled successfully in 12.0s (788 dynamic pages). **Deploy: YES** via `deploy-smartdetails` post-merge — observable immediately on next quote-send conversation. **Customer SMS body content UNCHANGED:** quote link arrives the same way; only the agent's pre-tool acknowledgment changes from "Want me to send a quote?" → [customer "Yeah"] → "Quote sent! ..." (4-message flow) to "Sending the quote now — check your texts!" (1-message flow, ~30-90 sec funnel reduction). **Operator manual verification (7 scenarios per audit Target 11):** (1) golden auto-send commit+total+no mid-flux; (2) Pattern 3 mid-flux do-NOT-fire; (3) mid-correction don't-fire; (4) customer-question don't-fire; (5) cancellation don't-fire; (6) multi-service finalization auto-fire; (7) implicit commitment ("Cool, I'll take it") auto-fire. All 7 pass → Issue 45 closed empirically. **DO NOT merge — operator merges after verifying tests + reading the diff.** Issue 45 was the last open SMS-AI prompt-discipline P2; all prior issues (42 + 43 + 44 + 46) already closed. |
| 2026-05-27 | 94 | Issue 45 audit — auto-send vs. "Want me to send a quote?" confirmation step | Audit done | _(this commit)_ (branch `audit/issue-45-auto-send-confirmation`) | Read-only diagnostic audit. NO `src/` changes, NO migrations, NO test changes. Deliverable: `docs/dev/ISSUE_45_AUTO_SEND_AUDIT.md` (12 targets + TL;DR + 3-option side-by-side + 7-pattern adversarial matrix + risk matrix + 4 operator decisions). **Root cause confirmed via source-side reading:** the system prompt does NOT explicitly require the "Want me to send a quote?" step anywhere. The friction is **emergent** from 5 latent cues that all key the `send_quote_sms` trigger on customer-spoken affirmations (Critical Rule 2 ✅ RIGHT example at `system-prompt.ts:82-89`; Critical Rule 20 at `:227`; tool usage guide at `:286`; Quote-send intent recognition at `:291`; Booking flow Step 1 at `:501`) plus a sixth tool-level cue at `tools.ts:231` ("Only call this when... **asked to be texted a quote**"). All 6 teach the LLM to wait for an explicit customer ask; the LLM's safety-via-confirmation instinct elicits one via "Want me to send a quote?" **Empirical evidence:** 4 captured Q-tests (Q-0086/Q-0087/Q-0090/Q-0091, 2026-05-25 evening) all showed 100% customer-confirm rate at the friction step + 0% actionable signal + 30-90 sec added funnel latency (worst-case unbounded). Sample-size caveat captured. **3 grounded fix options surfaced (audit's structural leaning toward A is NON-BINDING — operator decides):** **A — Proactive auto-send** (Critical Rule + `tools.ts` description retighten dropping "AND asked to be texted a quote"; LLM judges "configuration finalized"; max funnel improvement); **B — Word-list-gated auto-send** (explicit commit-phrase list, conservative); **C — Soft-send rephrase** (announce-first-act-second; minimum behavioral change). **Critical Rules numbering plan:** insert as Rule 17 (immediately after Rule 16 "Don't double-act" — side-effect-discipline cluster contiguous); renumber 17-21 → 18-22; 5 inline cross-references identified for mechanical update (`system-prompt.ts:93, :132, :205, :286, :359`). **D36 + D43 idempotency interaction analyzed:** existing 60-sec guard + `(service_id, tier_name, quantity)` triple-key fully cover auto-send double-fire scenarios. NO guard tuning needed. Watch-item flagged: quote supersession (Q-N marking when Q-N+1 lands) is pre-existing behavior, out of Issue 45 scope. **Other-tool regression check:** new rule MUST name `send_quote_sms` explicitly — generic "auto-fire side-effecting tools" wording would cascade to `send_info_sms` (spam risk) and `create_appointment` (operator-locked friction). Other 4 side-effecting tools' "Only call when explicitly confirmed" pattern intentional, unchanged. **Customer-facing reply timing:** runner architecture (`agent-runner.ts:326-414`) already supports tool fire + text reply in same agent turn — NO runner change needed. Recommended reply phrasing: tool-result-agnostic ("Sending the quote now — check your texts!") to mitigate Issue 27 regression risk. **7-pattern adversarial coverage:** all 3 options correctly NOT-fire on Patterns 1/3/4/6/7 (exploratory query, conditional commitment with trailing change-request, mid-correction, customer-question, cancellation). Patterns 2 (implicit commitment) and 5 (multi-service finalization) diverge per option strategy. Pattern 3 = highest-risk wording fidelity item — rule MUST require absence of mid-flux signal as precondition. **Implementation scope estimate:** single session, ~60-90 min, ~120-260 LOC, +4-12 tests. Files: `src/lib/sms-ai/system-prompt.ts` + `src/lib/sms-ai/tools.ts` (Option A only) + corresponding test files. NO migrations. NO endpoint changes. NO new tools. NO D45/D46/D47/D48 surface impacts. **Operator decisions still needed (4 well-framed):** (1) pick A / B / C; (2) auto-send reply phrasing (tool-result-agnostic favored); (3) preserve "Want me to send?" as fallback or delete entirely (audit leans DELETE); (4) sanity-check 7-pattern matrix. **Hard rules honored:** no `src/` changes, no migrations, no test changes, only new file = audit deliverable + 3 doc updates, every finding cites `file:line`, 3 options surfaced without picking winner, D38/D43/D45/D46/D47/D48 architecture preserved. **Gates:** none — doc-only audit. `git diff --name-only` shows exactly 4 doc files. **Deploy: NO** — implementation session (D49) will deploy once Decisions #1-3 lock. **DO NOT merge — operator reviews + locks decisions before D49 fires.** Issue 45 implementation can fire after operator decision-lock. |
