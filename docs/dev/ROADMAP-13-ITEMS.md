...
|---|---|---|---|---|---|
| 2026-05-26 | 83 | Issue 39 — `{services}` SMS-chip composition audit for multi-tier same-service quotes (read-only) — Option A (shared helper) recommended; Issue 40 (tier_label data) + Issue 41 (visual surfaces cross-cutting) flagged as separate concerns | Audit done | _(this commit)_ (branch `audit/issue-39-services-chip-composition`) | Read-only diagnostic audit. No source code, no migrations, no test changes. **Failure surfaced 2026-05-25 ~14:25 PT** (operator test from `+13107564789`, immediately after the Issue 38 D43 Session C closure): 2018 Suburban customer asked for seat cleaning, agent quoted floor_mats + per_row × 2 = $325, customer confirmed. `send_quote_sms` correctly persisted 2 `quote_items` (per Pattern X) but the SMS preview rendered `"Here's your quote from Smart Details Auto Spa for Hot Shampoo Extraction, Hot Shampoo Extraction: https://…"` — duplicated service name. Underlying quote data correct; bug is purely in the `{services}` chip composition. **Deliverable** `docs/dev/ISSUE_39_SERVICES_CHIP_AUDIT.md` (10-target audit + TL;DR + risk matrix + empirical reproduction + appendix with full live-DB SQL output). **Root cause:** every consumer of the `{services}` chip composes it via `items.map(i => i.item_name).join(', ')` — no view of `tier_name` / `tier_label` / `qty_label`, so multi-tier same-service rows produce duplicated tokens. Identified **6 chip-composing call sites:** `send-quote-sms/route.ts:532`, `quotes/[id]/accept/route.ts:121`, `book/route.ts:686`, `pos/jobs/[id]/cancel/route.ts:212`, `voice-agent/appointments/route.ts:311`, `convert-service.ts:202`. Identified **5 active `sms_templates` referencing `{services}`** in the DB: `quote_sms_midcall`, `booking_confirmed`, `booking_staff_notify`, `quote_accepted_staff_notify`, `appointment_cancelled`. **Recommends Option A** — new shared formatter `src/lib/quotes/services-summary.ts` (~80 LOC) mirroring Session 71's `line-item-pricing.ts` extraction pattern, adopted at all 6 call sites; byte-identical output for non-multi-tier cases. **Multi-tier rendering rule (operator-locked decisions 1-6, all verified implementable):** qty>1 → `${qty} ${pluralize(qty_label)}` (e.g. "2 Rows"); qty=1 → `tier_label` (e.g. "Per Seat Row" / "Carpet & Mats"); sort by `total_price DESC`, tie-break `display_order ASC` (verified against operator example `Hot Shampoo Extraction (2 Rows + Floor Mats)` — `$250 > $75`). Parens emitted when (a) >1 quote_item for the service, (b) any qty>1, OR (c) `pricing_model='scope'` with informative tier_label (operator decision 5). `vehicle_size` / `specialty` single-tier qty=1 OMIT parens (operator decision 6). **tier_label vs qty_label (deferred operator question, RESOLVED empirically):** Option X — qty=1 reads `tier_label`, qty>1 reads `${qty} ${pluralize(qty_label)}`. Live DB shows ONLY `Hot Shampoo Extraction.per_row` carries `qty_label='row'` + `max_qty=3`; every other tier has NULL qty_label. Simple `+s` pluralization sufficient (no irregulars in catalog). **Blast radius today: 1 multi-tier same-service quote** (Q-0084 retest from 2026-05-25 14:28 PT). Hot Shampoo Extraction is the SOLE multi-tier-same-service surface in the live catalog (`pricing_model='scope'`, 4 tiers, mixed size-aware); the other 15 multi-tier services are `vehicle_size` (8) or `specialty` (7), each picking exactly one tier per quote_item. **Issue 40 finding (P3, separate concern):** two `service_pricing.tier_label` values carry operator-internal suffixes that read poorly in SMS prose: `floor_mats.tier_label = "Floor Mats Only"` (recommended `"Floor Mats"`), `carpet_mats.tier_label = "Carpet & Mats Package"` (recommended `"Carpet & Mats"`). Admin-UI edit per CLAUDE.md Rule 14 — no code, no migration. Operator makes the 2 edits AFTER Issue 39 ships and verifies. **Issue 41 finding (P2, cross-cutting, Target 9):** 5 visual surfaces render `quote_items.tier_name` as raw snake_case slugs ("per_row", "floor_mats") instead of joined `service_pricing.tier_label`: public quote page (`(public)/quote/[token]/page.tsx:268-269`), admin slide-over (`quote-slide-over.tsx:161-162`), POS quote detail (`quote-detail.tsx:557-558`), quote PDF (`pdf/route.ts:304`), public receipt (`receipt/[token]/page.tsx:236-238`). Customer-visible TODAY on Q-0084 quote link. Recommended Issue 41 follow-on adopts the same helper across all 5 surfaces — NOT bundled with Issue 39 per brief's "ship the helper + fix SMS chip first" guidance. **Implementation scope (Issue 39):** ~1 focused session (60-90 min). Files: 1 new helper + 6 adoptions + 1 unit test file (+14 cases) + 1 adoption pin test file (+6 cases). Backward compatible — only multi-tier same-service cases change output. **Operator questions surfacing: ZERO.** All 7 locked decisions resolve empirically. One optional refinement flagged (uniform parens for vehicle_size/specialty single-tier qty=1; audit recommends NO). **Deferred to implementation (Issue 39):** D44 locks at session start with `Option X` qty rule + Target 5 condition-(c) disambiguation for single-tier scope services. **No code changes in this audit session.** |
