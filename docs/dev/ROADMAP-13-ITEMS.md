...
**Document version:** v3.7 (2026-05-19) — Items 1, 6, 12, 15a, 15b, 15c completed; 15d deferred; 15e scoped; **Item 15f COMPLETE (all sub-layers + Phase 1 Layers 8a + 8b + 8c + 8d + 8d-bis + 8e + 8f done — Phase 1 closed on `10f7cffb`)**; **Item 15g COMPLETE (all 5 layers done)**. **v3.7 adds**: Roadmap Snapshot summary (13-item status table + roll-up) and Out-of-Scope Workstreams section covering SMS AI v2, Next.js security upgrade, original triage, infra hardening, codebase JSONB audit, and process improvements shipped 2026-05-18 → 2026-05-19.
...
| P0 fix: `ai.txt` JSONB write + migration + regression test | ⚪ not started | Customer-facing. Public `/ai.txt` endpoint serves quoted JSON blob to AI crawlers instead of valid directives. ~30min CC work. |
| P1 fix: QBO module coordinated migration (4 routes + 2 lib modules + migration + tests) | ⚪ not started | Must be coordinated — half-migrated state would break QBO entirely. Self-consistent today (write `JSON.stringify`, read strip-regex). |
| Out-of-scope flag: `voice-calls-poll` cron brittle JSON handling | ⏸ deferred | Self-consistent today. Address in dedicated session if voice agent reliability work resumes. |
| Out-of-scope flag: coupon-enforcement admin component cross-consumer drift | ⏸ deferred | Silent UI corruption — saved `'hard'` reads back as `'"hard"'`. Some consumers compensate (`pos/coupons/validate` aggressive strip-all-quotes), others don't (`pos/promotions/available` silently breaks under hard-mode enforcement). Isolated impact. |
...
Phase 1 (feature work, 15f) is closed. Two natural sequences from here:

**Track 1 — Feature roadmap continuation:** 15e (POS appointment-modal parity) is next; unblocks 15d. Then the tip cluster (2 / 3 / 4) since they share infrastructure. Items 7–11 are independent small/medium fixes that can slot in anywhere. 13 and 14 are the largest greenfield builds — best saved for last.
