...
|---|---|---|
| Session 1: Diagnostic — refined-flow tool surface audit | ✅ done (2026-05-23, session #56) | Read-only audit completed. Output: `docs/dev/WORKSTREAM_J_SESSION_1_DIAGNOSTIC.md` — complete implementation spec for Sessions 2-5 across 10 audit targets. **Major findings:** Admin Purge cascade is more complete than Issue 28 assumed — DOES delete conversations, messages, vehicles, quotes, sms_consent_log, ~20 other tables; two genuine gaps remain (`coupons.customer_id` SET NULL not deleted, `print_jobs.payload` may carry PII after `transaction_id` is SET NULL); Issue 28 conversation-persistence pattern more likely from operator using soft-delete DELETE handler (Open Question Q1). Rate-limit attribution in Issue 26 misidentified — `MAX_AI_REPLIES_PER_HOUR=25` is webhook-level gate, not tool-level; when fires, agent never runs. D20 mostly already satisfied — quote_status already SELECTed and refreshed per-inbound; minimal work is adding `valid_until`, `accepted_at` to SELECT. `get_availability` reliable, keep. Tool schema gaps confirmed: `send_quote_sms` lacks `customer_type` + `notes`; `create_appointment` Branch B writes $0 pricing (Issue 25 evidence); new `convert_quote_to_appointment` tool wraps existing voice-agent endpoint Branch A. `notify_staff` enum needs `quote_sms_failed` reason. 14 gaps identified, 3 open operator questions surfaced. |
| Session 2: New tool `convert_quote_to_appointment` + extend `send_quote_sms` | ⚪ not started | Depends on Session 1. New tool: `convert_quote_to_appointment(quote_id, scheduled_at, notes?)` — wraps the staff-side quote conversion logic; returns appointment_id. Extend `send_quote_sms` schema to accept `customer_type` (per session #53 finding) and `notes` (for preferred-time capture pre-conversion). Tests + verification. |
| Session 3: Prompt update — refined flow rules + relax D19 absolute | ⚪ not started | Depends on Session 2. Replace D19 absolute "never book" rule with refined flow rules per D20-D29. Remove canonical text from session #53 that says "team will call to confirm scheduling — agent never books." Add multi-quote disambiguation (D22), time-asking rule (D24), reschedule rule (D26), cancellation deflection (D27). Expected prompt compression — refined flow is shorter than the rigid absolute rules. Target: -3K to -5K chars. |
| Session 4: Customer context refresh + cleanup | ⚪ not started | Depends on Session 2. Implement D20 (quote_status refresh in customer-context). Audit any references to `create_appointment` that need removal/restriction now that `convert_quote_to_appointment` is the canonical path. Possibly remove `create_appointment` from `SMS_AI_V2_TOOLS` if no longer needed. |
| Session 5: Live verification + observation harvest | ⚪ not started | Depends on Sessions 2-4. Operator runs new-customer test against refined flow. Verify pending appointment creation, quote acceptance, notify_staff firing, calendar visual distinction. Document any new observations as Issues 26+. Tag any rough edges for follow-up. |
...
- Session 2 (new tool) and Session 4 (context refresh) are tool/endpoint
  changes that can run in parallel
- Session 3 (prompt) goes after both tool/endpoint changes land
- Session 5 (verification) is last
...
|---|---|---|---|---|---|
| 2026-05-23 | 56 | Workstream J Session 1 diagnostic — refined-flow tool surface + Admin Purge audit | Docs only (no code changes) | _(this commit)_ | Branch `audit/workstream-j-session-1-diagnostic`. Read-only audit across 10 distinct surfaces. Output: `docs/dev/WORKSTREAM_J_SESSION_1_DIAGNOSTIC.md` (new) — complete spec for Sessions 2-5. **Major findings:** (1) Admin Purge cascade is more complete than Issue 28 assumed — the Purge route at `src/app/api/admin/customers/purge/route.ts` DOES delete conversations (by phone OR customer_id), messages, vehicles, quotes, sms_consent_log, and ~20 other customer-attached tables; two genuine gaps remain — `coupons.customer_id` is nulled (not deleted) and `print_jobs.payload` (TEXT) may contain receipt PII after `transaction_id` is SET NULL; Issue 28's persisting-conversation pattern more likely explained by operator using the per-customer DELETE handler (soft-delete only, does NOT touch conversations) instead of Data Management Purge — **Open Question Q1** flagged for operator. (2) Rate-limit attribution in Issue 26 is misidentified — `MAX_AI_REPLIES_PER_HOUR = 25` is a webhook-level gate (`twilio/inbound/route.ts:67`) checked BEFORE v2 routing; when limit fires, agent NEVER runs, so the "phone number issue" notify_staff attribution did NOT come from a rate-limited tool call but from an earlier turn where `send_quote_sms` returned 400 "Invalid phone number" and agent paraphrased. (3) D20 is mostly already implemented — `customer-context.ts` already SELECTs `quotes.status` and `getCustomerContext()` runs per-inbound (`agent-runner.ts:265`); minimal D20 work is adding `valid_until`, `accepted_at`, `sent_at`, `viewed_at` to existing SELECT (zero new round-trips). (4) `get_availability` tool is reliable — reads live DB, no caching, correct conflict checks; Issue 23 hallucination is prompt-side reasoning failure (already addressed session #53), tool kept. (5) Tool schema gaps confirmed — `send_quote_sms` doesn't accept `customer_type` or `notes`; `create_appointment` Branch B is the $0.00 surface (line 549-551), Branch A (quote conversion) preserves pricing via shared `convertQuote()`; new `convert_quote_to_appointment(quote_id, date, time, notes?)` tool wraps Branch A. (6) `notify_staff` enum has 7 reasons — no `quote_sms_failed`. **Three open questions for operator:** Q1 (Purge vs DELETE path used in failed test), Q2 (new tool: dedicated wrapper vs reuse voice-agent endpoint), Q3 (remove `create_appointment` from tool surface or keep + tighten prompt). **Total gaps: 14 across 10 targets** (full risk matrix in diagnostic). **Estimated implementation:** Session 2 ~3-4h + Session 3 ~2-3h + Session 4 ~1-2h + Session 5 ~1-2h. No genuine P1 regressions surfaced. `docs/dev/ROADMAP-13-ITEMS.md` updated to flip Workstream J Session 1 sub-item from `⚪ not started` to `✅ done` and reference the new diagnostic deliverable. `docs/CHANGELOG.md` updated with TL;DR. No source code, prompt, migration, or test changes. Verification: `git diff --name-only` shows only `docs/dev/WORKSTREAM_J_SESSION_1_DIAGNOSTIC.md` (new), `docs/CHANGELOG.md`, `docs/dev/ROADMAP-13-ITEMS.md`. |
