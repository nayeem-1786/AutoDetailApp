...
| Layer 4: Twilio webhook integration + feature flag routing | ⚪ not started | **NOW UNBLOCKED.** Must re-check `is_ai_enabled` at outbound-send time (mid-conversation operator toggle protection). Return-early Twilio ACK pattern per audit §4.2 (mirror `voice-agent/finalize-call`). Runner runs inside the background task, not the webhook request handler. |
...
| Pre-existing typecheck residue cleanup | 🟡 in progress | Was 29 errors. The 2 CC-introduced Layer 1+2 errors (`vi.fn` arity in `notify-staff/__tests__/route.test.ts:42` + `sendSmsMock` type in `staff-notification.test.ts:299`) fixed during Layer 3a (2026-05-19). Remaining 27 in `src/lib/quotes/__tests__/quote-service.modifiers.test.ts` (TS2352 Supabase mock-to-real assertion mismatch repeated across lines 168-585) — separate follow-up. |
...
|---|---|---|---|---|---|
| 2026-05-19 | 38 | Workstream A — Addon authorization flow discovery audit (read-only) | Audit done | _(this commit)_ | Branch `audit/addon-authorization-flow`. Discovery deliverable `docs/dev/ADDON_AUTHORIZATION_FLOW_AUDIT.md` (~580 lines) — end-to-end trace of the in-job addon-authorization flow as input for SMS AI v2 Layer 4 planning. TL;DR answers 3 Layer 4 questions: (1) the flow is detailer-driven via the "Flag Issue" 7-step wizard in `flag-issue-flow.tsx` (893 lines, **NO AI message-drafting integration** — 3 hardcoded `MESSAGE_TEMPLATES` with placeholder substitution); (2) the `[AUTHORIZE_ADDON]` parse block at `webhooks/twilio/inbound/route.ts:866-913` is designed-on-purpose fallback code with the legacy single-shot AI prompt at `messaging-ai.ts:202-211` injecting addon context + emission rules (per `job-addons.ts:339-345`) — but live DB confirms the path has **NEVER fired in production** (`messages` ILIKE `%[AUTHORIZE_ADDON%` = 0 rows, `job_addons` table = 0 rows total, shop is pre-launch); (3) **SMS AI v2 has zero addon awareness** — grep confirms no file under `src/lib/sms-ai/` mentions "addon", `getCustomerContext()` doesn't load `job_addons`, no addon-shaped tool exists. §A-§H sections cover: detailer surface (POS Jobs card orange "Flag Issue" button gated by `pos.jobs.flag_issue` permission, mounts `<FlagIssueFlow>`); initiation endpoint (`/api/pos/jobs/[id]/addons` POST, POS-HMAC auth, writes `job_addons` row with `crypto.randomUUID()` token + `expires_at` from `business_settings.addon_auth_expiration_minutes` default 30 min, slug `addon_authorization` chip-driven, **NO `createShortLink` involvement** — raw `/authorize/<UUID>` path); customer landing page (`src/app/authorize/[token]/page.tsx`, 314 lines, server-rendered, public token-scoped, auto-expires stale pending rows on visit, mounts `<AuthorizationClient>` 104-line two-button component); approve/decline endpoints (50 lines each, public, delegate to `approveAddon()`/`declineAddon()` helpers in `job-addons.ts`); **`jobs.services` JSONB NOT mutated** on approval — addon rows joined into ticket lazily at `checkout-items/route.ts:146-172`; **no detailer notification fires** on approve/decline (per PHASE8 design "in-app badge + console log for now"); helper sends second confirmation SMS to customer via slug `addon_approved`/`addon_declined` with `logToConversation: true`; `addon_authorization_expired` SMS in §E webhook block does NOT use `logToConversation` (Session 3A "Preserved-for-now"); schema (`job_addons` 21 columns, status CHECK enum `pending|approved|declined|expired`, issue_type CHECK enum 10 values, RLS enabled but policies `USING (true)` — service-role bypasses anyway); 10 follow-ups surfaced (0-row table = unexercised path; no detailer notification; checkout-time JOIN vs JSONB mutation; v2 zero addon awareness; wide-open RLS; unused-looking `/api/authorize/[token]` GET; expiry-reply no conversation log; email `?action=` query-param replay safety; `final_price` chip Path A vs Path B inconsistency; `job_addons` vs `service_addon_suggestions` naming collision). Constraints honored: zero `src/` changes; no test/migration/package changes; no design recommendations in deliverable (facts only). Lint 0 errors / 97 warnings (unchanged baseline). Typecheck 27 errors (unchanged baseline). Branch pushed for review, not merged. |
| 2026-05-19 | 37 | Workstream F — Three small cleanups (test scripts + SMS DEBUG revert + unused z import) | done | _(this commit)_ | Branch `chore/parallel-cleanups-2026-05-19`. Three independently-verifiable cleanups bundled into one commit. **Item 1:** added `"test": "vitest run"` and `"test:watch": "vitest"` to `package.json#scripts` between `typecheck` and `regen:sms-contracts` — `npm test` now works as the canonical test entry point. **Item 2:** removed the temporary `console.log('[SMS DEBUG] Twilio POST body:', formData.toString())` line and its "Revert in follow-up session" comment at `src/lib/utils/sms.ts:93-95` (Twilio 30034 diagnosis leftover). Verified no tests assert on the output via `grep -rn "SMS DEBUG"` — only the source line + 3 doc-only references in CHANGELOG / SMS_AI_V2_AUDIT / SMS_AI_V2_LAYER_3_DISCOVERY. **Item 3:** removed unused `import { z } from 'zod';` at `src/lib/utils/__tests__/validation-refund-shopuse.test.ts:2`. Confirmed unused (no `z.` references in the file's 204 lines — tests use `shopUseSchema` / `refundCreateSchema` from `../validation`). Verification: tsc 27 errors (unchanged), lint **97 warnings** (was 98 — Item 3 closed exactly one), 0 lint errors, `npm test` = 1754/1754 pass (unchanged), `npm run build` clean. |
