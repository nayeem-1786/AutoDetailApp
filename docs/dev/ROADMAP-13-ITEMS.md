...
| Session 4 (refined): update_customer pivot + send_quote_sms 60-sec idempotency guard (D35 + D36) | ⚪ ready | Empirical findings from 2026-05-23 evening multi-test verification (3 back-to-back tests, Q-0084 through Q-0087): upsert_customer fired 0 times across all 3 tests, send_quote_sms + create_appointment already handle creation correctly, findOrCreateVehicle correctly ADDS new vehicles without overwriting (one-to-many respected), customer record was correct at end-of-conversation in all 3 cases. Bundle three changes per D35 + D36: (a) rename `upsert_customer` → `update_customer`, endpoint behavior pivots to update-only with error-on-missing-customer + `instructions_for_agent` telling agent to use send_quote_sms / create_appointment to create; (b) three prompt rule rewrites — remove eager-creation language from D34, add "customer creation happens via send_quote_sms or create_appointment automatically", add "call update_customer when customer shares new info to enrich existing record" (vehicles explicitly excluded — findOrCreateVehicle handles them); (c) send_quote_sms 60-second idempotency guard for Issue 31 — match on customer_id + vehicle_id + service-set within last 60 sec → return existing quote_id with `was_duplicate: true` + `instructions_for_agent` silent-acknowledge. Tests updated for renamed tool and error-on-missing-customer behavior. Estimated 2-2.5 hours CC. |
