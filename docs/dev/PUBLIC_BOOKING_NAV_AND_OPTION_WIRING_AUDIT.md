...
> branch `fix/u-b-3-staff-assessed-request-quote-cta`). **W4 via
> Session #138** (Q-C-1 LOCKED Option A — line-item persistence mirror;
> branch `fix/u-b-4-is-taxable-mirror-products-pattern`) — see also the
> `Session #137` + `Session #138` entries in `docs/CHANGELOG.md`.
> **W5, W7, and Concern C remain open** — each gated on the
> corresponding operator decision (Q-D in the Open Operator Questions
> section).
