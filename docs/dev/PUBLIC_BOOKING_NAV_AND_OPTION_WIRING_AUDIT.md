...
> fix documentation. **W3 via Session #137** (Q-B LOCKED path (b);
> branch `fix/u-b-3-staff-assessed-request-quote-cta`) — see also the
> `Session #137` entry in `docs/CHANGELOG.md`. **W4, W5, W7, and
> Concern C remain open** — each gated on the corresponding operator
> decision (Q-C / Q-D in the Open Operator Questions section).
...
| "`is_taxable` honored" | ❌ **DEAD on booking deposit** | Hardcoded false in line items + zero tax UI at Step 4; POS finalization may apply tax (not in scope of this audit) |
