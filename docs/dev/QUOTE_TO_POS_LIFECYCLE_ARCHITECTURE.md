...
**Audit-informed update (Phase 3.0.1 audit `249c2673`, 2026-06-06):**
- **The two pre-existing identifiers use DIVERGENT generation mechanisms** — Quote uses (γ) application-level (`src/lib/utils/quote-number.ts`); Receipt uses (β) PostgreSQL BEFORE INSERT trigger (`supabase/migrations/20260201000037_create_functions_triggers.sql:35-54`). Phase 3 Theme A faces a 4-way fork (Z1 per-entity trigger / Z2 unified DB function / Z3 shared sequences table / Z4 PG sequence). See `docs/dev/NUMBERING_STRATEGY_AUDIT.md` Target E for the full landscape.
- **Format drift surfaced:** Quote's dormant DB trigger generates `Q-XXXXXX` (6-digit, never fires); production format is `Q-XXXX` (4-digit, app-level). AC-10's appointment_number format details (pad width, starting value) are open per Phase 3.0.1 F.2.
- **No existing generator is race-safe.** All four (Quote, Receipt, PO, Order) rely on UNIQUE constraint to fail-fast under concurrent contention; none implement retry. AC-10's race-safety stance is open per Phase 3.0.1 F.4.
- **Quote's items-error cleanup path introduces a number REUSE risk window** (not present in Receipt's β pattern). Phase 3 Theme A must lock AC-10's rollback semantics — affects which option family (γ vs β vs Z3/Z4) is acceptable.
- **The dormant `generate_quote_number()` trigger** in `20260201000037_create_functions_triggers.sql:79-98` should be assessed for deletion as part of any AC-10 implementation (Phase 3.0.1 F.6) — leaving a third dormant trigger compounds the maintenance hazard.
- **Six open operator decisions** (F.1–F.6) surfaced for Phase 3 Theme A planning: existing-pattern vs unified refactor, appointment_number format details, gap tolerance, race-safety design goal, job-number derivation strategy, dormant-trigger cleanup.
...
**Audit-informed update (Phase 3.0.3 audit `<MERGE_HASH>`, 2026-06-06):**
