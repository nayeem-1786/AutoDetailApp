...
**Implementation status (post-Theme F 2026-06-07):** AC-10 fully operational; β-triggers retired via Theme A.1 follow-up migration. AC-15 schema foundation laid via Theme E.1 — `customer_credits` table + types + repository deployed; E.2 (apply logic) + E.3 (operator UI) still pending. Phase 0.2 audit (`dcf511df`) findings F.2 / F.3 / F.5 / F.6 / F.7 closed via Theme F — walk-in seam now writes `quotes.converted_appointment_id`, A.3 forwards modifiers, dormant `/api/quotes/[id]/convert` endpoint deleted, converted-state views surface a "View Appointment" deep-link to admin appointments, and `convertQuote()` carries a two-arm race idempotency guard (foundational for Theme C's customer-accept auto-conversion). See Decisions Log entries 2026-06-07 PST (Theme A), 2026-06-07 PST (Theme A.1), 2026-06-07 PST (Theme E.1), and 2026-06-07 PST (Theme F).
