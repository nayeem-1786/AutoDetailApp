...
**Version:** 1.3
**Locked:** 2026-06-04 10:30 PST (v1.0); 2026-06-05 21:30 PST (v1.1); 2026-06-06 17:00 PST (v1.2); 2026-06-06 22:20 PST (v1.3)
...
### AC-10: Unified ticket number scheme — 6-digit format, shared sequences, all five identifiers (LOCKED v1.3)

**Commitment:** The unified identifier strategy uses the **appointment as the spine** for customer-facing reference, but the unification is **broader than appointment_number alone**. All FIVE human-readable operator/customer identifiers in the codebase converge on a single shared generation mechanism with a uniform 6-digit format. Theme A is the foundational identifier infrastructure session for the entire codebase; no follow-up "circle back and clean up WO/PO" patch-work sessions.

**Customer-facing identifier across the lifecycle:** the appointment_number (A-XXXXXX) is the customer's reference from appointment creation forward. The customer sees Q-XXXXXX during the quote phase only; receipts carry SD-XXXXXX as the financial-record identifier. Jobs inherit the appointment's A-XXXXXX (no separate prefix) — operationally tied. Transactions keep SD-XXXXXX as the receipt identifier.

#### Format specifications (LOCKED)

All five identifiers use a uniform 6-digit format generated by the shared mechanism below:

| Identifier | Format | Starting value | Migration status |
|---|---|---|---|
| Quote | `Q-XXXXXX` | `Q-100000` (new quotes); existing 4-digit `Q-XXXX` rows preserved as-is | Expand from 4-digit |
| Appointment | `A-XXXXXX` | `A-100000` | Greenfield (new column) |
| Receipt | `SD-XXXXXX` | unchanged from current `MAX(receipt_number_numeric) + 1` | Mechanism change only (already 6-digit) |
| Work Order | `WO-XXXXXX` | `WO-100000` (new orders); existing 5-digit `WO-XXXXX` rows preserved as-is | Expand from 5-digit |
| Purchase Order | `PO-XXXXXX` | unchanged from current `MAX(po_number_numeric) + 1` | Mechanism change only (already 6-digit) |

The migration is forward-only — no historical renumbering of existing rows. The current widths in production are 4-digit Q, 6-digit SD, 6-digit PO, 5-digit WO (verified by Phase 3.0.1 audit `249c2673`); the locked target is uniform 6-digit across all five.
...
| `pad_width` | `INT NOT NULL` | Zero-padding width — locked at `6` for all five rows |
| `updated_at` | `TIMESTAMPTZ NOT NULL DEFAULT NOW()` | Last increment time |

**Database function:** `next_identifier(entity_type TEXT) RETURNS TEXT` performs a `SELECT ... FOR UPDATE` on the matching row, increments `current_value`, formats `prefix || lpad(current_value::text, pad_width, '0')`, and returns the formatted string (e.g., `'Q-100001'`). The row-level lock serializes concurrent generators — only ONE caller can advance any given counter at a time. The function returns the formatted display string, not just the raw integer; format-rendering responsibility lives at the database layer, not in application code.

**Initial seed values** at migration time, per identifier type:

- `quote`: `current_value = 100000` (so first new quote becomes `Q-100001`)
- `appointment`: `current_value = 100000` (so first new appointment becomes `A-100001`)
- `receipt`: `current_value = (SELECT MAX(receipt_number_numeric) FROM transactions)` — continues the existing sequence; first new receipt is `MAX + 1`
- `work_order`: `current_value = 100000` (so first new work order becomes `WO-100001`)
- `purchase_order`: `current_value = (SELECT MAX(po_number_numeric) FROM purchase_orders)` — continues the existing sequence

Receipt + PO seeds use the existing-row maximum because both already use a 6-digit format in production and there is no semantic reason to introduce a gap; Quote + WO seeds restart at 100000 because the format expansion (4→6 / 5→6) makes the prior counter values incompatible with the new pad-width invariant; Appointment is greenfield with no prior rows.
...
- Existing identifiers preserved as-is in their pre-Theme-A formats (no backfill of historical `Q-1234`, `WO-10042`, etc.) — historical rows stay readable and reportable
- New identifiers start at their locked seed value per the table above
- The `identifier_sequences` rows are written by the Theme A migration; subsequent INSERTs go through `next_identifier()`
- `appointment_number` column added to `appointments` with `UNIQUE NOT NULL` constraint and a Theme A backfill that calls `next_identifier('appointment')` for every existing row (one-time, run inside the migration)
...
- **Operator decision (2026-06-06 evening session):** **best-in-class principle applied — unify ALL five identifier systems**, not just the three customer-facing ones (Q, A, SD). Future-guarding the WO path requires expansion from 5 to 6 digits same as Q. Per operator: *"future guarding all paths."* All five identifiers (Q, A, SD, WO, PO) unify under the shared `identifier_sequences` mechanism in a single landing session.
- **Operator decision (Option γ from earlier discussion preserved):** appointment-spine is cleanest because appointment is the only entity that exists across all lifecycle paths (online, SMS, phone, walk-in via quote, walk-in direct). Customer-facing reference is A-XXXXXX; jobs inherit; transactions keep SD-XXXXXX as the financial-record axis.
...
**Theme A scope estimate:** 300–500 prod lines + 1 migration + 20–30 tests in a single session (call-site updates across quote/order/receipt/PO creation paths; new `identifier_sequences` table + `next_identifier()` function + appointment_number column add + backfill; dormant trigger drop; concurrency test coverage).
...
| **Phase 3 themes** | Cross-cutting (Theme A unified identifiers [AC-10], Theme B pending/confirmed semantic [AC-11], Theme C Quote→Appointment formalized [AC-12], cancellation fee [AC-14], customer credits [AC-15], cancel-with-payment [AC-9]) | Phase 0.1 + 0.2 + refund/credit + 3.0.1 + 3.0.2 + 3.0.3 audits complete | `[ ]` Not started — **ready to detail** (all six informant audits merged; AC-10 / AC-11 / AC-12 v1.3 refinements landed) |
...
- Theme A — unified identifier infrastructure (5-system 6-digit unification + shared `identifier_sequences` + `next_identifier()`) per [AC-10](#ac-10-unified-ticket-number-scheme--6-digit-format-shared-sequences-all-five-identifiers-locked-v13)
