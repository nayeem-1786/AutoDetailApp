...
**Implementation status (post-Theme C.1 2026-06-07):** AC-10 fully operational; β-triggers retired via Theme A.1 follow-up migration. AC-15 schema foundation laid via Theme E.1 — `customer_credits` table + types + repository deployed; E.2 (apply logic) + E.3 (operator UI) still pending. Phase 0.2 audit (`dcf511df`) findings F.2 / F.3 / F.5 / F.6 / F.7 closed via Theme F — walk-in seam writes `quotes.converted_appointment_id`, A.3 forwards modifiers, dormant `/api/quotes/[id]/convert` endpoint deleted, converted-state views surface "View Appointment" deep-link, `convertQuote()` carries two-arm race idempotency guard. **AC-12 schema foundation laid via Theme C.1** — `appointment_channel` enum extended with `'customer_accept'`; `appointments` extended with `staff_acknowledged_at` + `scheduled_date_placeholder` + `quote_id` (with UNIQUE partial index backed by backfill from `quotes.converted_appointment_id`, completing the AC-12 race-protection contract at the DB layer); `pending_appointment_sla_alert` SMS template seeded. **C.1 is foundation only — Theme C.2 (handler refactor + SLA cron logic + placeholder VALUE strategy + customer-facing UX) is the next AC-12 session.** See Decisions Log entries 2026-06-07 PST (Theme A), 2026-06-07 PST (Theme A.1), 2026-06-07 PST (Theme E.1), 2026-06-07 PST (Theme F), and 2026-06-07 PST (Theme C.1).
...
| **Phase 3 themes** | Cross-cutting (Theme A unified identifiers [AC-10], Theme B pending/confirmed semantic [AC-11], Theme C Quote→Appointment formalized [AC-12], cancellation fee [AC-14], customer credits [AC-15], cancel-with-payment [AC-9]) | Phase 0.1 + 0.2 + refund/credit + 3.0.1 + 3.0.2 + 3.0.3 audits complete | `[~]` **Theme A merged** (2026-06-07 — AC-10 v1.4 implementation + 6 migrations applied to production) and **Theme A.1 follow-up merged** (2026-06-07 — `tr_transaction_receipt_number` + `tr_po_number` triggers dropped via 7th migration after production-verified next_identifier() adoption); **Theme E.1 merged** (2026-06-07 — AC-15 schema foundation: `customer_credits` table + ENUM + types + repository deployed; E.2 credit-application + E.3 operator UI still pending); **Theme F merged** (2026-06-07 — Phase 0.2 audit `dcf511df` cleanup bundle: F.2 walk-in writes `converted_appointment_id`; F.3 A.3 modifier forwarding; F.5 dormant admin convert endpoint deleted; F.6 "View Appointment" deep-link from converted quotes; F.7 two-arm race idempotency guard in `convertQuote()` — foundational for Theme C); **Theme C.1 merged** (2026-06-07 — AC-12 schema foundation: `appointment_channel` enum extended with `customer_accept`; `appointments` extended with `staff_acknowledged_at` + `scheduled_date_placeholder` + `quote_id` UNIQUE; `pending_appointment_sla_alert` SMS template seeded; race protection at DB layer complementing Theme F's F.7 application-level guard); **Theme C.2 (handler refactor + SLA cron + placeholder VALUE strategy + customer UX) + Theme B + AC-9 + AC-14 + remainder of AC-15 not started** |
