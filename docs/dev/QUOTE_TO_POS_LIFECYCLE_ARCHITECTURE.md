...
**Implementation status (post-Theme C.1 2026-06-07):** AC-10 fully operational; β-triggers retired via Theme A.1 follow-up migration. AC-11 webhook gap closed via Theme B.1 (`48884207`) — Stripe `payment_intent.succeeded` on `appointment_payment_link` metadata now flips `appointments.status` pending → confirmed with race-protected UPDATE filter + audit + outbound `appointment_confirmed` n8n webhook (mirroring admin PATCH / POS PATCH / convertQuote cascades). AC-15 schema foundation laid via Theme E.1 — `customer_credits` table + types + repository deployed; **AC-15 application logic operational via Theme E.2** (`9e8fa5df`) — `applyCustomerCredit` / `applyCustomerCreditsToTransaction` / `unapplyCustomerCredit` repository surface + `POST /api/pos/transactions/[id]/apply-credit` endpoint + audit log integration; E.3 (operator UI) still pending. Phase 0.2 audit (`dcf511df`) findings F.2 / F.3 / F.5 / F.6 / F.7 closed via Theme F — walk-in seam writes `quotes.converted_appointment_id`, A.3 forwards modifiers, dormant `/api/quotes/[id]/convert` endpoint deleted, converted-state views surface "View Appointment" deep-link, `convertQuote()` carries two-arm race idempotency guard. **AC-12 schema foundation laid via Theme C.1** — `appointment_channel` enum extended with `'customer_accept'`; `appointments` extended with `staff_acknowledged_at` + `scheduled_date_placeholder` + `quote_id` (with UNIQUE partial index backed by backfill from `quotes.converted_appointment_id`, completing the AC-12 race-protection contract at the DB layer); `pending_appointment_sla_alert` SMS template seeded. **C.1 is foundation only — Theme C.2 (handler refactor + SLA cron logic + placeholder VALUE strategy + customer-facing UX) is the next AC-12 session.** See Decisions Log entries 2026-06-07 PST (Theme A), 2026-06-07 PST (Theme A.1), 2026-06-07 PST (Theme E.1), 2026-06-07 PST (Theme B.1), 2026-06-07 PST (Theme E.2), 2026-06-07 PST (Theme F), and 2026-06-07 PST (Theme C.1).
...
**Status:** Theme G merged to `main` at `<MERGE_HASH>` (feature commit `<FEAT_HASH>`, branch `refactor/phase-3-theme-g-webhook-dead-code-removal`). Migration `20260607231959_drop_n8n_webhook_urls_setting.sql` applied to production. All 4 verification gates green: typecheck 0 errors; lint 0 errors / 97 baseline warnings (unchanged from main); build clean; vitest 3156 passed / 66 skipped / 0 failed.
...
**Scope:** five-finding cleanup bundle from the Phase 0.2 Quote → Appointment conversion audit (`dcf511df`). Session 1.7 (`f87aca58`) already closed F.1 (conditional `appointment_confirmed` webhook fire); Theme F closes the remaining five. F.4 (webhook-name-vs-status) and F.8 (accept-without-follow-through) stay deferred to AC-12 / Theme C territory. File-isolated items bundled because they share audit lineage and surface area.
