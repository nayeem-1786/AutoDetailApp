...
src/app/admin/catalog/services/[id]/prereq-helpers.ts # Session #123 — pure helper `getEditPrereqOptions(allServices, prerequisites, editingPrereqServiceId)` replacing the prior `editingPrereq ? allServices : prereqEligibleServices` ternary that paired with `disabled={!!editingPrereq}`. Returns current value PLUS every unused service so the edit-mode dropdown preserves the selection while excluding other already-used prereqs that would collide with UNIQUE `(service_id, prerequisite_service_id)` at save. See `docs/dev/PREREQ_SERVICE_DROPDOWN_AUDIT.md`.
src/app/admin/catalog/services/[id]/__tests__/prereq-helpers.test.ts # Session #123 — 5 unit cases locking `getEditPrereqOptions` invariants (current value always included; unused included; other already-used excluded; degenerate no-prereqs; defensive duplicate-in-list). Mirrors #119 precedent: no admin-page render harness for a ~2150-line component.
