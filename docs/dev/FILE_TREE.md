...
src/app/api/book/__tests__/loyalty-affordability.test.ts # Q2 (#162) — 4 route-level tests pinning the booking-side loyalty affordability gate + canonical recompute wiring: redemption within balance → 201 + server-recomputed loyalty_discount persisted on the appointment; redemption over balance → 422 + NO appointment written; tampered client loyalty_discount → server overrides with points-derived value; zero-points → no-op. Harness mirrors `deposit-tax-persistence.test.ts`. Booking does NOT debit the balance (deferred to Phase 3).
...
```
src/app/api/pay/[token]/intent/route.ts   — POST: create PI for /pay/[token] (Pay-Link Session 2). Session #159 (Item 2, 2026-06-20) extended to accept optional `tipCents` in body (full-payment links only); rejects with 422 on non-integer/negative/partial-link/>chargeCents/malformed-JSON; includes in PI amount + `metadata.tip_cents`; returns `tipCents` + `totalCents` in response.
src/app/api/pay/[token]/intent/__tests__/intent-tip-validation.test.ts        # Session #159 (Item 2, 2026-06-20) — 14 tests covering tipCents validation chain: no body → tip=0 + no metadata stamp; valid tip=2000 → PI amount=12000 + metadata.tip_cents="2000"; tipCents=0 explicit; non-integer 422; negative 422; non-number-string 422; partial-link + tip 422; partial-link + tip=0 allowed; partial-link + no body allowed; stored amount ≥ remaining → tip allowed (full-pay-by-coverage); tip > chargeCents 422; tip = chargeCents boundary; cancelled appt 409; already-paid bypass + alreadyPaid: true; malformed JSON 400; response shape lock.
```
...
src/app/api/webhooks/stripe/route.ts                                          # Phase 3 Theme D.3 (AC-9 completion, 2026-06-07) — added `charge.refunded` event branch + `handleChargeRefunded(admin, charge)` handler. Reconciles refunds initiated outside the D.1 cancel-orchestration layer (manual Stripe dashboard refunds, Stripe API direct refunds, dispute-resolution refunds). Per-Refund idempotency via `stripe_refund_id` so D.1 prior inserts + Stripe retries are silently skipped. Multi-refund Charges processed correctly. Source tx status flips `completed`→`partial_refund`→`refunded` via cumulative `charge.amount_refunded`. Appointment.status intentionally NEVER touched — refund alone is not a cancel signal. | Q4 (Session #163, 2026-06-22) — `appointment_payment_link` branch now writes `transaction_items` (Path A) sourced from `appointment_services` + `services` + the appointment's mobile fields, mirroring the booking-deposit shape (`book/route.ts:744-819`); non-fatal + after the payments-row commit so the per-PI guard covers it. Closes the empty-items pay-link receipt gap (JOB_RECEIPT_UNIFICATION_AUDIT Q4). Item 2's subtotal/discount writes untouched; discount fidelity deferred to Option A Phase 3.
src/app/api/webhooks/stripe/__tests__/payment-intent-succeeded.test.ts
src/app/api/webhooks/stripe/__tests__/payment-link-items.test.ts             # Q4 (Session #163, 2026-06-22) — 6 tests covering the pay-link branch transaction_items write (Path A): service line from appointment_services with correct shape; non-taxable mobile_fee line when appt.is_mobile; multi-service writes all lines with is_addon derived from classification; empty appointment_services → console.warn + transaction/payment still commit; transaction_items insert failure → console.error + NO throw + payment/appointment update still succeed (non-fatal/idempotency guard); discount case → items at FULL price_at_booking while transactions.subtotal stays net appt.total_amount (locks the known pre-existing discount gap).
src/app/api/webhooks/stripe/__tests__/payment-link-tip.test.ts                # Session #159 (Item 2, 2026-06-20) — 9 tests covering pay-link branch tip extraction: no metadata = backwards compat (writes tip=0); valid tip → tip_amount + tip_net (5% CC fee) + payments.amount = subtotal; tip = amount_received boundary; tip > amount_received → cap + console.warn; non-numeric → 0; negative → 0; fractional → integer-floored; payment_status="paid" on full-pay+tip via subtotal compare; payment_status="partial" on under-pay+tip; regression — amount > remaining but tip-included still resolves to paid via subtotal-based math.
src/app/api/webhooks/stripe/__tests__/charge-refunded.test.ts                # Phase 3 Theme D.3 — 12 tests covering charge.refunded branch: happy full refund + tx flip + audit + appointment-NOT-touched assertion, partial refund flip, unknown PI log-and-skip (200), per-Refund idempotency (D.1 already recorded), multi-refund mixed idempotency (only new ones inserted), multi-refund cumulative-full-threshold, already-at-target-status no-op optimization, empty refunds array log-and-skip, missing payment_intent log-and-skip, signature regression lock (400), refund-insert failure throws (500 → Stripe retries), no-fireWebhook regression lock.
...
src/app/(public)/pay/[token]/pay-form.tsx        — Stripe Elements form for /pay/[token]. Session #159 (Item 2, 2026-06-20) split into `<AutoMountPayForm>` (partial-link, legacy preserved) + `<DeferredMountPayForm>` (full-link, new tip-first UX: three preset chips reading from `TIP_PRESETS` constant + Custom $ input + No Tip + Continue to Payment); shared `<ElementsHost>` / `<InnerForm>` / `<PreparingPanel>` / `<InitErrorPanel>` extracted; pure helpers `computeTipCents` + `parseCustomCents`. Stripe Elements card form deferred on full-pay until customer confirms tip (or No Tip = one-click commit).
src/app/(public)/pay/[token]/__tests__/pay-form-tip.test.tsx                  # Session #159 (Item 2, 2026-06-20) — 9 component tests covering branching + tip-chip behavior: partial-link auto-mounts Elements with no tip UI + no body in fetch; full-link renders chips (15/20/25/Custom) + No Tip + Continue without firing fetch (deferred mount); 20% chip → live tip + total reflect; Continue with 20% → POSTs `tipCents: 2000`; No Tip → POSTs `tipCents: 0` immediately (skips Continue review); Custom $7.50 → live total reflects; Custom empty → Continue disabled; Custom > 100% → Continue disabled; server error on Continue → surfaces message + stays on tip stage. Stripe mocks render simple stubs so deferred-mount can be observed.
...
### Loyalty
```
src/lib/loyalty/redemption-math.ts                  — Phase 1 (Option A, Session #158): cents-canonical redemption math — pointsToCents / centsToPoints / getRedeemableRange. See JOB_RECEIPT_UNIFICATION_AUDIT_2026-06-20.md §0 row 8. (Backfilled to FILE_TREE in Q2 #162 — was missed at #158.)
src/lib/loyalty/redemption-guard.ts                 — Q2 (#162): pure double-spend-window guard. resolveBookingLoyaltyRedemption (booking affordability check + canonical discount recompute via pointsToCents — anti-tamper; first consumer of the Phase 1 helper) + detectLoyaltyOverspend (close-out OBSERVATIONAL overspend flag) + insufficientLoyaltyErrorMessage. NO balance debit — structural fix deferred to Option A Phase 3 (must-close).
src/lib/loyalty/__tests__/redemption-math.test.ts   — Phase 1 helper tests (pointsToCents / centsToPoints / getRedeemableRange).
src/lib/loyalty/__tests__/redemption-guard.test.ts  — Q2 (#162): 13 pure-logic cases — affordability boundaries (equal/over/off-by-one/zero/new-customer), anti-tamper recompute anchored to LOYALTY.REDEEM_RATE_CENTS, overspend detection both directions + balance-drained-to-0 double-spend.
```
...
src/lib/sms-ai/background-dispatch.ts             # SMS AI v2 Layer 4 — runV2AgentInBackground({inboundMessageBody, conversationId, phone}). Fire-and-forget wrapper called from the Twilio webhook AFTER it has already returned 200 to Twilio. Loads businessName + businessHours + currentDate internally, calls runSmsAiV2Agent, on end_turn/max_iterations + non-blank text chunks via splitSmsMessage + sends each via sendSms + INSERTs outbound `messages` rows with channel='sms' (matches legacy; messages_channel_check allows only 'sms' or 'voice'; agent identity via sender_type='ai') + updates conversation last_message_at/preview. INSERT + UPDATE return values are error-checked (supabase-js does NOT throw on PG-side errors); failures are logged with code + message + details. Layer 6 (Session #154) — emits ONE `[SmsAiV2] event=conversation_close ...` line per inbound (canonical operator grep target with total_ms, iters, stop, tool_calls, chunks, reply_sent, prompt_source, cache_reads, cache_creates, optional error_class). On dispatch_thrown path emits TWO lines (dispatch_thrown carrying message + conversation_close) to keep close at 11-field cap. Transport errors (message_insert_error / chunk_send_error / conversation_update_error / business_info_load_error / business_hours_load_error) also emit under [SmsAiV2] with event= discriminator — pre-Layer-6 these used [SmsAiV2 background]. All paths try/catch — function NEVER throws.
src/lib/sms-ai/observability.ts                   # SMS AI v2 Layer 6 / Session #154 (2026-06-19) — observability seam. Owns: SmsAiV2ErrorClass enum (9 values: api_error / max_iterations / unknown_stop / no_reply / flag_load_error / flag_dispatch_threw / dispatch_thrown / tool_key_missing / prompt_load_error) + isV2ErrorClass type guard + CacheStats { reads, creates } + emptyCacheStats factory + formatLogFields key=value formatter (handles string/number/boolean/null/undefined; quotes strings with whitespace/= /"; skips null+undefined fields) + formatErrorMessage unknown→string normalizer. Single source of truth for the 3 prefix vocabulary ([SmsAiV2] / [SmsAiV2 runner] / [SmsAiV2 dispatch]) consolidating the 7 pre-Layer-6 prefixes. Tier 2 (future): audit_log persistence will mirror conversation_close as a structured row using the same 10-11 field schema.
...
src/app/api/pos/transactions/__tests__/loyalty-overspend-detection.test.ts  # Q2 (#162) — 4 route-level tests pinning the close-out OBSERVATIONAL overspend detection: redemption > live balance → `loyalty_overspend_detected` audit_log row written (customer_id/appointment_id/requested/available/delta) + close-out STILL succeeds (clamp unchanged, not enforcement); within balance → no row; exact boundary → no row; balance-drained-to-0 → full-delta row. Harness mirrors `auto-receipt-interlock.test.ts`.
...
Item 4 / 4a-cash additions (Session #165, 2026-06-24 — in-checkout cash tip capture):
- `src/app/pos/components/checkout/__tests__/cash-payment-tip-capture.test.tsx` # 7 tests — cash tip persists at both grains (txn + payment row), $0 zero-friction default, `change_given` excludes the tip, preset %-of-subtotal + cents rounding, "No tip" reset to $0, and chip rehydration from checkout-context across a step remount (no silent over-charge). Mounts the real `CheckoutProvider`.
- `src/app/api/pos/sync-offline-transaction/__tests__/tip-replay.test.ts` # 3 tests — offline replay writes the queued tip at both grains (`tip_net`=gross, change excludes tip); missing `tip_amount` (legacy record) → 0; negative → clamped 0. Harness mirrors `transactions/__tests__/loyalty-overspend-detection.test.ts`.
...
Phase 3 Theme D.1 (AC-9 foundation — cancel orchestration) additions:
- `src/lib/appointments/cancel-orchestration.ts` — canonical cancel-with-money-handling primitive. `cancelAppointmentOrchestrated(supabase, { appointmentId, pathway, reason, cancellation_fee_cents, notifyCustomer, cancelledBy, actor, ipAddress })`. Pathway A (refund) issues Stripe refund + writes `refunds` row; Pathway B (credit) issues `customer_credits` row via E.1's `createCustomerCredit`. Both pathways mark any non-terminal linked job as cancelled (mirrors A.4 pattern; does NOT call `executeUnMaterialize` which has the wrong semantic for cancel — its `transaction_linked` guard returns 409 when money is attached, which is precisely the case Pathway A handles). All side effects inline — NO `fireWebhook` (Theme G structural absence preserved). Consumed by all three cancel routes (admin / POS / customer). D.2 (cancellation fee policy from `business_settings`) + D.3 (`charge.refunded` webhook) build on this foundation.
- `src/lib/appointments/__tests__/cancel-orchestration.test.ts` — 25 tests: guards (404 / already_cancelled / terminal / invalid_pathway), Pathway A (happy / fee / fee>=paid / pay-on-site no-op / no-PI source / multi-source most-recent-wins / Stripe-fail / refunds-insert-partial-state / Stripe metadata / source transaction status flip), Pathway B (happy / no payment / credit-create partial state), job cascade (active job marked cancelled / terminal-job skip / no-job no-op), audit (canonical entry shape / source mapping for all four actor tags), notifications (fired only when notifyCustomer=true), Theme G compliance (no fireWebhook call / import / n8n_webhook_urls reference).
