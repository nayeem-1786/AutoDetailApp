...
- `vehicle_size`: 3–5 rows per service (sedan, truck_suv_2row, suv_3row_van + optional `exotic`, `classic`). Exotic/classic rows are OPTIONAL — absence means "not priced, POS gate opens modal"; presence with price > 0 means "tier-priced, POS skips modal for single-flag vehicles". Added via Session 28.
...
| id | UUID | PK | |
| product_id | UUID | NOT NULL, FK → products(id) | |
| adjustment_type | TEXT | CHECK ('manual','received','sold','returned','damaged','recount','shop_use','customer_retained','voided') | `voided` added via `20260424000003` (Session 42Q) — written by `void_transaction` RPC |
...
**RPC — `revert_stock_count(p_count_id UUID, p_user_id UUID, p_confirmed_drift BOOLEAN) RETURNS JSONB`:** atomic reversal of a committed count. Two-pass: (1) locks every affected product `FOR UPDATE`, then pre-checks for negative-quantity outcomes — if any reversal would push `quantity_on_hand` below zero, returns a structured error JSONB `{status: 'error', error_code: 'NEGATIVE_QUANTITY', problem_products: [...]}` without side effects. (2) Authoritatively checks drift (`stock_adjustments` on affected products with `created_at > committed_at` and `reference_type IS DISTINCT FROM 'stock_count'`); if drift exists and `p_confirmed_drift=false`, raises without side effects. (3) Otherwise walks `stock_adjustments` rows where `reference_type='stock_count' AND reference_id=<count_id>` (excluding prior reversals via `reason NOT LIKE 'Reversal of%'`), writes an inverse row per original with `adjustment_type='recount'`, updates `products.quantity_on_hand` back to pre-commit values, and flips `stock_counts.status` to `'cancelled'` with `cancelled_by/at` populated and a drift-context line appended to `notes`. Returns `{status: 'success', count_id, reversals_created, drift_count, drift_products}`. Negative-qty is checked **before** drift — drift is meaningless when the action is impossible. Defined in `20260424000001` (initial), structured-error variant in `20260424000002` (Session 42K-patch-1).

**RPC — `void_transaction(p_transaction_id UUID, p_user_id UUID, p_reason TEXT DEFAULT NULL) RETURNS JSONB`:** atomic void of a POS transaction. Locks the transaction row (`FOR UPDATE`), guards on `status='completed'` (returns structured `NOT_VOIDABLE` error if `voided` / `refunded` / `partial_refund`), bulk-locks every affected product row, then in a single PG transaction: (1) restores inventory by walking `transaction_items` with `product_id IS NOT NULL` — increments `products.quantity_on_hand` and writes a `stock_adjustments` row per item with `adjustment_type='voided'`, `reference_type='transaction'`, `reference_id=<tx>`, `unit_cost` snapshot from `products.cost_price`. (2) Reverses loyalty (always full — restores `loyalty_points_redeemed`, claws back `loyalty_points_earned`, writes `loyalty_ledger` rows + a single `customers.loyalty_points_balance` update clamped at 0). (3) Reverses coupon `use_count` (always full, unlike refund's partial-vs-full gate) and decrements campaign `redeemed_count` + `revenue_attributed` clamped at 0. (4) Decrements `customers.lifetime_spend` and `visit_count` clamped at 0 (the `tr_update_customer_stats` trigger is INSERT-only and never reverses on UPDATE — this RPC closes that gap for void only). (5) Cancels any non-cancelled job linked via `jobs.transaction_id` — sets `status='cancelled'`, `cancelled_at`, `cancelled_by`, `cancellation_reason='Transaction voided[: <reason>]'`. (6) Flips `transactions.status` to `'voided'`. Returns `{status: 'success', transaction_id, items_restored, units_restored, loyalty_restored, loyalty_clawed, coupon_reversed, campaign_reversed, job_cancelled, job_id, customer_id}` for success; `{status: 'error', error_code: 'NOT_FOUND' | 'NOT_VOIDABLE', current_status?: ...}` for errors. Defined in `20260424000003` (Session 42Q). Replaces the previous JS-based void handler that silently skipped inventory restoration.

**Permissions:** `inventory.counts.manage` (sort_order 507) gates commit/cancel/transition/items/read routes. `inventory.counts.revert` (sort_order 508, super_admin+admin=true, others=false) gates the revert routes specifically. Both enforced at the API layer. `pos.void_transactions` gates the API route that calls `void_transaction`.
